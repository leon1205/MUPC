...
> ⚠️ **已过期（单元 K，2026-09-18）**：本节标题与下列 4 条 REST/WebSocket 集成的**载体
> `mupc-web-api` 已整 crate 删除**（12-本地显示终端 设计 §7.2/§7.3：PRD PL-4/PL-5 要求取消
> 对外 HTTP 面），故这些接口**当前不存在**。OTA 状态/资源监控等信息的呈现改由本机触摸屏 HMI
> （`mupcd` 的 `console_host` 控制通道 + 显示终端读通道）承接；**本节仅保留历史沿革**，
> 后续 OTA 需求须另立出口并经评审。⚠️ 此处**只加过期标记，不做删改**（评审建议 S-2）。
...
| 版本 | 主要变更 |
|------|----------|
| v1.0 | 合并 OTA 升级与系统可靠性两份源文档，结构化重整理 |
