...
> 注：§11.1~§11.3（削峰填谷/需量控制/防逆流）随三策略废弃，仅作历史追溯。当前策略引擎的验收以台区储能治理策略（详见设计文档 §2）与 AI 指令校验（§11.4）为准。

### 11.1 削峰填谷（PS）【已废弃·历史追溯】
...
> v2.x 容量档位/SOC 源融合等实现侧演进见 04 设计文档版本演进表（SOC 源优先级设计见 04 设计文档 §2.11.1；版本演进行随实施补录）。
