...
[REVIEWED: PASS] — v2.2 电压感知 P/Q 协同控制

| 版本 | 日期 | 作者 | 状态 |
|------|------|------|------|
| v2.2 | 2026-06-05 | 需求分析师 | 草稿 |
...
|------|----------|---------------------|---------------------|----------|
| 光伏超发（中午） | 电压升高（>1.05 p.u.） | **充电**（负值，吸收有功）→ 消纳多余光伏电力，减少倒送 | **感性**（负值，吸收容性无功）→ 抑制电压抬升 | Q-V 耦合：吸收无功 = 降低电压 |
| 农网灌溉（抽水） | 电压降低（<0.95 p.u.） | **放电**（正值，注入有功）→ 补充系统能量缺口 | **容性**（正值，提供励磁无功）→ 就地补偿电机无功需求 | 减少线路无功电流 = 降低线路压降 = 抬升电压 |
| 末端低电压（夜间） | 电压降低（<0.95 p.u.） | **放电**（正值）— 仅当无功补偿不足时 | **容性**（正值）— 优先手段，不消耗 SOC | 就地无功补偿 → 减少长线路无功传输损耗 |
...
| A2 | q_batt_set | f64 | [-300.0, 300.0] | kVar | 装置无功功率设定值（负值=感性=吸收无功=降低电压，正值=容性=输出无功=抬升电压）|

> **v2.2 q_batt_set 符号定义验证：** 当前定义"负值=感性，正值=容性"完整且正确，覆盖全部三种电压控制场景：
> - **高电压 → 需要降压 → 感性负载吸收无功 → q_batt_set < 0** ✅
> - **低电压 → 需要升压 → 容性负载输出无功 → q_batt_set > 0** ✅
> - 符号约定与电力系统无功-电压（Q-V）耦合关系一致：感性吸收无功降低电压，容性输出无功抬升电压。
