...
[REVIEWED: PENDING] — v2.3 多场景独立模型架构

| 版本 | 日期 | 作者 | 状态 |
|------|------|------|------|
| v2.3 | 2026-06-06 | 需求分析师 | 草稿 |

---

> 本文档为 AI 优化引擎模块的权威需求文档。历史来源文档已在 v1.0 文档体系重构中合并，不再单独维护。
>
> **v2.3 核心变更：** 1 个通用 RL 模型覆盖 5 个场景 → 5 个独立 RL 模型（每场景 1 个）。每台 MUPC 设备出厂预装 1 个场景模型，OTA 按需推送其他场景模型。新增 ModelRegistry 管理模型生命周期（懒加载+双缓冲热切换）。LSTM 预测模型保持 1 个通用模型不变。详见第 4、5、8 章。
>
...
| 职责 | 说明 |
|------|------|
| 时序预测 | LSTM 模型预测光伏出力 / 负荷变化趋势（1 个通用模型，场景无关） |
| 预设运行场景选择 | 5 种预设运行场景，支持远程控制/本地选择，同一时刻仅 1 种互斥运行 |
| 多场景模型管理 | 5 个独立 RL 模型（每场景 1 个），ModelRegistry 按需加载、双缓冲热切换 |
...
历史数据 → LSTMModel.predict() → 光伏/负荷预测值 → 供 RL 模型使用
                                                         ↓
远程指令/本地选择 → ModeSelector → 运行模式 → ModelRegistry.switch_to() → 热切换场景 RL 模型 + 奖励函数切换
                                                         ↓
融合数据 + 预测值 + 运行模式 → RLModel.decide() → ActionOutput
                                                         ↓
ActionOutput → ActionValidator (5条约束规则校验) → strategy-engine
                                                         ↓
新数据积累 → OnlineUpdater.update(active_scene) → 当前场景模型权重增量更新 → 保存场景检查点
```
...
**v2.3 新增：** 每个场景对应 1 个独立 RL 模型（文件名见 4.2 节）。场景切换时，ModeSelector 触发 ModelRegistry 执行模型热切换（双缓冲机制，旧模型继续服务直至新模型就绪）。LSTM 预测模型为 1 个通用模型，不参与切换。

### 4.2 5 种预设运行场景

| 场景 ID | 场景名称 | 优化目标 | 适用工况 | RL 模型文件 |
|---------|----------|----------|----------|-------------|
| MODE-01 | 农网灌溉模式 | 最大化光伏消纳 + 电压治理 + 防止变压器过载 | 农网台区灌溉季（4月~9月） | `rl_agricultural.rknn` |
| MODE-02 | 自主套利模式 | 最大化峰谷电价差收益 + 最小化电池损耗 | 工商业台区有分时电价 | `rl_arbitrage.rknn` |
| MODE-03 | 需量控制模式 | 减免需量罚金 + 最小化舒适度损失 | 工商业台区需量接近合同值 | `rl_demand.rknn` |
| MODE-04 | 虚拟电厂模式 | 最大化辅助服务收益 + 响应精度 | 已注册 VPP 服务且有调度指令 | `rl_vpp.rknn` |
| MODE-05 | 极致绿色模式 | 最大化绿电消纳比例 + 最小化碳排放 | 绿色电力消纳目标 | `rl_green.rknn` |

> **v2.3 说明：** 5 个 RL 模型输入/输出形状完全一致（48 维 → 5 维），可在同一 RKNN Runtime 双缓冲框架下互换。差异仅在训练时使用的奖励函数和训练数据分布。
...
| 切换类型 | 前置条件 | 切换机制 | 延迟要求 |
|----------|----------|----------|----------|
| 本地热切换 | 目标模型已在本地 `/var/lib/mupc/models/` | 双缓冲热切换，旧模型继续服务至新模型就绪 | < 2s |
| OTA 下载+切换 | 目标模型需从 OTA 拉取 | OTA 下载 → SHA256 校验 → NPU 加载 → 热切换 | < 60s（含下载），下载期间当前场景不变 |

> **v2.3 说明：** 出厂预装模式保证设备大多数工况下执行的是"本地热切换"路径（< 2s）。"OTA 下载+切换"仅在设备首次切换到非出厂场景时触发。
#### 4.3.1 远程控制选择（调度主站）
...
**性能要求：** 远程指令下发到模式生效 < 2s（目标模型已在本地时）

#### 4.3.2 本地选择（Web UI / 配置文件）

| 方式 | 界面/位置 | 说明 |
|------|----------|------|
| Web UI | 仪表盘 → AI 控制面板 → 运行场景选择器 | 单选下拉框，显示各场景模型状态（就绪/下载中/不可用），确认对话框后即时生效 |
| 配置文件 | `mupc/config/ai.toml` → `[mode] factory_scene = "MODE-01"` | 设备出厂预装场景，系统启动时加载 |

**性能要求：** Web UI 选择后模式生效 < 2s（本地模型）/ < 60s（需下载）
...
设备出厂时预装 1 个场景对应的 RL 模型，由配置文件 `ai.toml` 的 `[mode] factory_scene` 字段指定（默认 `"MODE-01"`）。OTA 可推送其他场景模型到本地模型目录 `/var/lib/mupc/models/`。

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `[mode] factory_scene` | `"MODE-01"` | 设备出厂预装场景 |
| `[mode] model_dir` | `/var/lib/mupc/models/` | 场景模型文件存储目录 |
| `[mode] model_manifest` | `/etc/mupc/models/manifest.json` | 模型清单文件（文件名→SHA256 映射） |
...
pub enum RunningMode {
    AgriculturalIrrigation = 1,   // MODE-01：农网灌溉模式
...
pub struct ModeSelector {
    ...
    registry: Arc<ModelRegistry>,   // v2.3 新增：模型注册表引用
}

impl ModeSelector {
    pub fn new(initial: RunningMode, registry: Arc<ModelRegistry>) -> Self;
    ...
    /// v2.3 新增：获取所有场景的模型状态列表（供 Web UI 展示）
...
| ID | 标准 | 验证方法 |
|----|------|----------|
| MODE-01 | 系统启动后加载出厂配置指定的默认场景模型（配置文件缺失时默认 MODE-01） | 启动测试 |
| MODE-02 | 远程 IEC 104/61850 切换场景延迟 < 2s（目标模型已在本地），< 60s（需 OTA 下载） | 集成测试 |
| MODE-03 | 同一时刻仅 1 个场景生效，并发切换时最后到达的指令生效 | 并发测试 |
| MODE-04 | 场景切换时自动更新优化目标权重和奖励函数 | 集成测试 |
| MODE-05 | 场景切换锁定窗口 < 100ms，切换期间旧模型继续服务，AI 决策不中断 | 性能测试 |
...
| MODE-09 | Web UI 场景选择器显示各场景模型状态（就绪/下载中/不可用），不可用场景置灰 | UI 集成测试（v2.3 新增） |
| MODE-10 | OTA 推送场景模型后，对应场景状态自动从 NotLoaded → Ready | 集成测试（v2.3 新增） |
| MODE-11 | 场景切换时，旧模型继续服务直至新模型就绪，推理不中断 | 性能测试（v2.3 新增） |
...
| 模式切换过程中断电 | 持久化文件写入中断 | 重启时从持久化文件恢复，文件损坏则回退出厂场景 |
| 目标场景模型文件缺失 | 本地无对应 .rknn 文件 | 返回 SceneModelState::NotLoaded，Web UI 显示"不可用"，需通过 OTA 下发（v2.3 新增） |
| 目标场景模型文件损坏 | SHA256 校验失败 | 记录 ERROR 日志，尝试从 OTA 备份恢复，恢复失败则该场景标记为 Error（v2.3 新增） |
| 切换过程中模型加载失败 | rknn_init 返回错误 | 保持当前场景不变，记录 ERROR 日志，目标场景标记为 Error（v2.3 新增） |
...
RLModel 不再是 1 个通用模型覆盖 5 个场景，而是 5 个独立训练的 RL 模型（每场景 1 个），每个模型针对其场景的奖励函数和训练数据分布进行专项训练。5 个模型输入/输出形状完全一致（48 维 → 5 维），由 ModelRegistry 统一管理生命周期（加载/卸载/热切换）。LSTM 预测模型保持 1 个通用模型不变。
...
| A2 | q_batt_set | f64 | [-300.0, 300.0] | kVar | 装置无功功率设定值（负值=感性=吸收无功=降低电压，正值=容性=释放无功=抬升电压）|
...
| ACT-04 | pv_limit >= 0.1（防逆流场景除外允许降至 0.0） | 光伏限功率下限保护 |
...
### 5.6 模型注册表（ModelRegistry）— v2.3 新增

```rust
/// 场景模型注册表 — 管理 5 个场景 RL 模型的加载/卸载/热切换
pub struct ModelRegistry {
    /// 当前激活的场景（场景 + 对应的 RknnRuntime）
    active: Arc<RwLock<(RunningMode, RknnRuntime)>>,
    /// 备用模型槽（用于双缓冲热切换，切换完成后变为 active）
...
impl ModelRegistry {
    /// 创建注册表，初始加载出厂场景模型
    pub fn new(model_dir: &Path, factory_scene: RunningMode, manifest: HashMap<RunningMode, ModelManifestEntry>) -> Result<Self, AiEngineError>;
    ...
    /// 从 OTA 下载并校验模型文件
    pub async fn download_model(&self, mode: RunningMode) -> Result<(), AiEngineError>;
...
```
switch_to(new_mode):
  1. 若 new_mode == active.mode → 幂等返回
  2. 检查目标模型文件是否存在 + SHA256 校验
  3. 模型不存在 → 返回 SceneSwitchResult::Downloading
  4. rknn_init 加载到 standby 槽（任务在 spawn_blocking 中执行）
  5. 加载完成后，standby ↔ active 原子交换
  6. 旧 active 槽中的 RknnRuntime 延迟 30s 后释放（等待正在进行的推理完成）
  7. 更新奖励函数权重映射
  8. 广播 ModeSwitchEvent
  9. 返回 SceneSwitchResult::Switched
```
...
| ID | 标准 | 验证方法 |
|----|------|----------|
| RL-01 | RL 模型加载成功（出厂场景模型） | 单元测试 |
| RL-02 | RL 决策延迟 < 1s | 性能测试 |
| RL-03 | RL 决策综合回报相比固定策略提升 >= 20% | 对比实验（相同数据，AI 策略 vs 固定策略）|
| RL-04 | 5 个场景各有独立的 .rknn 模型文件，输入/输出形状一致（48→5） | 模型文件验证（v2.3 新增） |
| RL-05 | 本地热切换延迟 < 2s（双缓冲，不中断推理） | 性能测试（v2.3 新增） |
| RL-06 | OTA 下载+切换延迟 < 60s | 集成测试（v2.3 新增） |
| RL-07 | 切换期间旧模型继续服务，推理不中断 | 并发测试（v2.3 新增） |
...
**优化目标：** 最大化光伏消纳，最小化变压器过载风险（v2.1 移除电压治理项）
...
> **v2.1 说明：** 原 `R_voltage_quality`（电压质量奖励）已移除。电压数据（D5）已从状态空间移除，三相不平衡治理由实时控制核心模块独立处理。

| 权重 | 默认值 | 说明 | 可配置范围 |
|------|--------|------|------------|
| w1 | 1.0 | 光伏消纳奖励权重 | [0.0, 3.0] |
| w2 | 0.5 | 电池损耗惩罚权重 | [0.0, 2.0] |
...
|------|---------|---------|---------|---------|------|
| 农网灌溉 | 1.0(光伏消纳) | 0.5(电池损耗) | 2.0(变压器) | - | 变压器过载惩罚最重，v2.1 移除电压质量 |
...
| 更新目标 | LSTM 预测模型权重 + **当前激活的场景 RL 模型权重**（仅微调在用的模型，不影响其他 4 个场景模型） |
| 安全保护 | 在线微调阶段的 loss 连续 10 个周期不下降或上升时，停止微调并回滚至上一检查点 |
| 数据存储 | 保留最近 30 天训练数据，本地存储 <= 1GB |
| 检查点独立 | 每个场景模型维护独立的检查点文件，路径为 `/var/lib/mupc/checkpoints/{scene_name}/`，场景切换时自动保存/加载 |

**v2.3 变更说明：** 微调范围从"全部 RL 模型"变更为"仅当前激活的场景 RL 模型"。场景切换时自动保存当前模型微调检查点，加载目标模型的检查点继续微调。

### 8.3 数据流

```
新数据积累(32个样本) → OnlineUpdater.update(active_scene) → 当前场景权重增量更新 → 保存该场景检查点
场景切换 → 保存旧场景检查点 → 加载新场景检查点 → 新场景微调继续
```
...
| UPDATE-05 | 场景切换时自动保存旧场景检查点、加载新场景检查点 | 集成测试（v2.3 新增） |
| UPDATE-06 | 微调仅影响当前激活的场景模型，其他场景模型权重不变 | 集成测试（切换后验证其他模型 SHA256 不变）（v2.3 新增）|
...
| 指标 | 要求 |
|------|------|
| 单场景 RL 模型 INT8 量化后大小 | <= 5MB（.rknn 文件），LSTM 模型 <= 5MB |
| 5 个 RL 模型全量存储 | <= 25MB（LSTM 1 个 + RL 5 个，出厂仅预装 LSTM + 1 个 RL，OTA 按需下发其余 4 个） |
| 推理运行时内存占用 | <= 200MB（当前激活的 1 个 RL 模型 + LSTM 模型，standby 槽额外 <= 100MB 临时占用） |
| 训练数据本地存储 | <= 1GB（保留最近 30 天训练数据） |
| 场景检查点存储 | 每个场景 <= 100MB，5 场景合计 <= 500MB |
...
| ID | 标准 | 优先级 | 来源 |
|----|------|--------|------|
| MODE-01~MODE-11 | 预设运行场景选择、互斥机制与多模型管理 | P0 | 场景改造 PRD [REVIEWED: PASS] + v2.3 多模型扩展 |
...
| RL-04~RL-07 | 5 场景独立模型、本地热切换 < 2s、OTA 下载+切换 < 60s、切换不中断推理 | P0 | v2.3 多模型架构 |
...
| ID | 标准 | 优先级 | 来源 |
|----|------|--------|------|
| UPDATE-01~UPDATE-06 | 在线微调 + 场景检查点独立管理 | P1 | Phase3C 设计 + 场景 RL PRD + v2.3 多模型扩展 |
...
│   ├── model_registry.rs         # 模型注册表（v2.3 新增，5 场景 RL 模型懒加载/双缓冲热切换）
│   ├── lstm_model.rs             # LSTM 预测模型（1 个通用模型）
│   ├── rl_model.rs               # MADDPG/PPO 决策模型（含 FusedSystemState、ActionOutput）
│   ├── reward_calculator.rs      # 奖励函数计算器
│   ├── data_fusion.rs            # 多源数据融合引擎
│   ├── action_validator.rs       # 动作约束校验器
│   ├── online_updater.rs         # 在线微调（仅更新当前场景模型）
...
| 序号 | 修订项 | 修订位置 | 说明 |
|------|--------|----------|------|
| 1 | 1 个通用 RL 模型 → 5 个独立场景 RL 模型 | 1.2 / 1.6 / 1.7 / 5.1 / 5.6 | 每个场景训练 1 个独立 RL 模型（输入/输出形状一致），LSTM 保持 1 个通用模型不变 |
| 2 | 新增 ModelRegistry | 5.6（新节） | 管理 5 个场景 RL 模型的加载/卸载/双缓冲热切换，模型清单文件（manifest.json）索引各场景模型文件名与 SHA256 |
| 3 | 场景切换联动模型热切换 | 4.1 / 4.3 / 1.7 | ModeSelector.switch() 触发 ModelRegistry.switch_to()，双缓冲机制保证切换不中断推理 |
| 4 | 设备出厂场景配置 | 4.3.4（新节）/ 4.2 | 设备出厂预装 1 个场景模型，OTA 按需推送其余 4 个场景模型 |
| 5 | Web UI 场景选择器显示模型状态 | 4.7 (MODE-09/10/11) | 各场景标注模型状态（就绪/下载中/不可用），不可用场景置灰 |
| 6 | 场景切换性能分级 | 4.3（新增性能分级表） | 本地热切换 < 2s，OTA 下载+切换 < 60s |
| 7 | 在线微调范围变更 | 8.2 / 8.3 / 8.4 | 仅微调当前激活的场景模型，每个场景独立检查点，切换时自动保存/加载 |
| 8 | 边���条件新增模型异常处理 | 4.8（新增 3 行） | 模型文件缺失/损坏/加载失败的检测与处理 |
| 9 | 资源占用更新 | 9.3 | 5 个 RL 模型全量存储 <= 25MB，场景检查点 5×100MB=500MB |
| 10 | 模块结构新增 model_registry.rs | 附录 D | 新增 model_registry.rs 文件，online_updater.rs 和 mode_selector.rs 描述更新 |
| 11 | 文档版本号更新 | 文档头部/尾部 | v2.2 → v2.3 |

**修订依据：** 训练发现 1 个模型覆盖 5 个场景难以收敛 → 拆分为 5 个独立模型，每台 MUPC 设备根据销售客户场景部署对应模型

**重要架构说明：**
- 5 个 RL 模型输入/输出形状完全一致（48 维 → 5 维），可在同一双缓冲框架下互换
- LSTM 预测模型不拆分（预测任务场景无关），保持 1 个通用模型
- 设备出厂预装 LSTM + 1 个 RL（出厂场景），OTA 按需推送其余 4 个 RL
- v2.2 新增的三相电压（D1）与 v2.1 移除的电能质量 D5 是不同用途：
