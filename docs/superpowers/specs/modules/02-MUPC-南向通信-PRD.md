...
> 文档版本：v1.14（2026-09-26，**仅 §9.6.2 N-4 的口径修订**（PCS 写归属下放至南向），**未过需求评审、不构成重新评审、未加任何门禁标记**；**本行与文末版本演进表的 `v1.14` 行同步**）｜状态：**需求评审通过（终审 2026-09-21）**；v1.7 / v1.8 / v1.9 / v1.10 / v1.11 为评审通过后的 **§9 增量补登/订正**，v1.12 为 **§10 新增章节**，v1.13 为 **§10 的 4 项文档级订正 + 4 条建议改进**（设计评审附项落正文），**v1.13-r1 为 §10.7 AC-8-5 的"C4 的构造法"示例订正（判据未动）**，**v1.13-r2 为 §10.9 Q-21 的取值备案**，**v1.14 为 §9.6.2 N-4 的 PCS 写归属修订**（均不构成重新评审）｜§9 站级南向设备语义点表集成（S3b-2）；**§10 块级采集周期覆盖（告警位单独快采，S3b-3）**
>
...
> **[§10.7 AC-8-5 示例订正（v1.13-r1，2026-09-23）：仅订正"C4 的构造法"的例子，判据未动、不构成重新评审]** 来源 = **02 设计 §12 的第二轮设计评审（v1.13-r1）阻塞项 S-2**：AC-8-5 的"C4 的构造法"原写"同组 3 个 FC04 块…**取该组某块** `interval_ms: 1000`" —— 按 **02 设计 §12.4.1 的分组语义**，**只要有块声明了 `interval_ms`，该块就按 `eff` 自成一组**（另两块继承站周期 2000）⇒ 该块所在组 `T_组 = 267.12 ms`、`1.5×T_组 = 400.7 ms ≤ 1000` ⇒ **C4 通过**；实际触发的是 **C9**（`U = 267.12/1000 + 2×267.12/2000 = 0.534 > 0.5`），而 C9 文案不写 `1202` ⇒ 原例**取不到"文案含 `1202`"**、C4 实际无人覆盖。⇒ **已把该例改为"三块均声明 `interval_ms: 1000`"**（⇒ 仍为同一读组、`T_组 = 801.36 ms`、`1.5×T_组 = 1202.04 ms` ⇒ `1000 < 1202.04` ⇒ **规则 20 的 C4 分支先于规则 23 返回**、文案含 `1202`），并补一句"**该组内三块 `eff` 相同 ⇒ 仍为同一读组**"。
>
> **未改（本次订正的边界）**：**AC-8-5 的判据本身**（非法值逐条返回 `Err`、文案须含站 id + 块名 + **`1202`**）、**C1–C9 的任何约束内容**、**任何 AC 的判据**、**§10.5 第 3 行**（"单轮最坏耗时"—— 它在设计侧由 02 设计 §12.7 的**规则 24** 落点，**PRD 侧一字未动**）、§10.7 表内其它行、**§1–§9 的全部内容**、任何代码/配置；**`[REVIEWED: PASS: 2026-09-21]`（§9）与 `[REVIEWED: PASS: 2026-09-23]`（§10）标记原文未动**。**本次只订正构造法的例子，判据未动 ⇒ 不构成重新评审**。**02 设计 §12 同轮按第二轮设计评审意见修订（v1.13-r1，待复审；含新增校验规则 24）。**
...
| N-4 | **（v1.14 修订）PCS 的写操作不由南向站级采集面执行**。PCS 的通信与控制由 `mupc-southd::pcs::PcsHandle` 承载（02 设计 **§13**），写入口**收敛为 4 条**（`send_dual_param` / `send_tai_command` / `stop` / `restore_interlock_latched`）且全部经既有链路（AI 决策 / 台区储能兜底 / IEC104 `p_set` / S2 联锁停机原语）；`south_stations` 段**不再接受** `role: pcs`（规则 **P-3**）。**⚠️ 写审计事件（谁 / 何时 / 写了什么 / 回读值）尚未落地**，登记为待办 | **原口径"由类型系统保证只读"已不成立、也不必要**（`StationBus` 仍无任何写方法，但 PCS 已迁出站级采集面）⇒ 独占改由"**单一所有者 `PcsHandle` + 4 条受限写入口**"保证。与 S2 联锁争用 PCS 4 区 500 停机原语的处置见 02 设计 §13.1① / §13.5.3（latch 权威源仍在 storage/DB） |
...
> ⚠️ **N-4 v1.14 修订说明（2026-09-26）**：本次修订是 **PCS 通信与控制整体迁入南向**（02 设计 **§13**，
> ADR-014）的**需求侧连带回写**，与代码**同批**（02 设计 §13.12 约定"避免文档先于事实"）。
> **本次修订未过需求评审、未加任何门禁标记**；§9 的 `[REVIEWED: PASS: 2026-09-21]` 与 §10 的
> `[REVIEWED: PASS: 2026-09-23]` **原文未动、覆盖范围不变**（N-4 的口径已由本次修订取代）。
> **N-1 / N-2 不变**：`PcsHandle` 只投 `on_station_telemetry` + `on_station_offline`，不调
> `on_battery_soc` / `on_grid_package`。
...
| ID | 类别 | 事项 | 处理口径 |
|----|------|------|----------|
| **Q-21** | **待裁定（产品/架构）**｜【非阻塞】 | **位块周期的最终取值**：① 2000 ms；② **1000 ms（本 PRD 推荐）**；③ 500 ms。三者还能否"端到端也 ≤2 s"取决于 12 号 R-33 的屏侧口径（§10.8.1 表） | **推荐 ② 1000 ms**：与 `battery`/`meter_batt`/`fire` 同档、**零改 `poll_ms`**、满足 C1–C5；且使 HVAC 从"5 s 特例"回到与其余三站**同档**（12 号设计对那三站的算式已是 1.85 s ✓ / 2.35 s ✗，属 **R-33 已登记的口径问题**，不是 HVAC 独有）。**若要"含兜底路径也 ≤2 s"** ⇒ 取 ③ 500 ms **且**把段级 `poll_ms` 1000 → 500（全局 tick 加倍，`U_口` 4.85%，仍 ≪ C9 的 50%）。<br>**③ 附项**：本 PRD **未复算 `grid_meter` 站的单轮耗时**（§9.8.1 该行标"既有"、未给值）⇒ 补算后再把该行纳入 §10.5 的 C9 复核表（**不影响本章结论**：其 `U` 只要 ≤0.5 即可）。<br>**2026-09-24 备案：首例已按推荐值 ② **1000 ms** 落地（S3b-3 T8–T10，生效配置 `hvac_di.interval_ms = 1000`；`§10.4` 的 `U_后 = 2.68 %` 与 `AC-8-1/8-2/8-3` 的计数均按该值）。该取值仍待产品追认；若改判 ①/③ 则须同批改 `§10.4` 的 `U` 与 AC 计数（③ 还须把段级 `poll_ms` 1000 → 500）。** |
...
| 版本 | 主要变更 |
|------|----------|
| **v1.14（2026-09-26，**§9.6.2 N-4 的口径修订 —— PCS 写归属下放至南向**；**未过需求评审、未加门禁标记**）** | **只改 §9.6.2 强制否定项表的 N-4 一行（+ 一行修订说明）**，来源 = 用户 2026-09-26 就 §11.8① 的裁定（**PCS 通信与控制整体迁入南向**，02 设计 **§13** / ADR-014），**与代码同批回写**（02 设计 §13.12："避免文档先于事实"）。**原口径**"`pcs` 站**由类型系统保证不得执行任何写操作**（FC05/06/0F/10）"⇒ **已不成立、也不必要**：`StationBus` 仍无任何写方法（只读保证保留），但 PCS **已迁出站级采集面** ⇒ 独占改由"**单一所有者 `PcsHandle` + 4 条受限写入口**"（`send_dual_param` / `send_tai_command` / `stop` / `restore_interlock_latched`，全部经既有链路）＋ **`south_stations` 不再接受 `role: pcs`（规则 P-3）** 保证。**并如实登记**：**写审计事件（谁 / 何时 / 写了什么 / 回读值）尚未落地**（02 设计 §13.5.3 明列"须新增的落地项"，落点见 `technical-debt.md` §6.13）。**未改**：N-1 / N-2 / N-3 / N-5 / N-6 的否定项与依据、**§9 的其余任何裁定**、§1–§8 / §10 的全部内容、任何代码/配置/设计文档。**§9 的 `[REVIEWED: PASS: 2026-09-21]` 与 §10 的 `[REVIEWED: PASS: 2026-09-23]` 标记原文未动、覆盖范围不变**；**本次未加任何门禁标记**（N-4 的新口径**待独立需求评审**）。 |
| **v1.13-r2（2026-09-24，**仅 §10.9 Q-21 备案**，判据未动、不构成重新评审）** | **只在 §10.9 Q-21 那一格的处理口径末尾追加一句备案**：首例已按**推荐值 ② `1000 ms`** 落地（S3b-3 **T8–T10**：块级 `interval_ms` + 调度器读组化；**生效配置** `mupc/deploy/config/mupc_core_config.production.yaml` 的 `hvac_di.interval_ms = 1000`）；`§10.4` 的 `U_后 = 2.68 %` 与 `AC-8-1/8-2/8-3` 的计数**均按该值**。**该取值仍待产品追认**；若改判 ①/③ 则须**同批**改 `§10.4` 的 `U` 与 AC 计数（③ 还须把段级 `poll_ms` 1000 → 500）。<br>**未改**：Q-21 原有的三档取值与推荐理由、**C1–C9 约束内容**、**任何 AC 的判据与数字**、**§10.4 的 `T`/`U` 数字**、§10.8 的两条边界声明、§10.9 的 Q-22/Q-23、**§1–§9 的全部内容（含 §9.4.1 的 6 站参考配置「逐字不变」）**、任何代码/配置（首例落地的那一行在**另一份生效配置**里，见 02 设计 §12.6）。**本次不构成重新评审**；**§9 的 `[REVIEWED: PASS: 2026-09-21]` 与 §10 的 `[REVIEWED: PASS: 2026-09-23]` 标记原文未动**。 |
| **v1.13-r1（2026-09-23，§10.7 AC-8-5 示例订正，判据未动、不构成重新评审）** | **只订正 §10.7 的 AC-8-5 那一格中"C4 的构造法"的示例文字**（来源 = 02 设计 §12 的**第二轮设计评审**阻塞项 S-2）：原写"同组 3 个 FC04 块…**取该组某块** `interval_ms: 1000`" —— 按 **02 设计 §12.4.1 的分组语义**，**声明了 `interval_ms` 的块按 `eff` 自成一组**（另两块继承站周期）⇒ 该块所在组 `T_组 = 267.12 ms`、`1.5×T_组 = 400.7 ≤ 1000` ⇒ **C4 通过**（实际触发的是 **C9**：`U = 0.534 > 0.5`，而 C9 文案不写 `1202`）⇒ 原例**取不到"文案含 `1202`"**、C4 实际无人覆盖。**⇒ 已改为"三个块均声明 `interval_ms: 1000`"**（仍为同一读组 ⇒ `T_组 = 801.36 ms`、`1.5×T_组 = 1202.04 ms` ⇒ `1000 < 1202.04` ⇒ **规则 20 的 C4 分支先于规则 23 返回**、文案含 `1202`），并补注"**该组内三块 `eff` 相同 ⇒ 仍为同一读组**"。<br>**未改**：**AC-8-5 的判据本身**（非法值逐条 `Err`、文案含站 id + 块名 + `1202`）、**C1–C9 的任何约束内容**、**任何 AC 的判据**、**§10.5 第 3 行**（设计侧由 02 设计 §12.7 的**规则 24** 落点）、§10.7 表内其它行、**§1–§9 的全部内容**、任何代码/配置；**§9 的 `[REVIEWED: PASS: 2026-09-21]` 与 §10 的 `[REVIEWED: PASS: 2026-09-23]` 标记原文未动**。**本次不构成重新评审**；**02 设计 §12 同轮按第二轮设计评审意见修订（v1.13-r1，待复审）** |
