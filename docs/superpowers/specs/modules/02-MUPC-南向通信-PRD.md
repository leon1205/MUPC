...
> 文档版本：v1.13-r2（2026-09-24，**仅 §10.9 Q-21 备案**，判据未动、不构成重新评审；**本行与文末版本演进表的 `v1.13-r2` 行同步**）｜状态：**需求评审通过（终审 2026-09-21）**；v1.7 / v1.8 / v1.9 / v1.10 / v1.11 为评审通过后的 **§9 增量补登/订正**，v1.12 为 **§10 新增章节**，v1.13 为 **§10 的 4 项文档级订正 + 4 条建议改进**（设计评审附项落正文），**v1.13-r1 为 §10.7 AC-8-5 的"C4 的构造法"示例订正（判据未动）**，**v1.13-r2 为 §10.9 Q-21 的取值备案**（均不构成重新评审）｜§9 站级南向设备语义点表集成（S3b-2）；**§10 块级采集周期覆盖（告警位单独快采，S3b-3）**
>
