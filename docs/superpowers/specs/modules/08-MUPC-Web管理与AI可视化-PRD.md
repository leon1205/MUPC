...
> **[SUPERSEDED: 2026-09-10]** 本模块内容已并入 **12-MUPC 本地显示终端（PRD v2.0）**；**Web 访问机制移除**；AI 类章节（§6 AI 决策可视化 / §7 专家干预 / §8 A/B 测试与模型回滚 / §8.1 版本管理 / §6.5 在线微调监控）已在 12 号 PRD 中标 **暂停**（AI 引擎停用，待观测空间重构 + AI 重启后启用）。
>
> **本文件不再作为实施依据，仅为历史追溯保留原文，全文未作删改。**
>
> 并入映射（详见 12 号 PRD v2.0 §3.5 与附录 A.3）：
>
> | 本文件章节 | 处置 | 12 号 PRD 归属 |
> |------------|------|----------------|
> | §3 系统状态监控 | 并入实现 | §3.1 F6（装置整体状态）/ F7（告警列表）/ F8（版本信息） |
> | §4 配置管理 | 并入实现 | §3.2 F9（触摸读写与保存生效） |
> | §5 日志管理 | 并入实现（**导出降级为不支持**；关键字检索须评估可行性） | §3.3 F10 |
> | §6 AI 决策可视化 | **暂停**（保留需求设计，本期不实现） | §3.6 |
> | §7 专家干预 | **暂停**（保留需求设计，本期不实现） | §3.6 |
> | §8 A/B 测试管理 / §8.1 模型版本管理 | **暂停**（保留需求设计，本期不实现） | §3.6 |
> | §9 边界条件 / §10 非功能性 / §11 数据持久化 / §12 验收标准 | 通用部分并入 12 号（§4 / §5 / §3.5）；AI 类随暂停 | 12 号 §3.5 / §4 / §5 |
> | §2 用户角色与认证授权 | Web 登录体系**取消**；角色收窄为现场人员 | 12 号 §2 / §2.1 |
>
> 权威来源：`docs/superpowers/specs/modules/12-MUPC-本地显示终端-PRD.md`（v2.0）

**目标 crate：** `web-api`（随 Web 访问机制移除，见 12 号 PRD v2.0 §3.5）
