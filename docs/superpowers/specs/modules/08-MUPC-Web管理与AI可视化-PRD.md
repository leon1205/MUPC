...
- **三级确认 + 密码验证**：模型回滚、切换到兜底模式需要三级确认 + 操作人密码二次身份验证
...
| 模式配置 | TOML | `/etc/mupc/mode.toml` | 重启后保持 |
...
| VIZ-10 | 当前运行模式在面板顶部展示，手动切换带"手动"标签 | 集成测试 |
...
**变更说明：**
- 合并三份文档中与 Web 管理和 AI 可视化相关的需求
- 去重合并配置管理、日志管理、状态监控等重叠内容
- 保留所有 [REVIEWED: PASS] 标记的功能需求
- 按目标模块（web-api crate）重组结构
