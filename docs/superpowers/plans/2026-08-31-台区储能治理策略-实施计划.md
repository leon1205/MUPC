...
- **机理说明**：S3 稳态目标恰为 p_st = P_表 - p_tgt_s3，故该钳位不改变稳态目标跟踪，仅在积分过冲（负荷快速回落、p_st 高于当前负荷裕度）时拦截放电，等效于针对斜坡速率不足（5kW/周期 vs 负荷瞬时跌落）的防过冲 windup 钳位。
...
- KPI 仍为净：`p_ctrl = p_total_base - p_st`（`p_st = Σ last_p_out`）；SOC 积分不变。
...
|---|---|---|---|---|---|---|---|---|---|---|
| OFF | 5.7 | 18.1 | 6.4 | 10.0 | 50.0 | 14.6 | 55.7 | 7.8 | 16.4 | 41.0 |
| ON（默认） | **5.1** | 18.1 | 6.3 | 10.0 | 50.0 | 21.7 | 55.7 | 7.8 | **0.0** | 41.0 |

**7-04（高负荷日，基线 返送 16.5% / 返送峰值 57.0kW / 受电峰值 77.6kW）：**

| s3_margin | 返送时长% | 返送峰值kW | 返送能量kWh | SOC日终% | SOC峰值% | 削峰净进口均值kW | 放电kWh | 充电kWh | SOC地板% | 受电峰值kW |
|---|---|---|---|---|---|---|---|---|---|---|
| OFF | 13.3 | 47.0 | 33.0 | 10.0 | 50.0 | 46.2 | 89.5 | 41.5 | 24.4 | 77.6 |
| ON（默认） | **11.9** | 47.0 | 28.2 | 10.0 | 52.2 | 44.7 | 89.5 | 41.5 | 19.1 | 77.6 |

**结论与决定（规格审视预测的"限幅在 net 下破坏削峰"未发生）：**

- **gross-clamp 在 net 反馈下表现良好，保留原形式**：两日 ON 控制后返送均低于基线（6-27 5.1%<6.2%，7-04 11.9%<16.5%）、低于 OFF（5.1<5.7，11.9<13.3）；SOC 日终均达 10% 地板（日终清空不受影响）；未出现限幅/欠输送限循环。
- **限幅反而避免电池过早放空**：6-27 ON 的 SOC 地板占比 0.0%（OFF 16.4%），放电能量两档相同（55.7 kWh）——OFF 提前放空后只能干坐，ON 把能量保留到后段峰期仍能放电；7-04 地板占比 19.1%（OFF 24.4%）。
- **受电峰值两档完全相同**（6-27 41.0 / 7-04 77.6）：限幅不造成需量峰值惩罚。7-04 受电峰值未削（77.6→77.6）是电池能量（SOC 地板）约束所致，非限幅问题。
- **6-27 ON 削峰净进口均值（21.7）高于 OFF（14.6）的解读**：这是 OFF 在负荷回落阶段**过冲到反向送电**（p_ctrl 为负值拉低均值，对应 OFF 返送 5.7%>ON 5.1%）造成的统计偏差，恰是限幅要拦截的过冲；非限幅导致的削峰欠输送。限幅下最大受电峰值与 OFF 相同、总放电能量相同，仅把"峰值期的过冲放电"转为"削峰+防返送"的合理分配。
- **理论稳态限循环（恒定高峰下 p_st 在 (base-5)/2 附近震荡）在实测数据中不显现**：两日高峰多为短时/被 SOC 地板约束，电池能量而非限幅成为高峰时段实际限制因素。
