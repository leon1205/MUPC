...
│  │ /api/config        │ │ predictions      │ │ [LEGACY] /api/ai/mode │   │
...
- 展示当前预设运行模式（v2.0+：由 ModeSelector 管理，非自动识别）
...
在 AI 决策面板顶部显示当前预设运行模式和切换来源。模式由调度主站远程指令或本地策略管理员选择确定（预设互斥场景，无自动识别）。
...
> 无"本地兜底模式"。AI 引擎异常时系统自动降级至本地策略（由 strategy-engine 内部处理）。无"恢复自动识别"功能（不存在自动识别）。
...
- [ ] 连续自动回滚 3 次后触发安全模式（AI 降级由 strategy-engine 内部处理，v2.0+）
- [ ] 回滚操作记录完整审计日志
- [ ] 权限不足的用户无法看到回滚按钮
- [ ] 回滚不影响正在运行的 A/B 测试
...
#### 6.5.18 DELETE /api/ai/abtest/{id}
...
| 9.1.4 | AI 引擎未启用 | 运行模式状态显示"手动模式"或"N/A"，置信度百分比字段留空 | 无操作可执行 |

**验收标准：**
- [ ] AI 引擎未启用时，预测曲线区域显示占位提示而非空白
- [ ] 决策逻辑面板和实时奖励面板在 AI 未启用时正确隐藏
- [ ] 运行模式状态字段在 AI 未启用时显示"N/A"
...
| 版本 | 主要变更 |
|------|----------|
| v1.0 | 合并三份文档中 Web 管理与 AI 可视化内容 |
| v1.1 | 新增第 9 章"边界条件与异常处理" |
| v1.2 | 同步 v2.0 预设运行场景设计 |
