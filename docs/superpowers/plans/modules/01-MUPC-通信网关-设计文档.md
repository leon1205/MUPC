...
| 点表生成 | §9.2.1 的 `build_uplink_points(&config.south_stations, Some(&config.south_pcs))`，装配期调用一次；**失败 ⇒ 拒启动**（`Err` 冒泡，与 `validate_south_stations`（`core_config.rs:679`）同范式）。**2026-09-26：PCS 迁入南向后签名扩为显式收 PCS 段（02 号设计 §13.9 末），避免 72 个 PCS IOA 静默丢失** |
...
/// **2026-09-26：PCS 迁入南向后签名扩为显式收 PCS 段（02 号设计 §13.9 末），避免 72 个 PCS IOA 静默丢失**。
pub fn build_uplink_points(cfg: &SouthStationsConfig, pcs: Option<&SouthPcsConfig>) -> Result<Vec<UplinkPoint>, String>;
...
| # | 缺陷 | 证据 | 后果 |
|---|------|------|------|
| 1 | `make_i_frame` 的 `i2`/`i4` **恒为 0x00** ⇒ 发送序号只写得进低 7 位 | `gateway/src/iec104/protocol.rs:270,272` | 序号 **≥ 128 即回绕**（**v1.4-r5 订正**：本文原写"≥ 64"，实测断裂点在 **128** —— `i1 = (s<<1)&0xFE` 承载 `s` 的 bit0–6 ⇒ `s ≤ 127` 正确、`128` 起回绕；T12 已在用例两侧钉住）；**234** 点一轮必然回绕 ⇒ 主站判 `send_seq` 错乱 | |
...
> **⚠️ 补充第 6 项缺陷（v1.4-r5 新增；T12 实现必然带出，PM 追认）**：**读侧对偶缺陷** —— `send_sequence` / `recv_sequence` 原写作 `((c1>>1)&0x7F) | ((c2&0x7F)<<7)`，对 `c1=0xFE, c2=0xFF` 算得 **`0x3FFF`**（高字节被 `&0x7F` 截掉一位）⇒ **只修写侧等于白修**：序号 ≥ 16384 起解析全错。修法 = 读侧同步去掉 `& 0x7F`（与写侧 15 位口径对齐）。**判别锚** = `test_i_frame_sequence_roundtrip_ge_128_and_wrap`（把读侧回退为带 `&0x7F` ⇒ 该用例在 `seq=16384` 处红；已注入验证）。**本项原不在缺陷表内，现补为第 6 行备查。**
...
> **⚠️ 三处口径钉死（v1.4-r5；T12 实现 + 评审）**：
> 1. **星期（DOW）必须 ∈ 1..7**（IEC 60870-5-4）—— 本行原只写"日+星期(1)"，**未给公式**，实现曾漏一次取模导致**周一到周三错为 0/1/2**（**且现有用例只取周四 ⇒ 全绿漏检**）。**唯一正确写法**：`weekday = ((days + 3).rem_euclid(7) + 1) as u8`（先取模再 +1，输出恒 1..7；1970-01-01 = 周四 ⇒ 4）。**判别锚** = `test_cp56time2a_iso_weekday_bytes`（周一/周三/周四字节级）+ `test_cp56time2a_iso_weekday_covers_1_to_7_over_seven_days`（连续 7 天恰好覆盖 1..7）。
> 2. **首字段实为"秒×1000 + 毫秒"**（0–59999，LE 两字节），本行原写"ms(2)"易被读成"纯毫秒"⇒ 按标准实现（实现与用例已按此钉住）。
> 3. **年域 `(year-2000) mod 100` 存在世纪歧义**：**设备 RTC 未设（1970）时，主站会读成 2070**（未来时标），与 PRD §8.4「不得重打时标」的意图相悖 ⇒ **登记为遗留**：建议装配层做"时钟有效性门禁"（RTC 未同步时不上送带时标帧或告警），**本增量不改**（设计未授权），T13/后续裁定。
...
| 2 | `startup.rs` | `build_uplink_points(&cfg.south_stations, Some(&cfg.south_pcs))?`（失败拒启动）→ `Arc<Vec<UplinkPoint>>`；落 `uplink_points.json`。**2026-09-26：PCS 迁入南向后签名扩为显式收 PCS 段（02 号设计 §13.9 末），避免 72 个 PCS IOA 静默丢失** |
...
| 版本 | 主要变更 |
|------|----------|
| **v1.4-r6（2026-09-26，PCS 迁入南向的连带订正 —— `build_uplink_points` 签名与两处调用点；未加任何门禁标记）** | **只订正 §9.2.1 的签名块、§9.1.8 装配表、§9.4 序 2 三处**（签名由单参 `&SouthStationsConfig` 扩为**显式收 PCS 段** `(cfg: &SouthStationsConfig, pcs: Option<&SouthPcsConfig>)`；两处调用点补 `Some(&config.south_pcs)`），**不改 §9 任何点数/IOA/掩码/档位/判据，不改帧编码与投递策略**。来源 = **PCS 通信与控制整体迁入南向**（02 号设计 **§13** / **ADR-014**；§13.9 末的 4 条要求 / 落地评审查出的静默缺口 **Δ-19**）：`build_uplink_points` 入参原为单参，而 PCS 迁出后**不再在 `south_stations` 里** ⇒ 若调用点不喂 PCS 段，其 **72 个 IOA 会静默从 IEC104/MQTT 点表消失**，且既有自检不响（`has_pcs == false` 的期望值恰为 567 = 5 站并集 ⇒ fail-open）。**未改**：§1–§8 与 §9 的其余内容、任何代码/PRD/其它模块文档；**既有门禁标记原文与覆盖范围不变**；**本次未加任何门禁标记**。 |
| **v1.4-r5（2026-09-24，T12 实现暴露的 §9.2.3/§9.2.4 勘误与 6 项登记）** | **只订正行文/补判据/登记，不改任何帧格式、序号编码、TypeID 值或投递策略**。来源 = **T12（`f41e6f7` + 修复 `9aa7097`）规格/质量评审**（规格 **SPEC_COMPLIANT**；质量 **APPROVED_WITH_CONCERNS**；3 次注入探针）。<br>**① 勘误（§9.2.3 缺陷 1 的后果描述）**：原写"序号 **≥ 64** 即回绕"，**实测断裂点是 128**（`i1=(s<<1)&0xFE` 承载 `s` 的 bit0–6 ⇒ `s ≤ 127` 正确）。<br>**② 补为第 6 项缺陷（读侧对偶）**：`send_sequence`/`recv_sequence` 的 `&0x7F` 会把 `0x7FFF` 读回 `0x3FFF`（序号 ≥ 16384 全错）⇒ **只修写侧等于白修**；判别锚 `test_i_frame_sequence_roundtrip_ge_128_and_wrap`。<br>**③ 勘误（§9.2.4 CP56Time2a）**：该行原只写"日+星期(1)"**未给公式** ⇒ 实现曾漏一次取模，**周一/周二/周三错为 0/1/2**（**现有用例只取周四 ⇒ 全绿漏检**；探针实证）。现**钉死 DOW ∈ 1..7 与唯一正确公式** + 判别锚两例。<br>**④ 登记（§9.2.4）**：首字段实为"**秒×1000 + 毫秒**"（0–59999），原写 `ms(2)` 易误读。<br>**⑤ 登记（遗留，§9.2.4）**：年域 `(year-2000) mod 100` 的**世纪歧义** —— RTC 未设（1970）时主站读成 **2070**（未来时标），与 PRD §8.4「不得重打时标」相悖；建议装配层加"时钟有效性门禁"，**本增量不改**。<br>**⑥ 登记（§9.0/§9.2.2）**：**`DataClass` 在 gateway 侧镜像定义**（与 `southd::uplink::DataClass` 同名不同物）—— §9.0 禁新增 `gateway → mupc-southd` 依赖边 ⇒ 镜像定义是**唯一自洽解**；**档位分配表仍唯一在 southd**，T13 逐值映射。<br>**⑦ 登记（§9.2.2 取舍）**：C 档 `send().await` **无超时/取消路径**（设计明写"阻塞等待"⇒ 合规）—— 单点故障（某主站写端卡死）可拖住上送方，**无上界**；已登记，本增量不加超时。<br>**⑧ 登记（§9.2.5）**：`recv_seq` 更新为对端 `N(S)` **未 +1** ⇒ 出向 I 帧的 `N(R)` 是"最后已收序号"而非"期望的下一个"。旧实现 `i4` 恒 0 使该字段本就无意义 ⇒ **非本批回归**，但**须与真实主站对点后定**（本轮不动）。<br>**⑨ 登记（§9.2.3 过渡态）**：`broadcast_telemetry` 的 `#[deprecated]` **过渡壳取 C 档（不丢）**，以保持改造前"必达"语义；**T13 迁移调用方后本壳删除**。（原实现取 A 档 ⇒ 过渡窗口内总表 6 点可能丢帧，已改。）<br>**⑩ 登记（§9.2.2）**：`PublishOutcome` 收敛为 `Delivered{subscribers}` / `Dropped{subscribers,dropped}` / `NoSubscriber` 三态（设计只点名 `NoSubscriber`）；**空批次同样走订阅者判定**（不得短路绕开 `NoSubscriber`）。<br>**未改**：§9.2.3 的**修正设计代码块**（`make_i_frame`/`make_s_frame`/`OutboundSeq`/`publish_asdus` 签名）、§9.2.4 的 TypeID 值表、§9.2.1.x 的点数/IOA/掩码、§9.2.6、§9.1/§9.3/§9.4 与 §1–§8、任何代码、PRD、12 号设计；**既有门禁标记**（`[DESIGN_APPROVED: 2026-09-23, 设计评审员]` 原文未动，覆盖范围仍为 §9 v1.4 增量）。**本增量未经设计复审**（勘误 + 登记类，不构成新契约）。 |
