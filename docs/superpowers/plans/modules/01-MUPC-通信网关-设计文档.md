...
> **版本：** v1.4（2026-09-23）**（按设计评审意见修订；设计评审复审通过，2026-09-23）**

> **文档定位：** 本文档记录实现级设计决策。需求级内容（功能描述、验收标准、性能指标）请参考 [01-MUPC-通信网关-PRD](../specs/modules/01-MUPC-通信网关-PRD.md)。
>
> **v1.3 增量（2026-09-23）**：新增 **§9 外设数据上云（U-74 / U-71）设计增量**（落 01 PRD §8 `[REVIEWED: PASS: 2026-09-23]`）——① §9.1 **最新值入口**（01/03/12 三份共用件，归属 `mupc-data-processing`）；② §9.2 IEC 104 **231 点**机械生成点表 + 总召（`C_IC_NA_1`）+ 连接初始快照 + **帧序号/编码修正**；③ §9.3 **MQTT 真做**（配置承载 / 生产者接线 / 分片主题 / 离线补送 / TLS fail-closed）。§5.5 的北向主题表由 §9.3.4 的新表取代（原行保留作历史）。本章**不改** PRD、**不改** 12 号设计。
>
...
| `StationSink::on_station_telemetry(id, role, pts)`（`startup.rs:535`） | ① **先** `mark_station_polled(id, now_ms)`（**本站在采**，与点数无关；`online` 合成事件的那次调用同样刷新）；② `is_event == false` 的每一项 → `apply`（**含位点**，LV-5 要求位块点同样有入口）；③ `is_event == true` 的项**不写**快照（它们是站级/信号级事件，归 `events` 表，不属"点位最新值"）；④ **`role == Battery` ⇒ 本批 `apply` 之后立即求值 12 组聚合并 `apply`**（§9.2.1.1，写入侧求值点） |
...
| 读者 | `get`/`station_snapshot`/`all` 持读锁克隆所需子集后即释放 | 单次全量 636 点克隆 ≈ 数十微秒；**聚合求值走一次 `station_snapshot("bms")`**（289 点），不得逐点取锁 |
| 内存上界（常驻） | **636 点**（= MQTT 全量 624 ∪ IEC104 聚合 12，PCS 启用；未启用 **564 点**，§9.2.1.0）× `PointId`(≈48 B 堆 + 24 B 结构) + `PointValue`(8+8+1 + 对齐) ≈ **≈ 75–95 KB**；`HashMap` 负载因子开销后上界 **≤ 128 KB** | 与 02 PRD §9.8.3 的"南向增量 < 100 KB"**同量级**；设计值按 128 KB 声明（含预留容量）。**接受**：636 点在装置内存（RK3588，GB 级）上可忽略 |
...
/// （IEC104 = 调度子集 231；MQTT = 全量 624；两者并集 = 636，含 IEC104 独有的 12 聚合）。
...
/// 生成**并集**上送条目（含 12 个 IEC104 独有聚合）。**失败即拒启动**（配置/点表漂移的 fail-fast 点）。
pub fn build_uplink_points(cfg: &SouthStationsConfig) -> Result<Vec<UplinkPoint>, String>;

/// **BMS 12 组聚合的运行期求值**（纯函数，无 IO / 无锁）：入参 = 位查询闭包，
...
|------|------|--------------------------|------|
| **IEC 104** | 段 1–7（§8.3.2 分段）| **159 / 231** | 不含 288 个 BMS 告警位与 114 个消防探测器明细（BMS 以 12 聚合替代，PRD §8.3.1） |
| **MQTT（北向）** | 全量 618 + 总表 6 | **552 / 624** | 不含 12 个 IEC104 聚合（聚合是**调度侧派生量**，物联平台订阅 288 个原始位） |
| **并集**（快照与点表 JSON 的规模） | IEC104 ∪ MQTT | **564 / 636** | 快照内存上界按此计（§9.1.6） |

> **该表的必要性**：原稿写"MqttUplinkPublisher 持与 IEC104 **同一份** `Vec<UplinkPoint>`"，但两者点集**不同**（231 ≠ 624）。若不区分，任一侧必然点数错。故 `UplinkPoint` 带 `channels`，`build_uplink_points` 产**并集**，两个上送器各按 `channels.has(..)` 过滤；`build_uplink_points` 的**点数断言**即上表三行（生成期自检项之一）。
...
for 站 in cfg.stations（按 cfg 顺序）:
    ...
    IEC104 段内序号 = 1..n（标量在前、位点在后；段 4 的 12 聚合即段内序 1..12）
...
| `battery`（遥信段） | 300 | **聚合**（`BMS_ALARM_GROUPS`） | 12 | 见 §9.2.1.3 的 **G-1～G-4 四条**（引用位存在 / 组间互斥 / **扣减排除表后覆盖 Alarm ∪ State** / 排除表逐位类别与 `point_table` 一致） |
...
| 全局 | — | 并集 | 564 / 636 | **三通道点数断言**（§9.2.1.0 表：IEC104 159/231、MQTT 552/624、并集 564/636） |
...
#### 9.2.1.1 12 组聚合的**运行期求值点**（P1：原稿缺此定义 ⇒ 231 点不可得）

**问题**：原稿只写"写入侧写单点（288 个 `bms_alarm_*`）、读侧只查值"，**没有任何一处把 288 位求值成 12 个聚合点** ⇒ 段 4 的 12 个 IOA 在快照里永远不存在。
...
对每组 g:
  ...
  否则 若 全部可读位 q == Ok        → (metric, Some(OR(位)) , Ok)
  否则（存在 q != Ok 的可读位）      → (metric, Some(OR(位)) ,
                                          worst(q of 该组可读位))   // Ok < Stale < Invalid < Unconfigured
```
...
> **覆盖合计 133 位**（36+40+9+2+5+41 = 133，其中 Alarm 128 + State 5）。指数区间逐位与 `point_table.rs:387-675` 的 `BitClass` 一致。

#### 9.2.1.3 生成期自检（P2：原稿"覆盖 Alarm ∪ State"与排除表矛盾 ⇒ 按字面实现会拒启动）

**原稿缺陷**：自检写"并集必须覆盖 `Alarm ∪ State` **全部位**"，但排除表本身剔除了 88 位（原稿只列出 28 位，且把 364–423/484–487 与 **424–483** 混为一行"Reserved"，而**代码里 424–483 是 `Alarm`**）⇒ 自检与排除表**互相矛盾**：按字面实现，`build_uplink_points` 必 `Err` ⇒ **拒启动**。
...
pub const BMS_AGGR_EXCLUDED: &[(u16, BitClass, &str)] = &[ /* 88 行，见下表 */ ];
```
...
| **G-2 组间互斥** | 12 组的位集合两两不交（`∪组` 的基数 == 各组基数之和 == **133**） |
| **G-3 扣减后全覆盖** | `(Alarm ∪ State) − BMS_AGGR_EXCLUDED == ∪组`（**集合相等**，不是"覆盖"）—— 等价计数式 **`card(A∪S) == 133 + card(EXCLUDED) == 133 + 88 == 221`** |
| **G-4 排除表一致** | ① `BMS_AGGR_EXCLUDED` 与 `∪组` 不交；② 每行的 `期望 BitClass` == `point_table` 实际类别；③ 排除表**逐位展开**后的基数 == 88（防"用一段区间糊过去"） |
| **G-5 通道点数** | §9.2.1.0 的三行断言（IEC104 159/231、MQTT 552/624、并集 564/636） |

**逐位排除表（88 行按类别归并，但**代码里必须逐位展开**——区间只是文档的简写）**：
...
| 290 | 1 | 簇放电 | **State** | 正常态 |
| 291 | 1 | 簇就绪 | **State** | 正常态 |
| 298 | 1 | 簇高压箱状态 | **State** | **待产品确认**（若需上主站：段 4 扩至 313，改表不改码） |
| 340–363 | 24 | 单体充/放电过温欠温 ×3、温升过大 ×3、极柱温度过/欠温 ×6、电压变化过大 ×3、…（各轻/中/重） | **Alarm** | **待产品确认（Q-A）**；默认不入：与 304（单体温度）/303（单体电压）语义部分重叠；**若要"不丢语义"⇒ 加第 13 组 `bms_aggr_cell_temp_charge_discharge`，段 4 = 301–313** |
| 424–483 | 60 | 簇一/二/三级告警、端子温度过低 ×3、MOS 过温/欠温 ×6、SOE 低 ×3、正/负极柱温度 ×12、绝缘检测低 ×3、继电器粘连 ×8、AFE 故障、各类从控/主控故障… | **Alarm**（⚠️ **原稿误标为 Reserved**） | **待产品确认（Q-B，本设计新增）**；默认不入的理由：主站侧为**安全总貌量**，逐项细分无调度用途。**但本组含"簇一/二/三级告警""AFE 故障""继电器粘连"等关键安全位** ⇒ 建议产品优先裁定（见 §9.8 Q-B 的方案 (b)：加 `bms_aggr_cluster_level` / `bms_aggr_relay_fault`，段 4 扩至 314） |

> **Reserved 位不参与 G-1～G-4 任何判据**：`point_table` 中 67 位 Reserved（200、292、297、364–423、484–487）既不在 `∪组`、也不在 `EXCLUDED`，**不影响自检**（这正是"扣减后全覆盖"比"覆盖"更准确的原因）。
> **与 PRD 的偏差登记**：PRD §8.3.2 写"建议 12 个"但括号内枚举到 21 项（含"运行态（充/放/就绪/禁充放/故障）"这类多值项）。本设计**取 12 组**（忠于 PRD 的"12 点 / IOA 301–312"契约），组内构成见 §9.2.1.2，逐位覆盖/排除**逐位可验证**（G-1～G-4）。见 §9.8 C-4 / Q-A / Q-B。
...
> **总点数**：6+40+57+12+13+31 = **159**（PCS 未启用）；+72 = **231**（PCS 启用）。
...
| **C** | 全部**位点 + 状态枚举 + 消防系统态**：`bms_aggr_*`×12 + `hvac_di_1..31`（43）+ fire 系统态 13 点（PRD §8.3.2 定其"变化上送"） | **56** | **56** | COS（值变化后 ≤ 1 s 内发出） |
| **合计** | 与 §9.2.1.0 的 IEC104 行一致 | **19+84+56 = 159** ✓ | **22+153+56 = 231** ✓ | — |
...
| 队列容量 | `telemetry_txs` 的 `mpsc::channel::<Vec<u8>>(100)`（`server.rs:132`）→ **2048**。依据：**最坏一阵同刻到齐** = 总召/初始快照的 231 帧（231 × 25 B ≈ 5.6 KB）+ 5 连接的并发写 ⇒ 2048 帧 ≈ **8 轮**余量（单点单 ASDU 形态下"帧数"即"条数"）。**容量在连接建立处一处修改**（`server.rs:132`） |
...
| **总召 / 初始快照突发** | 159 × 25 B | **3,975 B ≈ 3.9 KB** | 231 × 25 = **5,775 B ≈ 5.6 KB** |

> **与 PRD 上界的对照（登记 §9.8 C-18）**：① 稳态 **7.3 / 10.7 kbps < 12 kbps** ✓；② **PCS 启用 + C 档高峰 ≈ 12.3 kbps**，**略超** PRD §8.7 的 12 kbps 参考上界 —— 但该值是**按 18 B/帧**估的"上界量级"（PRD §8.7 明载"不得作为精确值引用"），本设计按真实帧长 25 B 给出精确值后必然更高。**处置**：稳态仍在上界内；若主站实测受限，按 PRD §8.7 允许的 **SQ=1 顺序 ASDU** 压缩（A 档 19 点可并为 1 帧 ≈ 6+1+1+1+2+ 19×(3+4+7) = 277 B，A 档降到 ≈ 277 B/s）——**本期仍取单点单 ASDU**（与既有实现同形态，KISS，且与 §9.2.3 的帧头修正解耦）。
> ③ 突发 **5.6 KB** 亦高于 PRD 估的 4.2 KB（后者按 18 B/帧且**不含 PCS**）⇒ 一并登记，请求 PRD 回填。

> **MQTT 侧带宽/资源**：见 §9.3.7（原稿只给了 IEC104 侧，评审已指出）。

#### 9.2.3 帧编码修正（**本增量的硬前提**）

**现状缺陷（必须同批修正，否则 231 点上送会被主站判为序号错误）**：

| # | 缺陷 | 证据 | 后果 |
|---|------|------|------|
| 1 | `make_i_frame` 的 `i2`/`i4` **恒为 0x00** ⇒ 发送序号只写得进低 7 位 | `gateway/src/iec104/protocol.rs:270,272` | 序号 **≥ 64 即回绕**；231 点一轮必然回绕 ⇒ 主站判 `send_seq` 错乱 |
...
- **新增外设点**（159/231 点）一律用 **TI=36 / TI=30 带时标**（满足 §8.4/§8.7 的"不得用响应时刻重新打时标"）；
...
| 约束 | 设计 |
|------|------|
| 响应时延（PRD 4.x：≤ 5 s，231 点） | **231 点 × 25 B ≈ 5.6 KB** 直写；实测应为**毫秒级**。设计声明 **≤ 1 s**（远优于 5 s 要求），并在集成测试断言 |
...
pub struct MqttUplinkPublisher {
    ...
    /// ⇒ 恰好 624 点（PCS 未启用 552），**不含 12 个 IEC104 独有的 BMS 聚合**
...
| B 档 | 4 条/5 秒 = 1,711+2,686+5,922+249 | **10,568 B / 5 s = 2,114 B/s** | 14,037/5 = 2,807 B/s |
| C 档（稳态） | 1 变位/s（站内合并窗 200 ms ⇒ ≤ 1 条/s，≈ 150 B/条） | **≈ 150 B/s** | ≈ 150 B/s |
| **稳态合计** | | **≈ 3.5 KB/s ≈ 28 kbps** | **≈ 4.4 KB/s ≈ 35 kbps** |
| C 档高峰（10 变位/s） | 5 窗/s × (110 + 2×50) ≈ 210 B | +1,050 B/s ⇒ **≈ 4.4 KB/s ≈ 35 kbps** | ⇒ **≈ 5.4 KB/s**（**略超** PRD 参考上界，见下） |
| **首轮全量快照（一次性）** | 624 点 × ≈50 B + 6 条消息头 | **≈ 29.1 KB** | **≈ 33.0 KB** |
| 其中 C 档位块首轮（bms+hvac+fire） | 333 点，3 条消息（**站间不合并**） | **≈ 17.6 KB** | 同 |

> **与 PRD §8.7 的对照（登记 §9.8 C-18）**：① **稳态 28 / 35 kbps < 40 kbps** ✓（PRD 上界 5 KB/s）；② **首轮快照 ≈ 29–33 KB**，**高于** PRD 估的 20 KB（PRD 按"点名 18 B + 值/单位/质量 25 B = 43 B/点"估，实际点名更长：如 `mb_e_act_comb_1`）⇒ 请求 PRD 回填；③ **PCS 启用 + C 档 10 变位/s 高峰 ≈ 5.4 KB/s 略超 5 KB/s** ⇒ 处置：把 `cos_merge_ms` 上限从 1000 提到 **2000**（合并更粗，高峰降为 ≈ 3 窗/s ⇒ ≈ 5.0 KB/s）**或** A 档在高峰窗内退避（PRD §8.7 的"≤ 5 KB/s"为含 TLS 的上界量级）。**本期默认取前者**（合并窗 2000 ms，仍满足 PRD "变化后 ≤ 1 s 内发出"？—— **不满足** ⇒ 见下"待确认"）。

> ⚠️ **由此暴露的 PRD 内部张力（须产品裁定，§9.8 Q-C）**：PRD §8.6.3 要求 C 档"变化后 **≤ 1 s** 内发出"，而 PRD §8.7 又要求 MQTT 稳态 ≤ 5 KB/s（≈40 kbps）。二者在"COS 高峰 10 变位/s + PCS 启用"同时成立时**不可兼得**（合并窗必须 ≥ 1 s 才压得住带宽，而 ≤ 1 s 时延要求合并窗 ≤ 1 s）。**本设计的默认**：`cos_merge_ms = 200`（**保时延**，取 §8.6.3 的硬要求），带宽高峰按 35 kbps 声明（**由 PRD §8.7 自行订正上界或接受**）。

**资源增量（PRD §8.7 要求"CPU/内存增量须由设计给出"）**：

| 项 | 设计值 | 依据 |
|----|--------|------|
| 快照内存（共用件） | **≤ 128 KB**（636 点） | §9.1.6 |
| 点表常驻（`Vec<UplinkPoint>` 636 条） | **≈ 90 KB**（条 ≈ 140 B：2×`String` + 3 字段 + 对齐） | 生成期一次，不可变 |
| `uplink_points.json`（落盘） | **≈ 120 KB**（636 条 × ~190 B JSON） | §9.2.1 启动期产物，供主站点表对点（RC-U74-02） |
...
> **离线缓存上界的算法（可复算）**：`cache.max_age_s = 1800` 与 `cache.max_messages = 10000` **先到先淘汰**。按上表，**生产节奏 = A 3 条/s + B 0.8 条/s ≈ 3.8 条/s、≈ 3.4 KB/s** ⇒ 30 min 内累计 **≈ 6,800 条 / ≈ 6.1 MB**（PCS 启用 ≈ 7.9 MB）—— **时间窗先到**（条数远未达 10,000）。故 **设计值声明：典型 6–8 MB，上界 ≤ 10 MB**（若站点数/档位数翻倍使条数先生效，则按 10,000 × ≈1 KB ≈ 10 MB 封顶）。
...
|----|-----------------------------------|
| `data-processing` 单测 | 快照并发（多写多读）、`apply` 去重不广播、**`apply` 值不变仍刷新 `ts_ms`**（§9.1.5）、`mark_station_offline` 保原值 + 清站活性、`is_fresh` 边界（标量 =5 s 真/假各一侧；**位点 = 站级活性真/假各一侧**）、`Lagged` 后全量重读、内存上界（636 点构造 + `HashMap` 容量断言）→ AC-U74-12/13 |
| `mupc-southd` 单测 | `build_uplink_points`：**三通道点数断言**（IEC104 159/231、MQTT 552/624、并集 564/636）、**段基址**、**段内序**（块 addr 升序 + 位块在后）、**子集规则**（fire 排 `fire_det`、hvac 排 `hvac_in`、battery 位点不进 IEC104）、**聚合自检 G-1～G-5**（引用位存在 / 组间互斥 / 扣减排除表后全覆盖 / 排除表类别与 `point_table` 一致 / `EXCLUDED` 基数 == 88）、**`evaluate_bms_aggregates`**（OR 语义 / 组内部分缺位 ⇒ 质量降级且不写 0 / 全缺 ⇒ `None`）、配置变更 ⇒ 点数/IOA 变化（防手写常量回归）→ AC-U74-01 |
...
| 本机 IEC104 客户端 | IOA 逐点对点（159 点）、总召、初始快照（**含连接前的值 + 装置长跑后稳态位点仍在**，§9.1.3 的位点活性回归）、站离线不发、PCS 段缺席、带宽计数（稳态 ≤ 7.3/10.7 kbps、突发 ≤ 5.6 KB）→ AC-U74-01/10/11/14 |
...
| **C-2** | PRD §8.3.1 合计行「231（**含 PCS 则 303**）」算术不自洽：231 已含 PCS 72；不含 PCS 应为 **159** | 按用户裁定与 AC-U74-01 的「PCS 未启用时不含 72」落设计：**名义 231 / 未启用 159**。**PRD 该括注须订正为"不含 PCS 则 159"**（登记 §9.8 C-2，不自行改 PRD） |
...
| **C-4** | PRD §8.3.2 称 BMS 聚合"建议 **12** 个"但枚举出 **21** 项；且未列出位 340–363（单体充放电过温欠温等 24 位） | **取 12 组**（IOA 301–312 契约不变），组内构成与**逐位排除**见 §9.2.1；340–363 默认不入（与 303/304 语义重叠），**待产品确认**（并入则扩为 13 点）⇒ §9.8 Q3 |
...
| **C-16**（v1.4 新增） | 原稿写 `MqttUplinkPublisher` 持"**与 IEC104 同一份** `Vec<UplinkPoint>`"，但两条通道点集**不同**（IEC104 231 含 12 聚合；MQTT 624 不含） ⇒ 按字面实现必有一侧点数错 | `UplinkPoint` 新增 **`channels: ChannelMask`**；`build_uplink_points` 产**并集 636/564**，两侧各按掩码过滤（§9.2.1.0）。**12 个 BMS 聚合 = IEC104-only**（物联平台订阅 288 个原始位）；**288 位 + 114 探测器 + 3 温湿度 = MQTT-only**（不上主站） |
...
| Q3 | BMS 聚合组数（C-4） | (a) 12 组（IOA 301–312） (b) 13 组（并入位 340–363） | **(a)**；**Q-A/Q-B 为其细分项** |
...
| **Q-A**（v1.4 新增） | **24 位单体温度/电压细分 Alarm**（位 340–363：充/放电过温欠温 ×3、温升过大 ×3、极柱温度过/欠温 ×6、电压变化过大 ×3…）是否入主站聚合（C-4 的细分） | (a) 不入（与 304/303 语义重叠，主站要总貌） (b) 入 —— 加第 13 组 `bms_aggr_cell_temp_charge_discharge`，**段 4 扩为 301–313** | **(a)**；改表不改码 |
| **Q-B**（v1.4 新增，**高优先**） | **60 位 Alarm（位 424–483）是否入主站聚合** —— 含**簇一/二/三级告警、AFE 故障、继电器粘连 ×8、绝缘检测低 ×3、从控/主控故障、SOE 低、MOS 过温、正/负极柱温度…**（⚠️ 原稿把它们误标为 Reserved，故从未进入任何讨论） | (a) 不入（主站要总貌量；逐项无调度用途） (b) 入 —— 加 `bms_aggr_cluster_level`（424–426）+ `bms_aggr_system_fault`（454–483 内的故障类），**段 4 扩为 301–314** | **(b) 倾向**（**安全相关漏项**：主站目前拿不到"簇级告警/AFE 故障/继电器粘连"）；**但点数契约（12 点）需产品一并裁定**；默认按 (a) 落码（`BMS_AGGR_EXCLUDED` 已逐位登记），产品若选 (b) 只需改表 |
| **Q-C**（v1.4 新增） | **MQTT C 档时延 vs 带宽**：PRD §8.6.3「变化后 ≤ 1 s 发出」与 §8.7「稳态 ≤ 5 KB/s」在"PCS 启用 + COS 高峰 10 变位/s"下**不可兼得**（§9.3.7 C-18） | (a) 保时延（`cos_merge_ms = 200`，高峰 ≈ 5.4 KB/s，请 PRD 订正上界） (b) 保带宽（合并窗 ≥ 1000 ms，违反 ≤1 s 时延） (c) 降 COS 点位范围（只发 Alarm 类） | **(a)**；请产品与物联平台确认"高峰瞬时 5.4 KB/s 是否可接受" |
...
| C-2 | 订正 PRD §8.3.1 合计行括注（"含 PCS 则 303" → "不含 PCS 则 159"） | 纯文档订正 | 请项目经理落入 PRD 修订 |
...
| **C-18（文档订正）** | PRD §8.7 两行带宽/快照量级按 18 B/帧估，与设计精确值不符 | 以本设计 §9.2.2 / §9.3.7 的精确值回填该两行 | 请项目经理落入 PRD 修订 |
