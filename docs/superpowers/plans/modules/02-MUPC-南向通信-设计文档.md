...
>
> **[§11 勘误 v1.9：RC-11 撤销（消防/空调直连 EMS，非并接 BMS 485-2），2026-09-22]**
> **勘误范围（现场核对结论的设计侧同步登记，不改变已批准的设计语义；`[DESIGN_APPROVED: 2026-09-21]` 与 `[CODE_REVIEWED: PASS: 2026-09-22]` 标记保留不动、不重开评审）**：
> ① **§11.11.3 的 RC-11 撤销（本条的实质）**：现场核对结论 = **消防、空调各自直连 EMS（BECG-3568），未挂接储能侧 BMS 的 485 总线** ⇒ **双 master 共总线风险不成立，`fire`（`ttyS6`）/`hvac`（`ttyS3`）两站按原计划启用**。**依据** = `hw/微信图片_20260908170935_64_1061.png`（BECG-3568 接线拓扑图，即 EMS 的硬件载体）：`RS485-1 → PCS`（另有 CAN 直连）、`RS485-2 → BMS`、`RS485-3 → 空调`、`RS485-4 → 关口表`、`RS485-5 → 储能表`、`RS485-6 → 消防`。**原疑问的来源（如实登记，防后人重犯）** = **BMS 对 PCS 的 CAN 协议图 2-1-1「总控模块连接PCS设备」**（`hw/通信协议/_extracted/proto_BMS 对 PCS__…CAN2.0通信协议A2版.txt`，该图对象清单含「消防主机」「空调」）—— 那是**储能厂家（华塑）以 SCU 总控为中心画的"自有系统视图"**，**不是本项目的接线拓扑**，据此推断"消防/空调在 MUPC 侧并挂 BMS 总线"属**误读**；BMS 对 EMS 协议的离散输入位 **482「485-2通讯失联故障」**（`proto_BMS对 EMS__…LAN通信协议B0版.txt`；位 480/481/482 = CAN电流采集故障 / 485-1通讯失联 / 485-2通讯失联）**只说明 BMS 自身有第二个 485 口，不能推出消防/空调挂在其上**。**原条目内容一律保留**（见本表 RC-11 行）—— 让读者看到"曾有此核对项、后如何结清"。
> ② **同步范围（本文档内不得一处写"已撤销"、另一处仍当启用门槛）**：**§11.11.3** 的 RC-11 行（加"已撤销"与结清依据，**原文保留**）、同节 **RC-9…RC-12 来源注**（有效续号收窄为 **RC-9 / RC-10 / RC-12**）、**§11.12.2 Δ-6**（上报需求侧回写 §9.9.2 的清单同步收窄）、**§11.3**（补"设备-端口映射的硬件依据"一句，指向拓扑图与 BECG-3568 规格书）。
> ③ **RC-8 不受牵连，仍然成立（刻意保留）**：RC-11 与 RC-8 **同构但不同事** —— 二者共享的判断是"**跨段互斥校验只覆盖 `intercore` 串口、覆盖不到其它情形 ⇒ 这类情形只能现场核对**"这一**校验能力边界**（该边界**未**因本轮结论改变），而 RC-11 只是**该边界内一个具体的待核对情形**（现已由现场核对排除）。**RC-8（`pcs` 只读站 `ttyS7` 与 intercore `ttyS0` 是否物理独立，PRD §9.10 Q-15）仍未结清**，其"若同总线 ⇒ `pcs` 站不启用（保持注释）"的门槛**一字不动**（§11.7.4 的 `pcs` 行、§11.11.3 的 RC-8 行）。
> ④ **另登记（原始证据的两处瑕疵）**：该拓扑图**原图**末尾两个串口框**误标为 `RS485-4`**（与关口表重复），已在 **`hw/微信图片_20260908170935_64_1061_修正.png`** 订正为 `RS485-5` / `RS485-6`；**原件保留未动**（作为原始证据；此后引用以**订正件**为准）。
> ⑤ **不改动**：§11 头部 `[CODE_REVIEWED: PASS: 2026-09-22]` 段中"遗留项 … RC-11 …"系**评审时点的记录**，**已被本条勘误取代**（标记与记录均**保留不改**，实际口径以本条为准）；§11.5.1 十七条落点、§11.5.2 四判据语义、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分、**§10 全章**、**PRD**（Q-19 的关闭由**需求侧另行同步**，本设计**不擅自改 PRD**）。演进记录见附录 v1.9。
>
> **[§11 勘误 v1.10：W1 fmt 判据收敛 + W2 AC-5 等价落点订正，2026-09-22]**
> **勘误范围（处置 `[CODE_REVIEWED: PASS: 2026-09-22]` 的两项警告 W1/W2 —— 均为文档口径类，**不改任何代码/配置/PRD**；`[DESIGN_APPROVED: 2026-09-21]` 与 `[CODE_REVIEWED: PASS: 2026-09-22]` 两标记保留不动、不重开评审）**：
> ① **W1 裁定（§11.11.2.1 —— 取消判据 ② 的独立判据地位，统一收敛为判据 ① 的行级口径 + 显式基线清单）**。**矛盾复述**：判据 ②（文件级："本 Task **新建**的 `.rs` 文件必须 **0 处**报出"）与同节条目 3（把 T3 新建的 `tests/s3b2_config.rs` 的不符行按判据 ③ 基线处理）对同一族文件给出**相反口径**；实测钉证 = T3（`f83a838`）新建的 `mupc-southd/src/points.rs` 有 **16 条不符行 / 8 hunk**（`rustfmt --edition 2021 --color never --check` 于 v1.10 勘误时复跑，2026-09-22）。**裁定**：取评审建议 ②（"T1–T3 全部不符行统一按判据 ③ 基线处理"）并加一层收敛 —— **判据 ② 不再是独立判据，而是判据 ① 在"纯新增文件"上的推论**。**理由**：(a) 纯新增文件的每一行都是新增行 ⇒ 判据 ① 的"不符行 ∩ 新增行 = ∅"在数学上**退化为**"该文件 0 处报出"，判据 ② 从来不是独立命题；(b) 真正的缺陷是**时间基准点**未写明 —— 判据 ② 隐含"在文件被新建的那个 Task 时点评定"，而 fmt 行级口径**迟至 v1.6（T4 期）才订立**，T1–T3 新建文件在创建时点**无判据可跑**；若允许判据 ② 追溯适用于 T1–T3，等于"判据追溯适用于自身订立之前"，不可机械执行。**收敛后的可机械执行口径**：判据 ① 为**唯一判据**（对每个 Task：不符行集合 ∩ 该 Task diff 新增行集合 = ∅）；"新建文件零报出"仅作为判据 ① 的推论对 **v1.6 口径订立后（T4 起）新建的文件**适用，时间基准点 = 新建该文件的 Task 的 diff 起点；**T1–T3 期间新建/修改文件的全部不符行（合计 66 条，逐行 blame 归因：T3 66 条、T4–T7 自身零条；T4 引入的 1 条 `s3b2_config.rs:377` 已由 T7 修净）显式登记为「S3b-2 格式债基线清单」**（落 §11.11.2.1 新增段，含逐文件数量），此后一律按判据 ③ 既存基线处理、不计入任何 Task 的判据 ①。**另立格式债待办**：该 66 条与仓库既有基线（1204 处 / 106 文件，v1.6 实测）同属历史债 ⇒ **单独立项登记至 `docs/technical-debt.md`、独立 PR 清理，不与本特性混提**（登记动作归项目经理执行；§11.11.2.1 的基线清单为该 66 条的**权威源**）。**不受影响项**：§11.11.2.1 条目 1 的"三个新增文件零报出"（`point_table.rs` / `tests/point_table_vs_reference_config.rs` / `tests/s3b2_decode_e2e.rs`）于 v1.10 勘误时复跑仍 **0 条**，结论不变；§11.13 T7 闸门 ② 对该三文件的"复跑仍须 0 处"照旧。
> ② **W2 裁定（§11.11.2 AC-5 行 —— 括号注订正为"等价落点"表述，不补 core-bin 断言）**。原括号注"（core-bin 侧单测断言 `last_data_ts` 不被刷新）"**无直接落点**（core-bin 侧无该断言）。**裁定**：取评审建议 ②（订正为等价落点），**不取**建议 ①（补断言）。**理由**：等价保证是**结构性**的且已被双重钉住 —— (a) `last_data_ts` 全仓**唯一写入路径** = `AiIntegrator::set_latest_data`（`ai_integration.rs:186` 定义、`:212` 写入），其生产代码**唯一调用点** = `SouthSink::on_grid_package`（`startup.rs:378`）；`on_station_telemetry`（`startup.rs:385`）与 `on_battery_soc`（`startup.rs:445`）均**不触碰**它；(b) scheduler 侧单测 `pcs_station_never_triggers_grid_or_soc_channels`（`scheduler.rs:2542`）/ `non_battery_role_never_triggers_soc_channel`（`scheduler.rs:1543`）断言 `on_grid_package` **不被非 grid 站调用**。两条合取 ⇒ "meter_grid 之外不推进闸门"是**写入路径唯一性 + 调用侧断言**的机械推论；再补一条 core-bin 时间戳比对断言只是对同一事实的重复校验（且需在 bin crate 引入 mock 时钟接缝，成本收益不匹配），**不立项、不登记补断言待办**。落点 = §11.11.2 AC-5 行括号注的订正（本次已改）。
> ③ **不改动**：§11.4 数据结构、§11.5.1 十七条落点、§11.5.2 四判据语义、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分的实质内容（仅在 fmt 口径注后补一条 v1.10 阅读说明）、**§10 全章**、**PRD**；§11 头部 `[CODE_REVIEWED: PASS: 2026-09-22]` 段中对 W1/W2 的描述系**评审时点记录**，**保留不改**，处置口径以本条为准。演进记录见附录 v1.10。
>
> **[§11 勘误 v1.11：D-1 裁定闭合（空调串口校验位按 ①）同步，2026-09-22]**
> **勘误范围（用户裁定的设计侧同步登记；`[DESIGN_APPROVED: 2026-09-21]` 与 `[CODE_REVIEWED: PASS: 2026-09-22]` 两标记**保留不动、不重开评审**；**只改本设计文档** —— 不改 PRD、不改代码/配置）**：
> ① **裁定内容（本条的实质）**：用户于 2026-09-22 就 **PRD §9.10 的 D-1（空调串口校验位；设计侧对应 **RC-6**）** 作出裁定 —— **采用方案 ①**：新增 **per-station `parity` 字段**（`none` 缺省 / `even` / `odd`），**同口各站 `parity` 必须一致**（与 `baud_rate` 同级、同一道校验规则），**空调站配 `even`**；**方案 ②（要求现场把空调参数 40016 置 0）被否**（依赖现场操作、不可复现、不可审计）；**空调站投产限制随之解除**（原"在裁定前空调站不得投产"**不再适用**，空调站可与其余站点一同投产）。**PRD 侧登记** = `02-MUPC-南向通信-PRD.md` 的 `[§9 增补 v1.11]`（登记 1/2/3）与 §9.10 D-1 行 ⑤⑥（**本设计不擅自改 PRD**）。
> ② **同步范围（本文档内，一处不留旧口径）**：**§11.4.1** 的 `StationParity` 注（明确"**已裁定按 ①、2026-09-22 生效**"）、**§11.4.5** 末条（`parity` 透传由"裁定前置"改为"**已落地**"，并指明落地函数与用例）、**§11.5.1 规则 16**（`parity` 一侧为**活判据**）、**§11.11.2 AC-1 ③**（补规则 16 的 `parity` 用例条目）、**§11.11.3 RC-6 行**（**仍成立**，性质改为"按 ① 落地后的**真机实测 + 现场核对**"）**与 RC-11 行内的 `hvac` 括注**、**§11.12.1/§11.12.2 的"无 D-1 条目"声明**（见 ⑥）、**§11.14 一致性声明**。
> ③ **"同口 `parity` 一致性"及其 AC 项 —— 保留有效（回应评审记录中的悬空待办）**：终审/评审记录中曾登记「**D-1 若裁定为 ②，§9.4.3 的「同口 `parity` 一致性」与 AC-1 ③ 对应触发项将失去对象（`baud` 一侧仍可验），须随裁定回写**」（**该措辞在 PRD 顶部终审块第 ⑥ 段**；本设计**正文**原无同款文字）。**因实际裁定为 ①，该回写动作不适用、无需执行** —— 规则 16 的 `parity` 一侧与其 **AC-1 ③ 触发项均保留有效**，且有**可执行落点**：实现 = `mupc-southd/src/config.rs` 跨站一致性段（`:308-327`，`parity` 与 `baud_rate` **同一循环**、同一条防线）；用例 = `mupc-southd/tests/s3b2_config.rs::ac1_rule16_same_port_parity_consistency`（`:698`：同口异 `parity` → `Err` 含 `parity`；同口同 `parity` → `Ok`）。
> ④ **RC-6 的处置：仍然成立，但性质变更（不得一刀切撤销）**："**代码已支持 `parity` 透传**"（**已实现**，本轮交付）与"**现场空调确实工作在偶校验**"（**未核实**）是**两件事** —— 后者是厂方参数 40016 的**实际取值**，只能现场核对。故 **RC-6 保留**：现场动作 = ① **真机实测**空调站通信正常（9600/8E1，`parity: even` 已在配置中）；② **顺带核对** 40016 实际值确为偶校验（结论记入投运记录，与 RC-1 同批）；**实测不成立时**的处置 = **改配置 `parity`**（字段与透传均已就位，**禁止**改代码加设备特判）。
> ⑤ **回代码核实（2026-09-22，四条与裁定描述一致，未发现不符）**：a. `StationParity{None(缺省), Even, Odd}` 与站级 `parity` 字段已定义（`config.rs:29-39` / `:75`），**规则 16 的 `parity` 同口一致性校验已在 `validate()` 跨站段实现**（`config.rs:308-327`）；b. **空调站在** §9.4.1 参考配置、`mupc/deploy/config/mupc_core_config{,.production}.yaml`、`mupc/crates/mupc-southd/tests/fixtures/south_stations_s3b2.yaml` 中**均为 `parity: even`**（其余各站 `none`）；c. `parity` **已透传至串口**（`port_runtime.rs::bus_config`，`:161-171` **三值逐一对映** `rs485_plugin::Parity`），并有纯函数用例 `bus_config_passes_station_parity_through`（`:607`）；d. 故 **§9.4.1 的 `parity` 字段自本条起生效**（原"其生效以 D-1 裁定为前提"的条件**已满足**）。
> ⑥ **Δ 表 / 追认清单：无 D-1 条目，无需同步（核对结论）**：`parity` 是 **PRD §9.4.1 已定义的站级字段**、**非"设计新增"** ⇒ **不进 §11.12.1 的追认清单**；裁定与 PRD 表述**同向、无差异** ⇒ **不进 §11.12.2 的 Δ 表**（已在该表后补"v1.11 说明"）。**§11.5.3 的用例分类清单亦无需增补**：规则 16 的 `parity` 用例（`ac1_rule16_same_port_parity_consistency`）属**新增用例**，不在"fixture 订正 / 断言订正 / 断言不得改"三类订正清单的范围内。
> ⑦ **不改动**：§11.4 其它数据结构、§11.5.1 其余 16 条与规则 18/19、§11.5.2 四判据语义、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分、**§10 全章**、**PRD**（由需求侧并行登记）；§11 头部 `[CODE_REVIEWED: PASS: 2026-09-22]` 段与 §11.11.3 RC-11 行中的**历史记录保留不改**。演进记录见附录 v1.11。
>
...
| `crates/mupc-southd/src/port_runtime.rs` | 修改 | `StationBus::read_discrete`；`Rs485PortBus` 实现（`bus_lock` + `spawn_blocking` 同构）；`MockBus` 位注入；**`parity` 透传（v1.4 补；实现 = `bus_config` 纯函数，由 `open` 调用；v1.11 起为已裁定项 ① 的落地）** | §10.2/§10.7 口级总线 |
...
/// 2026-09-22；**字段自该日起生效**，见 §11 头部勘误 v1.11 与 §11.11.3 RC-6）
...
| 16 | 同口一致性 | `validate_port_consistency` | 同 `port` 各站 `baud_rate` **与 `parity`** 必须一致 → Err | 既有 baud 循环扩展 parity（缺省 `none` 参与比较：空调站配 `even` 而同口另站未写 → Err，防静默忽略）。**v1.11**：D-1 已裁定按 ①（新增站级 `parity`、空调 `even`）⇒ 本条的 `parity` 一侧为**活判据**（可执行用例见 §11.11.2 AC-1 ③，实现落点 `config.rs:308-327`） |
...
| **② 纯新增文件零报出（v1.10 收敛：判据 ① 的推论，非独立判据）** | 本 Task **新建**的 `.rs` 文件全部行均为新增行 ⇒ 判据 ① 退化为"该文件 **0 处**报出"。**适用对象限定（W1 裁定）**：仅适用于 **v1.6 口径订立后（T4 起）各 Task 新建**的文件，时间基准点 = 新建该文件的 Task 的 diff 起点；**T1–T3 期间新建/修改的文件不适用本条追溯**，其不符行一律按下方「S3b-2 格式债基线清单」登记、按判据 ③ 处理 | `rustfmt --edition 2021 --color never --check <新文件>` → 无输出 |
...
3. **判据 ③ 的实例（须区分，防误判为 T4 回归）**：同一文件 `tests/s3b2_config.rs` 整体另有 **26 个 hunk / 33 条不符合行**（按判据 ① 的 `-` 行计；**该文件合计 27 个 hunk / 34 条**），**全部由 T3 引入**（`git show 8db56df` 的既有行），按判据 ③ **不在本 Task 处置范围**。**这正是判据 ① 与 ② 并存的原因**：对"跨 Task 演进的文件"必须用**行级**判据（①），只有**纯新增文件**才适用文件级判据（②）（**v1.10 收敛注**：② 已订正为 ① 在纯新增文件上的**推论**且限定适用于 T4 起新建的文件，不再是可追溯至 T1–T3 的独立文件级判据 —— 裁定理由见 §11 头部勘误 v1.10 ①）。同理，T1–T3 若曾修改既有文件（如 `config.rs` / `points.rs` / `scheduler.rs`），其既存不符合属基线，不计入本特性。

**S3b-2 格式债基线清单（v1.10 新增 —— W1 裁定的登记落点，66 条不符行的权威源）**：

T1–T3（早于 v1.6 fmt 口径订立）期间新建/修改文件的全部 `rustfmt` 不符行，**统一按判据 ③ 既存基线处理**，不计入任何 Task 的判据 ①。逐行 blame 归因（评审报告 2026-09-22 W1/O1 + v1.10 勘误时 `rustfmt --edition 2021 --color never --check` 复跑，2026-09-22）：**T3 引入 66 条；T4–T7 自身零条**（T4 曾引入 1 条 `s3b2_config.rs:377`，已由 T7 修净 —— 复跑该文件 33 条/26 hunk，与 v1.7 记录的 34 条/27 hunk 之差即此条）。
...
| `src/scheduler.rs` | **6** | 25 | 其余 19 条为既有基线（v1.6 记录的 22 处同源） |
| `src/mapper.rs` | **3** | 3 | 全部 T3 引入 |
| `src/port_runtime.rs` | **1** | 12 | 其余 11 条为既有基线 |
| **合计** | **66** | — | 与评审报告 O1「16 + 其余 50（33/7/6/3/1）」一致 |

**格式债待办（另立，指向 `docs/technical-debt.md`）**：上表 66 条应作为**独立的格式债任务**登记至 `docs/technical-debt.md`（单独立项、独立 PR 清理，**不与本特性混提**；登记动作归项目经理执行，本清单为权威源）。对照项：判据 ② 适用域内的三个新增文件（`src/point_table.rs` / `tests/point_table_vs_reference_config.rs` / `tests/s3b2_decode_e2e.rs`）v1.10 复跑仍 **0 条**，条目 1 结论不变、不在基线清单内。
...
> **RC-9…RC-12 的来源与编号说明（v1.4 扩至 4 条；v1.9 订正为 3 条）**：PRD §9.9.2 的 RC 表只到 **RC-8**，但这四项分别由 **PRD §9.7.6**（钢瓶气压）、**§9.10 Q-9**（探测器地址序）、**§9.10 Q-19**（消防/空调总线归属）、**§9.7.2 第 3 条 + §9.8.4**（消防/PCS 版本核对，v1.4 补）明文要求（前两项还需**展示层口径**与**交叉校验**落地，RC-12 明确**不得**在系统内落代码）。本设计**续号补充**并**上报需求侧回写 PRD §9.9.2**（§11.12.2 Δ-6），不擅自改动 PRD 的 RC 编号体系。
>
> **v1.9 订正**：上句中 **RC-11 已于 2026-09-22 现场核对后撤销**（结论见本表 RC-11 行）⇒ **有效续号的为 RC-9 / RC-10 / RC-12 三项**（**编号不回收、不重排**，保留 RC-11 的空位与原始条目以便追溯），**上报需求侧回写 PRD §9.9.2 的清单随之收窄为三项**（Δ-6 同步）。**RC-11 撤销 ≠ RC-8 撤销**：RC-8（`pcs` 只读站与 intercore 串口是否同总线）**仍未结清**，门槛**一字不动**（理由见 §11 头部勘误 v1.9 ③）。
...
> **v1.11 核对：本表无 `parity` / D-1 相关条目，无需同步。** `parity` 是 **PRD §9.4.1 已定义**的站级字段（`none` 缺省 / `even` / `odd`，同口一致性校验同 `baud_rate`），**非"设计新增"** ⇒ 本表**不需**新增条目、也**不需**状态变更（对照项：本表的 1 / 2 / 3b 等均为设计侧新增或 PRD 未定义项）。D-1 的裁定（按 ①）与 PRD 表述**同向、无差异**，处置见 §11 头部勘误 v1.11。
...
| **Δ-6**（**新，v1.3；v1.4 扩列；v1.9 收窄**） | PRD §9.6.1 要求 `fire` 的数据进 **events + SSE**，但 **PRD §9.9.1 的 AC 表没有对应验收项**；同理 **§9.7.6 的钢瓶气压展示口径、§9.10 的 Q-9/Q-19、§9.7.2 第 3 条 + §9.8.4 的消防/PCS 版本核对，均未进 §9.9.2 的 RC 表** | 需求侧确有要求（上述各条均为明文），但**验收侧无落点** | 本设计在 §11.11.2（AC-5 事件侧扩展）与 §11.11.3（**续号 RC-9/RC-10/RC-11/RC-12**）补齐落点并上报；**建议需求侧回写 PRD 的 AC/RC 表**（本设计不擅自改 PRD 编号体系）。<br>**v1.9 收窄（2026-09-22）**：其中 **§9.10 Q-19 / RC-11 已由现场核对结清并撤销**（消防/空调直连 EMS，非并接 BMS 485-2 —— 见 **§11.11.3 的 RC-11 行**）⇒ 本项**待需求侧回写 §9.9.2 的 RC 清单收窄为 RC-9 / RC-10 / RC-12 三项**（**编号不回收**，RC-11 空位与撤销依据保留在 §11.11.3）；**Q-19 自身的关闭由需求侧在 PRD §9.10 另行同步**（本设计不改 PRD） |
...
> **v1.10 收敛注（W1 裁定，本注与上表 T7 行闸门 ② 的阅读说明）**：判据 ② 已收敛为判据 ① 的推论并限定适用于 **T4 起新建**的文件 —— T7 行闸门 ② 所列三个文件（`point_table.rs` / `tests/point_table_vs_reference_config.rs` / `s3b2_decode_e2e.rs`）**不受影响**（v1.10 复跑仍 0 条，"T7 复跑仍须 0 处"照旧）；**T1–T3 新建的 `points.rs`（16 条）/ `s3b2_config.rs`（33 条）等 66 条不符行不适用判据 ②**，已显式登记为 §11.11.2.1「S3b-2 格式债基线清单」（判据 ③ 处理 + 独立格式债待办指向 `docs/technical-debt.md`）。T7 闸门 ③ 中"T3 遗留在 `tests/s3b2_config.rs` 的 26 个 hunk / 33 条既存不符合行不动"与本清单一致。
...
| 版本 | 主要变更 |
|------|----------|
| **v1.11（2026-09-22，D-1 裁定闭合同步 —— 不改设计语义、不改代码/配置、不重开评审）** | **只改本文档**（不新建文件、**不改任何 Rust 代码/配置文件**、**不改 PRD**、**不改 §10**）。**背景**：用户于 2026-09-22 就 **PRD §9.10 的 D-1**（空调串口校验位；设计侧对应 **RC-6**）给出裁定 —— **采用方案 ①**：新增 **per-station `parity` 字段**（`none` 缺省 / `even` / `odd`，**同口各站 `parity` 必须一致**，与 `baud_rate` 同级同一道校验），**空调站配 `even`**；**方案 ②（现场把空调参数 40016 置 0）被否**；**空调站投产限制解除**。**同步 1（§11.4.1 —— `StationParity` 注）**：明确"**已裁定按 ①、2026-09-22 生效**"（`parity` 是 PRD §9.4.1 字段，非设计新增）。**同步 2（§11.4.5 末条 —— `parity` 透传）**：由"**D-1 裁定的代码侧前置**"改为"**已落地**"（裁定已下 ⇒ 只改配置即可生效），并补落地函数与用例（`port_runtime.rs::bus_config` `:161-171` 三值对映、`bus_config_passes_station_parity_through` `:607`）。**同步 3（§11.5.1 规则 16）**：注明 `parity` 一侧**为活判据**。**同步 4（§11.11.2 AC-1 ③）**：补"规则 16 的 `parity` 一侧"条目（同口异 `parity` → `Err`；用例 `tests/s3b2_config.rs::ac1_rule16_same_port_parity_consistency` `:698`）。**同步 5（§11.11.3 RC-6 行 / RC-11 行内 `hvac` 括注）**：**RC-6 仍成立**（性质由"裁定前置"变为"**按 ① 落地后的真机实测**"）—— "代码已支持 `parity` 透传"（**已实现**）与"**现场空调确实工作在偶校验**"（**未核实**，厂方参数 40016 的实际值）是**两件事**，故 **RC-6 不撤销**：现场动作 = 真机实测 9600/8E1 通信正常 + 顺带核对 40016 实际取值；不符时**改配置**（不得改代码）。**同步 6（§11.12.1 表后 / §11.12.2 表后）**：各补一句"**无 D-1 条目、无需同步**"的核对结论（`parity` 为 PRD 已定义字段 ⇒ 不入追认清单；与 PRD 同向无差异 ⇒ 不入 Δ 表）。**同步 7（§11.14）**：`parity` 一行改为"已裁定按 ①、2026-09-22 生效"。**"失去对象"待办处置（回应评审记录）**：终审/评审记录中「D-1 若裁定为 ②，「同口 `parity` 一致性」与 AC-1 ③ 触发项**失去对象**（`baud` 侧仍可验）、须随裁定回写」（措辞在 **PRD 顶部终审块第 ⑥ 段**，设计正文原无同款文字）—— **前提未发生（裁定为 ①），该回写不适用、无需执行**；规则与 AC 项**保留有效**。**回代码核实（2026-09-22，四条与裁定描述一致）**：`StationParity{None(缺省),Even,Odd}` + 站级字段 + 规则 16 一致性校验（`config.rs:29-39`/`:75`/`:308-327`）；空调站在 §9.4.1 参考配置、`deploy/config/mupc_core_config{,.production}.yaml`、`tests/fixtures/south_stations_s3b2.yaml` **均为 `parity: even`**（余站 `none`）；`parity` 已透传（`port_runtime.rs::bus_config`）。**未改动**：§11.4 其它数据结构、§11.5.1 其余 16 条与规则 18/19、§11.5.2 四判据、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分、§10 全章、PRD；两标记 `[DESIGN_APPROVED: 2026-09-21]` / `[CODE_REVIEWED: PASS: 2026-09-22]` 均**保留**（放行后的勘误，非修订、不重开评审）。 |
| **v1.10（2026-09-22，`[CODE_REVIEWED: PASS]` 后的勘误订正 —— 处置评审警告 W1/W2，不改设计语义、不改代码、不重开评审）** | **只改 §11**（不新建文件、**不改任何 Rust 代码/配置文件**、**不改 PRD**、**不改 §10**）。**背景**：项目级最终代码评审（`../../reports/S3b-2-代码评审报告-2026-09-22.md`）放行 S3b-2 时留下 **2 项文档口径类警告**，本勘误逐条裁定并落文本。**勘误 1（W1 —— §11.11.2.1 fmt 判据收敛）**：判据 ②（文件级"新建文件 0 处报出"）与同节条目 3（T3 新增行按判据 ③ 基线）**口径相左**（钉证：T3（`f83a838`）新建的 `points.rs` 实测 **16 条不符行 / 8 hunk**，v1.10 勘误时 `rustfmt --color never` 复跑）。**裁定 = 取消判据 ② 的独立判据地位，统一收敛为判据 ① 的行级口径**：纯新增文件的每一行都是新增行 ⇒ 判据 ① 数学上退化为"0 处报出"，判据 ② 本非独立命题；真正缺陷是**时间基准点**未写明（fmt 口径迟至 v1.6/T4 期才订立，判据 ② 不得追溯适用于 T1–T3）。收敛后：判据 ① 为唯一判据；"新建文件零报出"仅对 **T4 起新建**的文件适用（基准点 = 新建 Task 的 diff 起点）；**T1–T3 的全部不符行（66 条，逐行 blame：T3 66 / T4–T7 自身 0 / T4 那 1 条 `s3b2_config.rs:377` 已由 T7 修净）显式登记为「S3b-2 格式债基线清单」**（§11.11.2.1 新增段，含逐文件表：points.rs 16 / s3b2_config.rs 33 / config.rs 7 / scheduler.rs 6 / mapper.rs 3 / port_runtime.rs 1；与评审报告 O1 一致），按判据 ③ 处理；**另立格式债待办**指向 `docs/technical-debt.md`（独立立项、独立 PR，登记动作归项目经理，本清单为权威源）。条目 1 的三个新增文件复跑仍 0 条、T7 闸门 ② 对其"复跑仍须 0 处"照旧；§11.13 T7 注后补 v1.10 阅读说明。**勘误 2（W2 —— §11.11.2 AC-5 括号注订正为等价落点）**：原注"（core-bin 侧单测断言 `last_data_ts` 不被刷新）"无直接落点。**裁定 = 取评审建议 ②（订正表述），不补 core-bin 断言**：等价保证是结构性的且已双重钉住 —— (a) `last_data_ts` 唯一写入路径 = `set_latest_data`（`ai_integration.rs:186`/`:212`），生产唯一调用点 = `SouthSink::on_grid_package`（`startup.rs:378`；`on_station_telemetry:385`/`on_battery_soc:445` 不触碰）；(b) scheduler 侧 `pcs_station_never_triggers_grid_or_soc_channels`（`scheduler.rs:2542`）/`non_battery_role_never_triggers_soc_channel`（`:1543`）断言 `on_grid_package` 不被非 grid 站调用 ⇒ 再补时间戳比对断言属重复校验（需 mock 时钟接缝，成本收益不匹配），不立项。AC-5 行括号注已改为等价落点表述（含 file:行）。**未改动**：§11.4 数据结构、§11.5.1 十七条落点、§11.5.2 四判据语义、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分实质、§10 全章、PRD；§11 头部 `[CODE_REVIEWED: PASS: 2026-09-22]` 段的 W1/W2 描述系评审时点记录，保留不改、以本条为准。两个标记 `[DESIGN_APPROVED: 2026-09-21]` / `[CODE_REVIEWED: PASS: 2026-09-22]` 均**保留**（放行后的勘误，非修订、不重开评审）。 |
| **v1.9（2026-09-22，`[DESIGN_APPROVED]` 后的勘误订正 —— 不改设计语义、不重开评审）** | **只改 §11**（不新建文件、**不改任何 Rust 代码/配置文件**、**不改 PRD**、**不改 §10**）。**背景**：用户于 2026-09-22 就 **PRD §9.10 Q-19**（设计侧续号为 **RC-11**）给出**现场核对结论**：**消防主机、空调各自直连 EMS（BECG-3568），未挂接储能侧 BMS 的 485 总线** ⇒ **双 master 共总线风险不成立，RC-11 撤销**，`fire`（`ttyS6`）/`hvac`（`ttyS3`）两站**按原计划启用**。**依据**：`hw/微信图片_20260908170935_64_1061.png`（BECG-3568 接线拓扑图 = EMS 的硬件载体）逐口为 `RS485-1 → PCS`（另有 CAN 直连）/ `RS485-2 → BMS` / `RS485-3 → 空调` / `RS485-4 → 关口表` / `RS485-5 → 储能表` / `RS485-6 → 消防`。**原疑问的来源（如实登记，防后人重犯）**：**BMS 对 PCS 的 CAN 协议图 2-1-1「总控模块连接PCS设备」**（提取件 `hw/通信协议/_extracted/proto_BMS 对 PCS__…CAN2.0通信协议A2版.txt`，对象清单含「消防主机」「空调」）是**储能厂家（华塑）以 SCU 总控为中心画的自有系统视图**，**并非本项目接线拓扑** ⇒ 据此推断消防/空调在 MUPC 侧挂 BMS 总线属**误读**；BMS 对 EMS 协议离散输入位 **482「485-2通讯失联故障」**（`proto_BMS对 EMS__…LAN通信协议B0版.txt`）**只说明 BMS 自身有第二个 485 口，不能推出消防/空调挂在其上**。**勘误 1（§11.11.3 的 RC-11 —— 撤销）**：RC 列标注"**已撤销（2026-09-22）**"，**原条目内容一字不删**（保留"曾有此核对项"的历史），并追加**结清结论/依据/原误读来源/证据瑕疵**四段；原回写项标注"前提已不成立、不再适用"；原设计侧准备的"与 RC-8 同构（跨段互斥校验只覆盖 `intercore` 串口、不覆盖此情形）"一句**保留并注明该论断仍成立**（只是此情形已排除）。**勘误 2（同节 RC-9…RC-12 来源注 —— 有效续号收窄）**：明确"**有效续号的为 RC-9 / RC-10 / RC-12 三项**"，**编号不回收、不重排**（保留 RC-11 空位与原始条目以便追溯），上报需求侧回写 PRD §9.9.2 的清单**随之收窄为三项**。**勘误 3（§11.12.2 Δ-6 同步）**：待需求侧回写 §9.9.2 的 RC 清单收窄为三项；**Q-19 自身在 PRD §9.10 的关闭由需求侧另行同步**（本设计不改 PRD）。**补登（§11.3 加"设备-端口映射的硬件依据"一段）**：6 站"站 ↔ 串口"对应关系与 §10.1 的 RS485 分配**同源**，依据 = **BECG-3568 接线拓扑图** + **BECG-3568 BOX 规格书**（8 路隔离 RS485 = COM1–8 ↔ `ttyS0`、`ttyS2`–`ttyS8`，无 `ttyS1`）；端口/从站号仍为**待现场校准项、不得硬编码**。**另登记（原始证据瑕疵）**：拓扑图**原图**末两框误标 `RS485-4`（与关口表重复），已在 `hw/微信图片_20260908170935_64_1061_修正.png` 订正为 `RS485-5` / `RS485-6`（**原件保留未动，作为原始证据**；引用以订正件为准）。**RC-8 不受牵连（刻意确认）**：RC-11 与 RC-8 **同构但不同事** —— 共享的是"跨段互斥校验的能力边界**未变**"这一陈述，而 RC-8（`pcs` 只读站 `ttyS7` 与 intercore `ttyS0` 是否物理独立，PRD §9.10 Q-15）**仍未结清**，"若同总线 ⇒ `pcs` 站不启用（保持注释）"的门槛**一字未动**（§11.7.4 的 `pcs` 行、§11.11.3 的 RC-8 行**均未修改**）。**未改动**：§11.5.1 十七条落点、§11.5.2 四判据语义、§11.5.3 三类清单、§11.4.7.2 两条裁定、§11.13 Task 拆分、**§10 全章**、**PRD**；§11 头部 `[CODE_REVIEWED: PASS: 2026-09-22]` 段中"遗留项 … RC-11 …"属**评审时点记录**，**保留不改、以本条为准**。两个标记 `[DESIGN_APPROVED: 2026-09-21]` / `[CODE_REVIEWED: PASS: 2026-09-22]` 均**保留**（批准后的勘误，非修订、不重开评审）。 |
