...
| 16 | 同口一致性 | `validate()` 内联段 | 同 `port` 各站 `baud_rate` **与 `parity`** 必须一致 → Err | 既有 baud 循环扩展 parity（缺省 `none` 参与比较：空调站配 `even` 而同口另站未写 → Err，防静默忽略）。**v1.11**：D-1 已裁定按 ①（新增站级 `parity`、空调 `even`）⇒ 本条的 `parity` 一侧为**活判据**（可执行用例见 §11.11.2 AC-1 ③，实现落点 `config.rs:308-327`） |
