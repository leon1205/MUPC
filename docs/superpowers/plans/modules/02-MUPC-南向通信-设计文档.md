...
> **[§12 T9 实现实测证伪的合同勘误回写（v1.13-r4，2026-09-24）—— 门禁标记不变]** 依据 = **T9 实现 + 注入探针实测**（`6370041` / `9ecb51a`）；**只订正合同文本中与实测不符处，不改任何契约、约束与伪码语义**：
> - **① §12.4.5 `judges_evaluable` 的 `meter_grid` 支路漏 `p_total`**：v1.13 只列 `p/q/pf/u/i`，与 **PRD §10.3.2 的 `R(role)` 表**（唯一权威口径，含 `p_total`）**不对称**。T9 按评审建议新增的**交叉断言** `runtime_guard_and_config_criterion_agree`（两侧集合必须相等）**当场查出该不对称**（当前不可达，因 MeterGrid 不进遥测/事件段 ⇒ 守卫对其恒不被调用）⇒ 已两侧对齐为 `p/q/pf/u/i/p_total`，并删去"不对称不可达"的豁免断言改为正面相等断言。
> - **② §12.8 `edge_memory_is_per_group` 的构造**：原写"快组 t=0/1000/2000…、慢组 t=5000"—— **该构造非判别性**：快组 1000 **整除**站周期 5000 时，承载组在 t=5000 prime 的**同一 tick** 内快组随即再 prime ⇒ 掩盖"记忆被清空"⇒ 把 tracker 键退回"按站存"该用例**仍绿**（T9 注入探针实测：快组 **2000** 时注入 ⇒ **FAILED**；快组 1000 时注入 ⇒ **ok**）⇒ 订正为**快/慢组取 2000 / 5000**。**实现按订正后的构造**（`scheduler.rs` 用例）。
> - **③ §12.8 S-3 锚的承载组到期 tick**：原写 `t=0/2000/4000/6000`，**"…/6000" 不成立** —— 退避按 `offline_count` 指数后移 `next_due`（`oc=2 ⇒ extra = 2 × 组周期 = 4000`）⇒ 承载组 `next_due` 被推到 **8000**、**t=6000 不采**（T9 探针实测 `input_call_count(1,100) == 3` 通过）⇒ 订正为 `t=0/2000/4000`，并登记"第三次 `fail_input_once` 属**滞后消费**（会在 t=8000 被吃掉）；用例止于 t=6000 故无害，**将来延长 tick 须注意**"。
> **未改**：C1–C9 的任何约束、AC-8-1…AC-8-7 的任何判据、§12.4.1–§12.4.4 的伪码与不变量、任何数字口径、§1–§11、任何代码（本行只改合同文本）；**`[DESIGN_APPROVED: 2026-09-23]` 标记原文未动，覆盖范围仍只到 §12**。
...
> **[§13 新增（v1.14，2026-09-26）：PCS 控制面归属]** 新增 **§13**，落实用户 2026-09-26 就 **§11.8①「PCS 写归属须单独评审后再立项」** 作出的裁定 —— **把 PCS 通信与控制整体迁入南向**，`mupc-intercore` 收敛为纯核间 TCP 帧协议。**改动范围**：① 本文件**仅追加 §13**（**§1–§12 正文零改动**；§11.8 的"归属未定前一律只读"由 §13.1 结清取代，差异登记 **Δ-15**）；② 既有门禁标记 `[DESIGN_APPROVED: 2026-09-21]` / `[CODE_REVIEWED: PASS: 2026-09-22]` / `[DESIGN_APPROVED: 2026-09-23]`（§12）**原文未动、覆盖范围不变**（**均不含 §13**）；③ **未改**任何 PRD、其它模块文档、任何代码/配置 —— 跨文档回写（02 PRD N-4、10 号 §11 与 ADR-011 交叉引用、03/12 号引用、`technical-debt.md`）**与代码同批在 T4 执行**（§13.12），避免文档先于事实。
>
> **本块不代表任何批准**：**本次未获门禁标记**（**不自行添加 `[DESIGN_APPROVED]`**）。**待设计评审；其中 Δ-16（在线/离线判据统一）须评审确认、Δ-17（三相降级粒度）为待裁定项**（§13.11）。**实现状态：已实现**（提交号见 **§13.13**）—— **但未获门禁标记，待独立评审**（§13.13 的"已实现"只指代码落地，不代表设计准予）。
...
13. [PCS 控制面归属（结清 §11.8①）](#13-pcs-控制面归属结清-118)
...
fn judges_evaluable(role: Role, reads: &BlockReads) -> bool {
    match role {
        // p/q/pf/u/i/**p_total** 齐备（= `R(meter_grid)` 的完整集合，PRD §10.3.2 的
        // `R(role)` 表为**唯一权威口径**；`p_total` 供 `scalar_total` 降级求和。
        // **v1.13-r4 订正**：v1.13 此处只列 p/q/pf/u/i，与同表的 `R(meter_grid)` **不对称**
        // —— T9 的交叉断言 `runtime_guard_and_config_criterion_agree` 就是为此而加，
        // 并当场查出该不对称；现已两侧对齐）
        Role::MeterGrid => ["p", "q", "pf", "u", "i", "p_total"].iter()
...
| 单测·调度 | `edge_memory_is_per_group`（**T9 实测订正构造，v1.13-r4**）：快/慢组周期必须取 **2000 / 5000**（**不是**本节原先写的 1000/5000）；**交错 tick** 下位块的 0→1 **必须**产事件（若 tracker 键未改 ⇒ 静默吞掉 ⇒ 本用例**必红**）。**为什么不能用 1000/5000**：快组 1000 **整除**站周期 5000 时，承载组在 t=5000 prime 的**同一 tick** 内快组随即再次 prime ⇒ 把"记忆被清空"这一缺陷**掩盖**掉 ⇒ 即使把 tracker 键退回"按站存"，用例仍**绿**（T9 已用注入探针实测：快组 2000 时注入该缺陷 ⇒ **FAILED**；快组 1000 时注入 ⇒ **ok**） | §12.4.4 的机制钉子 |
...
>
> **本章为新增章节**（接续 §11/§12 编号）。**§11.8 正文一字未改**（其"归属未定前 `southd` 对 PCS 一律只读"在本章生效后由 §13.1 的结清取代，差异以 **Δ-15** 登记）。§1–§12 的其余条款（含 `StationBus` 只读保证、`Role::Pcs` 点表、618 点参考配置、S3b-3 分组调度）**均不被本章改动**，只改 PCS 的**归属与驱动**。
>
> ⚠️ **本章本次未获门禁标记**（待设计评审；**不自行添加 `[DESIGN_APPROVED]`**）。既有 `[DESIGN_APPROVED: 2026-09-21]`（覆盖 §11）与 `[CODE_REVIEWED: PASS: 2026-09-22]` 的**原文未动、覆盖范围不变**。

### 13.1 §11.8「写 Task 立项时须先结清的 4 件事」逐条结清

**本次评审即 §11.8① 所要求的"单独评审"**，裁定 **下放 southd**。四件事的结清如下：

| §11.8 条 | 原文要求 | 本章结清方式 |
|------|----------|--------------|
| **① PCS 写归属** | "并入 intercore 还是下放 southd"须单独评审；归属未定前 `southd` 对 PCS 一律只读 | **裁定下放**。`PcsHandle` 成为 PCS 4 区 **500 的唯一写方**；S2 联锁经 `InterlockPort`（4 方法薄 trait，**签名不变**，`interlock.rs:333-343`）调用它 ⇒ **不存在两个写方**。**latch 权威源仍为 storage/DB**（C-1 双 latch 结构不变）；`PcsHandle` 只持"运行期兜底 latch"，与现状 `ModbusRtuTransport.stopped_latched` 同义 |
| **② 写路径的仲裁** | 写必须与读共用同一个 `Rs485PortBus`（同一 `bus_lock` + 同一 `tx_lock` 语义） | **以更强的方式满足**：PCS 的采集与控制**同属 `PcsHandle`、共用同一把锁**（§13.4）⇒ 原文所防的"读路径与写路径分属两个所有者、口内请求交错"这一前提**不再存在**。并且**PCS 独占该串口**（校验规则 **P-2**），不与任何 `south_stations` 站共口 |
| **③ 控制动作的准入** | 写指令必须经策略/AiValidator 链路（PRD N-5），须有"谁/何时/写了什么/回读确认"审计 | 写入口**收敛为 4 条**且全部来自既有链路（§13.5.3）；**新增写审计**：每次写序列投一条 `events` 记录（调用方 token / 时刻 / 寄存器+值 / 回读值），复用既有事件通道与 `AlertFeed`，**不新增通道** |
| **④ 与 S2 的分工不得重叠** | `southd` 负责**采集面**；S2 负责**安全联锁面**；两面接口 = 既有事件通道，**不新增直连调用** | **部分修订，登记 Δ-15**：④ 的"不新增直连调用"**限定于采集面**。PCS 迁入后新生的**控制面**（`PcsHandle`）与联锁面之间**必须有**一条直连 —— 停机是实时动作，事件通道是异步旁路，**不可承载**。该直连**收敛为唯一一条**：`InterlockPort` 薄 trait；**采集面与联锁面之间仍无任何直连**（维持 ④ 的本意） |
...
```
   ...
   ┌──────────────────────────────────────────────────────────┐
   │  一把 tokio::sync::Mutex<()>（采集与控制共用，§13.4）      │
   │                                                          │
   │  ┌── 采集循环（每 interval_ms 一拍）───────────────────┐  │
   │  │ FC04 读 3 区 1000..=1075（现有 pcs_3zone 块，76 字）│  │
   │  │ → points::expand 解码 → StationSink 投递            │  │
   │  │ → 更新 PcsSnapshot（SOC/RUN_STATE/三相）             │  │
   │  │ → 停机告警去抖（1013=0 且 started 且 !latched）      │  │
   │  │ 失败：bad+=1；快照失效；bad>=3 → 离线 + 事件         │  │
   │  └─────────────────────────────────────────────────────┘  │
   │                                                          │
   │  ┌── 控制序列（从 intercore 迁入，语义原样）──────────┐  │
   │  │ 切模式 → 启停（含 S-4 守卫）→ 写功率 → 回读 → 审计  │  │
   │  └─────────────────────────────────────────────────────┘  │
   │                                                          │
   │  状态机：mode / started / stopped_latched /               │
   │          restart_authorized / connected                  │
   └──────────────────────────┬───────────────────────────────┘
                              ▼
                     Rs485Device（常开，**独占该口**）
                              ▼
        StationSink → storage(telemetry+events) / 上云点表 / AlertFeed（既有）
```

**对外接口**：`Arc<PcsHandle>` 为**具体类型门面**（各消费方收同一类型，与现状收 `Arc<IntercoreClient>` 同构）。S2 联锁侧**不直接**持 `PcsHandle`，而经 `InterlockPort` 薄 trait 适配（§13.5.2）。

### 13.4 采集循环、控制序列与快照（三项读路径合并为一条）

**现状（intercore）在 PCS 链路上有 3 条独立读路径**：

| 路径 | 周期/触发 | 读什么 |
|------|-----------|--------|
| `run_heartbeat_loop`（`modbus.rs:544`） | 每 `heartbeat_poll_ms` | 1013（1 字） |
| `latest_soc`（`modbus.rs:706`） | 每次调用（策略/展示按需） | 1010（1 字） |
| `read_three_phase`（`modbus.rs:729`） | 每次调用（展示 1 Hz） | 1022×3 + 1029×4（**两笔**） |

⇒ 稳态约 **4 次读/秒**，且三者各自持 `bus` 锁、各自维护在线判定（**判据还不一致**：心跳 3 拍判离线，`latest_soc` 因走 `read_input` **单次失败即 `mark_offline`**）。

**迁移后合并为一条采集循环**（`pcs_3zone` 块的 76 字**已完整覆盖** 1010 / 1013 / 1022-1024 / 1029-1032）：

- **一拍一次读**：FC04 `1000..=1075` ⇒ 同时得到 SOC、RUN_STATE、三相、全部 72 点
- **成功**：解码 → ① 投 `StationSink::on_station_telemetry` ② 刷新 `PcsSnapshot` ③ `bad=0` + 在线 ④ 停机告警去抖判定（条件与现状逐字相同：`run_state==0 && started && !stopped_latched`）
- **失败**：`bad += 1`；**立即置快照失效**；`bad >= 3` ⇒ 判离线 + `on_station_offline`（**不做退避** —— 与现状心跳的固定周期逐字一致；退避是站级调度器的语义，PCS 不引入）

> **"快照失效"与"在线态"是两个独立判据，勿混**：快照失效是**单拍**语义（本拍无可用读数 ⇒ 按需读返回 `None`），在线态是**累积**语义（`bad >= 3` 才翻）。前者对齐现状"`latest_soc` 读失败即 `None`"，后者对齐现状"心跳 3 拍判离线"；**两条现状判据本就不同源**，合并后统一为上述两档（差异与理由见 **Δ-16**）。

**快照结构**（只承载"按需读"的项，避免语义漂移）：

```rust
struct PcsSnapshot {
    valid: bool,                 // 采集失败 ⇒ false（且随 bad>=3 整体失效）
    ts: Instant,                 // 本拍采集时刻
    soc: Option<f64>,            // 1010，域校验 [0,100] 后
    run_state: Option<u16>,      // 1013，合法域 0..=3（M9a 口径）
    i_phase: Option<[f64; 3]>,   // 1022-1024 ×0.1 A
    p_phase: Option<[f64; 3]>,   // 1029-1031 ×0.1 kW
    p_total: Option<f64>,        // 1032 ×0.1 kW
}
```
...
| 方法 | 现状 | 迁移后 | 语义差异 |
|------|------|--------|----------|
| `last_run_state()` | 心跳维护的同步缓存（`modbus.rs:775`） | 采集快照（同步 getter） | **无**（刷新周期同为 `interval_ms`≈现状 `heartbeat_poll_ms`） |
| `is_connected()` | 心跳/事务维护 | 采集循环维护 | **无**（判据同"连续 3 拍失败"） |
| `latest_soc()` | **真读** 1010；失败即 `None` | 快照（值 + 本拍 `ts`） | 返回值与故障态**逐字等价**（失败 ⇒ 快照失效 ⇒ `None`）。**唯一差异在时间戳来源**：由"调用时刻"变为"本拍采集时刻"，最多旧 `interval_ms`(1 s) ≪ `DATA_STALE_AFTER`(5 s) ⇒ SOC 双源裁决判据不受影响 |
| `read_three_phase()` | **两段真读**，段间独立降级 | 快照（**单块**读的结果） | **降级粒度变化 → Δ-17**：块读任一失败即三相全 `None`，不再有"电流段成功、有功段失败"的中间态 |

> **收益**：PCS 链路读次数 **4 次/秒 → 1 次/秒**；三条路径各自的在线判定（判据不一致）合并为**单一口径**；`read_three_phase` 为规避 `mark_offline` 而走的 `read_input_once` 特例（`modbus.rs:719-727` 的 W1 整改）**随路径消失而消解**。

### 13.5 控制面接口

#### 13.5.1 `PcsHandle` 对外方法（从 `intercore::ModbusRtuTransport` 的 trait 实现迁入）
...
| `authorize_restart()` | 同 | M1 单次授权（消费即弃） |
| `is_interlock_stopped()` | 同 | **现状生产零调用**（仅测试覆盖）；迁移时**保留**，理由：trait 完整性 + 联锁诊断 |
| `shutdown()` | 同 | **现状生产零调用**；迁移时**删除**（TCP 侧保留） |
...
#### 13.5.3 写入入口的准入（结清 §11.8③）

PCS 4 区写入口**收敛为 4 条**，全部来自既有链路，无"采集站直写"：

| 入口 | 调用方 | 经 AiValidator/策略 |
|------|--------|---------------------|
| `send_dual_param` | `AiIntegrator`（AI 决策 / 应急降级）、`StrategyCommandHandler`（IEC104 p_set） | **是** |
| `send_tai_command` | `TaiStorageStrategy` 兜底 | **是** |
...
south_pcs:
  enabled: true
  port: "/dev/ttyS0"          # 现场按实际 RS485 口修改；须独占（规则 P-2）
  protocol: modbus
  slave: 1                    # PCS 从站地址（拨码，默认 1）
  baud_rate: 19200
  data_bits: 8
  stop_bits: 1
  parity: none                # none / even / odd
  interval_ms: 1000           # 采集周期（含心跳职能，§13.4）
  response_timeout_ms: 200    # ⚠️ 订正（2026-09-26）：原示例写 1000 属笔误 ——
                              # 真值是原 `intercore.modbus_rtu.response_timeout_ms` 的
                              # 默认 200（`default_response_timeout_ms()`），且两份
                              # deploy/config yaml 生产模板也取 200。见 Δ-21
  regs:                       # ← 自原 south_stations 的 pcs 站整段迁入（内容零改动）
    - name: pcs_3zone         # 1000–1075；_k ↔ 寄存器 999+k
      ...
      format: uint16
      scale: 1.0
      points: [ ... ]         # 31 条，逐字照搬（含 4 个 int32_scaled + lo_hi 电量点）
```

**移除**：`intercore.modbus_rtu` 子段（`serial_port` / `baud_rate` / `data_bits` / `stop_bits` / `parity` / `slave_addr` / `response_timeout_ms` / `heartbeat_poll_ms`）；`intercore.transport` 的 `"modbus_rtu"` 取值。
...
| `InterlockPort` trait 4 方法 | **无** |
| `POINT_REGS` 静态登记表（含 `Role::Pcs`） | **无** |
| `AiIntegrator` 的 SOC 双源裁决逻辑 | **无**（只换来源类型） |
| 618 点总数与逐点内容 | **无**（仅归属段变化，§13.7） |

> ⚠️ **上云点表的调用点必须改（2026-09-26 落地评审查出的静默缺口，登记于 §13.11）**
>
> §13.9 上表"上云点表**无**变化"指的是**契约不变**（点内容 / 通道掩码 / 档位口径），**不等于调用点不用动**：`build_uplink_points` 的入参是 `&SouthStationsConfig`，而 PCS 迁出后**不再在 `south_stations` 里** ⇒ 若调用点不喂 PCS，其 72 个 IOA 会**静默**从 IEC104/MQTT 点表消失，且**现有自检不会响**（`has_pcs == false` 时的期望值恰为 567 = 5 站的并集）。
>
> **要求（落在 T3 装配，见 §13.10）**：
> 1. `build_uplink_points` 的签名扩为**显式接收 PCS 段**（如 `(cfg: &SouthStationsConfig, pcs: Option<&SouthPcsConfig>)`），使**编译器强制**每个调用点表态；
> 2. `has_pcs` 取自**显式入参**而非"站表里是否存在 Pcs 站"；
...
| 编号 | 差异 | 处置 |
|------|------|------|
| **Δ-15** | §11.8④「两面接口 = 既有事件通道，不新增直连调用」被**部分修订**：新增 `PcsHandle`（控制面）与联锁面之间的**唯一一条**直连（`InterlockPort`） | **本设计主动登记**（§13.1④）。理由：停机是**实时动作**，事件通道是**异步旁路**，不可承载；且该直连**不经过采集面**，④ 的本意（采集面不得驱动控制）**未被破坏**。**建议需求侧在 PRD N-4 同轮确认** |
| **Δ-16** | **在线/离线判据被统一**：现状两条路径判据**不一致** —— 心跳（1013）为**连续 3 拍失败**判离线，而 `latest_soc` 走 `read_input`，**单次失败即 `mark_offline`**（连带清 `last_run_state` ⇒ DO1 灭灯、复位 `started`/`mode` 哨兵）。迁移后统一为**连续 3 拍失败** | ★**登记并判为改进，但须评审确认**。**依据**：`read_three_phase` 早已因同一问题（瞬时误码 ⇒ 离线/在线抖动 + DO1 闪断 + 控制侧寄存器被反复重写）被评审整改为走 `read_input_once`（`modbus.rs:719-727` W1），**`latest_soc` 未曾同等整改**；统一后该抖动路径**整体消失**。**判为改进的理由**：判据只**放宽**瞬时容错（1 拍 → 3 拍），不改变"真故障 ⇒ 离线"的结论；且与既有心跳口径**对齐**。**风险**：离线判定最长延迟由 1 拍变为 3 拍（≈3 s，同现状心跳）；**须在 §8.7 真机项中随联锁/DO 链路复核**。<br>⚠️ **落地评审查出一处必须一并保留的语义（2026-09-26 补记）**：原 `read_input`/`write_reg` 包装的副作用是**两重** —— (a) `connected` 在线态（**移交采集循环**，正确）；(b) **`mark_offline` 的缓存失效**（清 `started` + `mode` 哨兵）—— 即 **W1 的要害**：*断线期间 PCS 可能掉电/复位 ⇒ 缓存不可信 ⇒ 下次**强制重写** `REG_MODE`/`500=1`*。(b) 的语义**不在"判据统一"的范围内、不得随 (a) 一起消失**。**落点 = 采集循环的离线跃迁处**（`bad >= 3` 那一步）**必须同时复位 `started=false` + `mode=0xFF`**（与 `mark_offline` 等价）。**漏掉的后果**（fail-open）：连续 3 拍失败后恢复时，`ensure_mode` 与 `ensure_started` 均**缓存命中** ⇒ 跳过 `REG_MODE` 重写、且**连 S-4 的 1013 读都不发生** ⇒ 在**未知实际模式**下直接写 1006-1011 功率寄存器。**该复位已写入实施计划 Task 8。** |
| **Δ-17** | 三相展示读的**降级粒度**由"两段独立"变为"单块"（§13.4） | ★**待评审裁定**。影响面：12 号设计 §3.4 F5.5「点级独立降级」的 **transport 侧**落点。**收益**：PCS 链路读次数 4→1 次/秒。**论据**：① 采集块（1000×76）**已包含**两段；② 现状的"点级降级"仅在"读 76 字失败、读 3/4 字成功"的**窄场景**生效；③ F5.5 的**消费侧落点**（`DisplayDataProvider` 按 `Option` 字段打 Offline/NotRead 角标）**不变**，只是 transport 侧不再提供部分成功 |
| **Δ-18** | `latest_soc()` 的时间戳来源由"调用时刻"变为"本拍采集时刻" | 登记备查，**判为无影响**：最大偏差 `interval_ms`(1 s) ≪ `DATA_STALE_AFTER`(5 s)；SOC 双源裁决判据与结论不变。**须在 §8.7 真机项中随 SOC 链路一并复核** |
| **Δ-19** | **上云点表调用点必须改**：`uplink::build_uplink_points` 入参为 `&SouthStationsConfig`，PCS 迁出后其 72 个 IOA 会**静默**从 IEC104/MQTT 点表消失 —— 且**现有自检不会响**（`has_pcs == false` 时的期望值恰为 567 = 5 站并集，与"5 站无 PCS"的合法形态无法区分，fail-open） | ★**落地评审查出的真缺口**（T2 评审独立推演 + 实证）。处置 = §13.9 末的 4 条要求（签名显式收 PCS 段 / `has_pcs` 取显式入参 / 生产与测试共用合成函数 / 不一致时 fail-closed）。⚠️ §13.9 原文的"上云点表**无**变化"极易被读成"这一路不用动"，正是该缺口的成因 —— 已在该处加"契约不变 ≠ 调用点不变"的醒目注 |
| **Δ-20** | **P-4 的覆盖面**（§13.8）：原文声称 `south_pcs.regs` 复用 "`points::expand` 同一函数、空洞 ≤ 4、单块 ≤ 120 寄存器" | ★**实测两重不实**，已在 §13.8 的 P-4 行就地订正：① 空洞（规则 11）**不在** `expand`；② "单块 ≤ 120 寄存器"**在全仓不是任何一条独立规则**。**且最尖的一条是规则 5**（漏写 `scale` ⇒ serde 缺省 `0.0` ⇒ 整块 `raw × 0` **静默全 0**）。处置：实现侧**显式调用**块级校验函数（**已覆盖 5/10/11/12/14/19**；其中 12/19 原为站级内联、已抽成函数两处共用；**10 的判据与 role 无关**故同样复用）；规则 15（需 `&[StationConf]`）与依赖 role 的 6/13 **明确不复用并如实登记** |
| **Δ-21** | **§13.7 的 `south_pcs.response_timeout_ms` 示例值**：原文示例写 `1000` | 落地取 **`200`**（= 原 `intercore.modbus_rtu.response_timeout_ms` 的真实默认 `default_response_timeout_ms()`；两份 `deploy/config` 生产模板亦为 200）⇒ **原示例 1000 属笔误**，已在 §13.7 就地订正。**非需求变更，仅为示例值与真值对齐** |
...
| 关联 | 关系 |
|------|------|
| 本文档 §10 / §11 / §12 | **调度框架、点表登记、分组调度、只读保证（`StationBus` 无写方法）全部不变**。本章只把 **PCS 一个 role** 的驱动与配置**迁出站级调度器**。`south_stations` 的其余 5 站**零改动** |
| 本文档 §11.4.4 `POINT_REGS` | `Role::Pcs` 的 72 行登记表**保留原样**（查表键与配置段位置无关）；§11.4.4 的"漂移防护"测试断言**行数由 618 拆为 546 + 72**，**总数与逐点内容不变** |
| 本文档 §11.8 | 正文**一字未改**；其"归属未定前 `southd` 对 PCS 一律只读"由本章 §13.1 结清取代（Δ-15） |
...
### 13.13 实现状态（2026-09-26，Task 12 回写）

> ⚠️ **口径**：本节只登记**代码是否已落地**（提交号可复现）。**本章仍未获任何门禁标记****
> —— `[DESIGN_APPROVED]` / `[REVIEWED: PASS]` 等**一律不自行添加**，待**独立设计评审**（本节的
> 归类来自执行记录与提交信息，**独立评审报告未随仓提交**；若与实际评审记录不符，以评审记录为准）。
>
> **基线**：本章由 **`25fe6b9`** 新增（v1.14）；实施计划 = **`5e118b6`**
> （`docs/superpowers/plans/2026-09-26-PCS迁入南向-实施计划.md`，共 12 个 Task）。
> **提交范围**：`d292515..HEAD`（不含本文档提交）。

**① 逐 Task 提交与结果**

| Task | 内容（计划口径） | 提交（按序，短号） | 结果 | 评审轮次 |
|------|------------------|--------------------|------|----------|
| **T1** | `rs485-plugin` R-1（`write_single_register_from`）/ R-2（写响应回显校验） | `f5654e0` → `c452c8e`（修 FC06 异常分支不可达）→ `2fc1064`（规格评审勘误）→ `efe2883`（评审项收口）→ `938d341`（收口记录） | ✅ 落地；`rs485-plugin` 既有用例全绿 | 规格评审 → 修复（两轮，含规格评审轮内返工） |
| **T2** | `rs485-plugin` R-3（同步交换缝，`feature test-seam` 门控） | `4a7bd1e` → `44ac7af`（门控 `cfg(test)` → Cargo feature，下游可见）→ `7bd151a`（勘误）→ `298da59`（评审项收口） | ✅ 落地；`seam_visibility.rs` 钉住下游可见性 | 规格评审 → 修复（两轮） |
| **T3** | `StationBus::write_single` + `Rs485PortBus`/`MockBus` 实现 | `a0f1ecb` → `778d461`（评审项收口） | ✅ 落地（写能力只开一条缝） | 规格评审 + 质量评审（**质量评审为"单代理两阶段"**，非两个独立评审者） |
| **T4** | `pcs/regs.rs` 寄存器表迁入 | `556914b` → `aa7a975`（评审项收口） | ✅ 落地 | 规格评审 → 修复（两轮） |
| **T5** | `pcs/sim.rs` 从站仿真迁入（去 `tokio-modbus` 类型依赖）+ `pcs_slave` bin | `41e755c` → `9b3f3b1`（勘误）→ `1ed84cb`（收口）→ `4d636c7`（复审三处计划错误订正）→ `941830c`（前向引用锚点） | ✅ 落地；`--features pcs-slave-bin` 默认不构建 | 规格评审 → 修复 → 复审（多轮） |
| **T6** | `SouthPcsConfig` 顶层段 + `south_stations` 拒绝 `role: pcs`（P-3/P-4） | `cca8a20` → `159e15a`（P-4 覆盖如实登记）→ `1ec47b2`（规格评审订正）→ `90695dc` / `db7a2e7`（质量评审收口）→ `138fbc8`（质量评审文档） | ✅ 落地（含规则 10 复用、块级 5/11/12/14/19 显式调用） | 规格评审 + 质量评审（两轮，提交信息明文含"质量评审"） |
| **T7** | `PcsHandle` 状态机与控制序列（4 条写入口） | `3598e4d` → `491334c`（规格评审订正，含 Δ-16 的 W1 缓存失效）→ `aa09773` / `1e2aabd`（质量评审收口）→ `75fbc0e`（计划片段订正） | ✅ 落地；迁回 3 个原文用例 + 补 I-4/正向写锚 | 规格评审 + 质量评审（两轮） |
| **T8** | 采集循环 + `PcsSnapshot` + 采集面拆 `pcs/collect.rs` | `bb81823` → `bb2a927`（规格评审订正：净退化与未认领项）→ `5f0d01b`（M1 去抖）/ `d522c9a`（质量评审收口：块基址 fail-closed）→ `6d50ac4`（纯移动拆分） | ✅ 落地；三条读路径合并为一条 | 规格评审 + 质量评审（两轮） |
| **T9** | PCS 帧级 e2e（E1–E7）+ 总线重叠探测器 | `2baa250` → `aaafdb5`（E6 补互斥探测器）→ `d38f9b0`（收口记录） | ✅ 落地 | 规格评审 + 质量评审（**质量评审为"单代理两阶段"**，非两个独立评审者） |
| **T10** | `core-bin` 装配换型（**原子提交**：注入点改持 `PcsHandle` + `InterlockPort` 适配 + `core_config` 增 `south_pcs` 删 `intercore.modbus_rtu` + P-1…P-5 + 两份 yaml） | `4f131a5`（原子实现）→ `793a029`（评审项收口：`station_roles` 回归网 + 死配置接线 + 上云签名显式收 PCS 段） | ✅ 落地；安全链唯一切换点 | 规格评审（**质量评审被并入规格评审**，实际只过一轮） |
| **T11** | 删 `intercore` 的 PCS 面 + 移除 `tokio-modbus` / `tokio-serial` | `56ee0dc` → `22fed56`（收口：`remote_addr` 不再撒谎 + 清死依赖） | ✅ 落地；`intercore` 收敛为纯核间 TCP 帧协议 | 规格评审 + 质量评审（两轮） |
| **T12** | 跨文档回写（本 Task）+ 三处代码收口 | `c4fc5f7`（代码收口：`station_shell` 文档如实 / `data_bits`/`stop_bits` 校验前移 / fail-fast 源文本锚）+ **本文档提交** | ✅ 落地 | 未过独立评审（本 Task 自检；文档回写与代码同批，见 §13.12） |

**② 评审轮次的如实登记（勿夸大）**

- **本轮 12 个 Task 均经过规格评审**（每个 Task 的实现提交后都有一轮规格评审 + 返工）。
- **仅 T3 / T9 的质量评审是"单代理两阶段"** —— 同一代理先做规格、再做质量两段，**不是两个独立评审者** ⇒ 严格口径下这两个 Task **只算一轮独立评审**。
- **T10 的质量评审被并入规格评审** —— 未单独执行第二轮 ⇒ **只过一轮**（该 Task 是安全链唯一切换点，覆盖密度见其提交与回归网）。
- **T12 未过独立评审**（本文档提交即本次回写）。
- 其余 Task（T1 / T2 / T4 / T5 / T6 / T7 / T8 / T11）各经**两轮**（规格评审 → 修复 → 质量评审 → 修复）；其中 **T6 / T7 / T8 的提交信息明文写有"质量评审"**，其余以"评审项收口"记录。

**③ 差异登记补充（承接 §13.11）**

本轮落地中又暴露两条**此前未登记**的差异，已补登于 **§13.11 的 Δ 表**：**Δ-23**（`intercore` TCP
通道保留但生产路径无消费者，且 `IntercoreClient` 只发不收 —— 接收原语须未来新增）与 **Δ-24**
（`south_pcs.enabled=false` 时两条下行链路的降级口径不对称：IEC104 `p_set` 如实回 `success:false`，
AI 侧双参数/分相下发静默 no-op）。**此处不另立第二份**。

**④ 未结项**：写审计事件（§13.5.3 明列"须新增的落地项"）**本次未落地**；停机写 500=0 路径的
真机行为、`remote_addr` 访问器（零调用方）去留、fail-fast 的覆盖局限等 —— 逐条见
`docs/technical-debt.md` **§6.13**（并在 **§8.7** 挂了真机复核项）。
...
| 版本 | 主要变更 |
|------|----------|
| **v1.14-r1（2026-09-26，**本轮落地（T1–T12）的勘误与差异汇总回写**；**未获门禁标记**，`[DESIGN_APPROVED]` 待独立评审）** | **只追加 §13.13「实现状态」，并汇总本轮落地中的勘误与差异；§13.1–§13.12 的结论与 §1–§12 正文零改动**。汇总：**①** **§13.7 示例值订正** —— `south_pcs.response_timeout_ms` 由 `1000` 改为 **`200`**（= 原 `intercore.modbus_rtu` 的真实默认；两份 `deploy/config` 生产模板亦为 200）⇒ 登记 **Δ-21**；**②** **§13.8 P-4 覆盖面的两重不实已订正**（空洞不在 `points::expand`；"单块 ≤ 120 寄存器"在全仓不是独立规则）⇒ 登记 **Δ-20**，实现侧改为**显式调用**块级 5/10/11/12/14/19，规则 15 与依赖 role 的 6/13 **明确不复用并如实登记**；**③** **§13.9 补"上云调用点必须改"的醒目注**（"契约不变 ≠ 调用点不变"）⇒ 登记 **Δ-19**，落地形态 = 签名显式收 PCS 段 + `has_pcs` 取自显式入参 + 生产/测试共用 `station_shell` + 不一致 fail-closed；**④** **Δ-22**（M9a 口径收敛：总线读成功但 1013 域外不再计 `bad`）已裁定接受并登记；**⑤** **补登 Δ-23 / Δ-24** —— Δ-23 = `intercore` TCP 通道保留但**生产路径无消费者**且 `IntercoreClient` **只发不收**（接收原语须未来新增）；Δ-24 = `south_pcs.enabled=false` 时 IEC104 `p_set` **如实回 `success:false`**，而 AI 侧双参数/分相下发**静默 no-op**（降级口径不对称，待裁）；**⑥** **新增 §13.13**：T1–T12 逐 Task 的提交号、结果与**评审轮次的如实登记**（**T3 / T9 的质量评审为"单代理两阶段"、T10 的质量评审"并入规格评审"** ⇒ 均**只算一轮独立评审**；T12 未过独立评审），并列出未结项（写审计事件等）指向 `technical-debt.md` §6.13；**⑦** 实施计划片段随各 Task 的多处订正（Task 5 的 `PcsResponse` 字段数 / FC16 语义 / `tokio-serial` 门控；Task 6/7 的 latch 检查顺序与计划片段）已在计划侧回写。**未改**：§13.1–§13.12 的任何结论与判据、§1–§12 正文、任何 PRD 与其它模块文档、任何代码/配置；**既有门禁标记原文与覆盖范围不变**（**均不含 §13**）。**本增量未经设计复审**（勘误 + 实现状态登记类，不构成新契约）。 |
| **v1.14（2026-09-26，**新增 §13「PCS 控制面归属」；**未获门禁标记、待设计评审**）** | **只追加 §13，§1–§12 正文零改动**（§11.8 正文一字未改，差异登记 Δ-15）。来源 = 用户 2026-09-26 对 **§11.8①**（"PCS 写归属须**单独评审**后再立项"）的裁定：**将 PCS（两级式 PCS = 实时控制模块）的通信与控制整体迁入南向**，`mupc-intercore` 收敛为**纯核间 TCP 帧协议**。要点：**① §11.8 四件事逐条结清**（§13.1）——裁定下放、`PcsHandle` 成 500 的**唯一写方**（S2 联锁经 `InterlockPort` 4 方法薄 trait 调用，**签名不变、约 1300 行状态机零改动**）、latch 权威源仍在 storage/DB（C-1 双 latch 不变）、新增写审计、④ 部分修订并登记 **Δ-15**（控制面与联锁面之间的**唯一一条**直连）；**② 三项 ADR**（§13.2）——**ADR-014** PCS 归属 southd、**ADR-015 取代 ADR-011**（Modbus 栈统一到 `rs485-plugin`、删 `tokio-modbus`；原理由"缺 FC16"实测不成立：`rs485-plugin` 具 FC02/03/04/06，FC16 全仓零使用点）、**ADR-016** 新增顶层段 `south_pcs` 且 `south_stations` 不再允许 `role: pcs`；**③ 三条读路径合并为一条采集循环**（§13.4）——现状心跳 1013 / `latest_soc` 1010 / `read_three_phase` 两段共 **≈4 次读/秒**、**在线判据还不一致**（心跳 3 拍判离线，`latest_soc` 因走 `read_input` 单次失败即离线）⇒ 合并为**每 `interval_ms` 读 `pcs_3zone` 块（1000..=1075，76 字）**一次同时产出 SOC/运行态/三相/全部 72 点，**1 次读/秒**、单一口径；按需读项改**快照**（`PcsSnapshot`），失败即快照失效；**④ 采集与控制共用同一把 `Mutex<()>`**（§13.3）⇒ 以更强方式满足 §11.8②；**⑤ `rs485-plugin` 三处配套**（§13.6）——`write_single_register_from`（读侧有 `_from`、写侧没有）、写响应补**回显校验**（现仅判 `len>=8`，停机写 500=0 被错帧应答**检测不出来**）、`#[cfg(test)]` 字节流工厂缝（否则 T-L0 的 E1–E6 无法搬迁）；**⑥ 618 点归属拆为 546 + 72**（§13.7，总数与逐点内容不变，`POINT_REGS` 零改动）；**⑦ 新增校验 P-1…P-4**（§13.8）并修补既有空档（现校验只比串口名、不认设备 ⇒ 换口即可绕过形成对同一台 PCS 的双 master）；**⑧ 零变化清单**（§13.9）；**⑨ 分期 T1–T4**（§13.10），**T3 装配换型必须是原子提交**（安全链唯一切换点，保证任一时刻 latch/S-4 只有一份实现）；**⑩ 差异 Δ-15/Δ-17/Δ-18**（§13.11），其中 **Δ-17（三相降级粒度由两段独立变为单块）为待评审裁定项**。**未改**：§1–§12、任何 PRD、其它模块文档、任何代码/配置；**既有门禁标记原文与覆盖范围不变**。 |
| **v1.13-r4（2026-09-24，**T9 实现实测证伪的合同勘误回写**；**未新增门禁标记**，`[DESIGN_APPROVED: 2026-09-23]` 的覆盖范围仍只到 §12）** | **只订正合同文本中与实测不符处**（依据 = T9 的 `6370041`/`9ecb51a` + 注入探针）：**①** §12.4.5 的 `judges_evaluable` 的 `meter_grid` 支路**漏 `p_total`**（与 PRD §10.3.2 的 `R(role)` 表不对称）⇒ 两侧对齐为 `p/q/pf/u/i/p_total`——**该不对称正是 T9 按评审建议新增的交叉断言 `runtime_guard_and_config_criterion_agree` 当场查出的**（此前不可达：MeterGrid 不进遥测/事件段）；**②** §12.8 的 `edge_memory_is_per_group` 构造**非判别性**（快组 1000 整除站周期 5000 ⇒ 承载组 prime 后同 tick 快组再 prime ⇒ 掩盖记忆被清空 ⇒ 把 tracker 键退回"按站存"该用例仍绿）⇒ 订正为**快/慢组 2000 / 5000**（注入探针实测：2000 时注入 ⇒ FAILED；1000 时 ⇒ ok）；**③** §12.8 的 S-3 锚承载组到期 tick `t=0/2000/4000/6000` 中的 **"…/6000" 不成立**（退避 `oc=2 ⇒ extra=2×组周期=4000` ⇒ `next_due` 推到 8000）⇒ 订正为 `t=0/2000/4000`，并登记"第三次 `fail_input_once` 属**滞后消费**（t=8000 才被吃掉），用例止于 t=6000 故无害、延长 tick 须注意"。**未改**：C1–C9 约束、AC-8-1…AC-8-7 判据、§12.4.1–§12.4.4 的伪码与不变量、任何数字口径、§1–§11、任何代码/配置/PRD/12 号设计；**既有门禁标记原文与覆盖范围不变**。 |
