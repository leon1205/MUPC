...
#### 11.2.6 消防事件（`fire → events`）的落点（P0-3 的方案探索）
...
#### 11.4.7.2 状态型事件的口径裁定（T5 代码评审升级为设计裁定）
...
#### 11.5.3 既有测试的连锁订正清单（**扩至「用例名 + 断言」级**；P0-4）
...
##### 11.5.3.1 订正规模（逐个数核实，取代此前的"≈13 处"）
...
##### 11.5.3.4.1 【C2 与规则 10 的冲突：实现判定顺序约束】（评审「须补 2」）
