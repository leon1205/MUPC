...
| **Δ-20** | **P-4 的覆盖面**（§13.8）：原文声称 `south_pcs.regs` 复用 "`points::expand` 同一函数、空洞 ≤ 4、单块 ≤ 120 寄存器" | ★**实测两重不实**，已在 §13.8 的 P-4 行就地订正：① 空洞（规则 11）**不在** `expand`；② "单块 ≤ 120 寄存器"**在全仓不是任何一条独立规则**。**且最尖的一条是规则 5**（漏写 `scale` ⇒ serde 缺省 `0.0` ⇒ 整块 `raw × 0` **静默全 0**）。处置：实现侧**显式调用**块级校验函数（5/11/12/14/19，签名与 role 无关、可直接复用）；规则 15 与 role 相关的 6/13 **明确不复用并如实登记** |
