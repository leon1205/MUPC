...
| **P-4** | `south_pcs.regs` 复用既有 `RegBlockConf` 全部校验（`points::expand` 同一函数、空洞 ≤ 4、单块 ≤ 120 寄存器…） | 采集与配置期**同一函数**（§11.4.3 既有口径），不另起一套 |
...
| 上云点表 `uplink::build_uplink_points`（IEC104/MQTT 通道掩码） | **无** |
