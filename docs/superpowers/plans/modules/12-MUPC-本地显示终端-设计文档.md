...
> | **v2.0-r4（补注）** | 2026-09-15 | 维持 `[DESIGN_APPROVED]` | **实施期实测收口 R-24**（附录 **A.13**）：`LV_MEM_SIZE` 由"256 KB 未压测"定稿 **1 MB**（实测 256 KB/512 KB 均不足、1 MB 通过），并把「6 页共存」不变量上锁。**本节只闭合本文档自身延后到实施期的待测项（§10 / §14 R-24 原文即要求实施期实测后定稿），未改动任何已批准条目**（视觉规范 / 分区 / 控件语义与强确认 / 双通道 / 6 页 IA / 16 出口迁移表均未动）；**不自行盖章**——如评审认为需确认，列入下一次评审批即可 |
> | **v2.0-r4（补注 3）** | 2026-09-16 | 维持 `[DESIGN_APPROVED]` | **B3 收尾后的 PM 裁定批次（UI 文档附录 A.8）落到本文档的口径确认**：EDGE-03 与 EDGE-20 **同一触发**（读通道断）⇒ 整屏降级遮罩**可穿透输入**；「秒级恢复倒计时」改口径为**已断开时长**；**逐字段冻结标记**列为后续单元 **B3-2c**、**薄层能力缺口（样式读回/事件冒泡/滚动位置/rotation/mem_monitor）**列为后续单元 **B4**。**视觉规范值与已批准结论一律未变**；**不自行盖章**。 |
> | **v2.0-r4（补注 2）** | 2026-09-15 | 维持 `[DESIGN_APPROVED]` | **PM 裁定批次（UI 文档附录 A.7）落到本文档的 5 处口径同步**：§6.2 的**确认强度分级**行、**保存**行、**恢复默认值**行三处的 `requires_reconnect` 判据由「任一字段（视图口径）」改为「**本次改动涉及**」（理由：`requires_reconnect` 字段恒在视图内 ⇒ 视图口径下 L1 永不可达、L2/L2+ 之分失效，且只改日志级别却弹「链路瞬断」属谎报副作用）；**恢复默认值明细**的「当前值」明确取**后端注入真值**（非法值落占位符 `–`）；§6.3 **P3 列表行**标注**斑马纹仅 P3**、消息列**单行截断 + `…`**、「回到最新」改为**右下恒显**。**视觉规范值与已批准结论一律未变**（L1/L2/L2+ 三级、行高 44、列宽口径、斑马纹色值均未动）；**不自行盖章**——如评审认为需确认，列入下一次评审批即可 |
> | **v2.0-r4（补注 4）** | 2026-09-16 | 维持 `[DESIGN_APPROVED]` | **CF-04 降级获 PM 裁定**：§4.3.5 的降级方案（HotApply 子集 + 连接类参数只落盘 + 提示需重启）**已裁定——接受**（不投入"把 6 个字段做成真热生效"的改造）；实测计数 **9 / 7 / 1 / 6** 已写入 §4.3.5，PRD（CF-04 降级）与 UI 设计文档相关文案同步。**本文档的已批准条目与数值一律未改**（本节只补"已裁定"结论 + 回写落点 + 残余登记）；**不自行盖章**。 |
>
...
6. **`publish_ms ≤ 4000`**（**B3-2d 裁定 · PM**，契约 `MAX_PUBLISH_MS` = 4000）——`publish_ms` 是本表**主拍路径**的周期项：主拍一旦慢于最慢的一段采集（= 约束 2 的 4000），本表的拆解即失效（瓶颈只剩主拍自己）。该上界对"更慢的主拍"**没有验收意义**，它的作用是把 `min = publish = 60_000` 这类**退化组合**（端到端 ≈61 s，远超验收 ≤2 s）**挡在启动期**，而不是留到现场用手感发现。取 4000 与约束 2 **同量级**（两条各自独立、服务不同指标：本条服务本节 ≤2 s，约束 2 服务 F6.3 ≤5 s）。
...
- 两地址**不得相同**；
- `publish_ms >= 100`（**上界 4000 见下条「时延红线」**，B3-2d）、`live_ring >= 100`；
- **时延红线（§4.2.1 约束 1/2/5/6，越界即启动报错，不解释为"调优空间"）**：
  ...
  - `publish_ms <= 4000`（**B3-2d 裁定**，契约 `MAX_PUBLISH_MS`；§4.2.1 约束 6 的落地——主拍慢于最慢一段采集即无验收意义，该上界用于把 `min = publish = 60_000` 这类退化组合挡在启动期）；
  - `250 <= min_publish_interval_ms <= publish_ms`（合并窗口下界防抖动打爆通道，上界防退化为纯主拍）。
    > ⚠️ **【A-2 裁定 · 2026-09-17】** 本节原写 `100 <= …`，与 **§4.2.1 约束 2**「`min_publish_interval_ms ≥ 250 ms`」**自相矛盾**（同一份设计里两节打架）。**以契约为唯一真源**：契约 `display-proto/src/config.rs::MIN_MERGE_WINDOW_MS` 已由 `100` 抬到 **250**，本节与 §11.1 的这一处已就地订正为 250。（原文保留，不改写历史）**当时登记的遗留（未关）**：`publish_ms` **仍无上界** ⇒ `min = publish = 60000` 这类配置照样过校验（端到端 ≈61 s，超 §4.2.1 的 ≤2 s 前提）——要关掉它需给 `publish_ms` 加一条上界，设计尚未给数值 ⇒ **登记为设计缺项**，当时不擅自定值。
    > ✅ **【B3-2d 裁定 · 2026-09-18 —— 上述遗留已关闭】** **PM 裁定 `publish_ms` 上界 = 4000**：契约新增 `display-proto/src/config.rs::MAX_PUBLISH_MS = 4000` 并在 `validate()` 里拒判（越界报 `display.publish_ms`），设计 **§4.2.1 新增约束 6**、本节上一条同步补 `publish_ms <= 4000`、§11.1 的回归清单同批补入。**该"设计缺项"至此不再是缺项**（技术债 U-45 据此结案）。取值依据：主拍慢于**最慢的一段采集**（`device_poll_ms` ≤4000）时 §4.2.1 的拆解表失效 ⇒ 该上界对"更慢的主拍"没有验收意义，它的作用是把 `min = publish = 60_000` 这类**退化组合**（端到端 ≈61 s，验收 ≤2 s）**挡在启动期**。
...
| **确认强度分级**（吸收 UI §2.5 / §7.3） | L0 无确认（浏览/筛选/切页）/ **L1 双步确认**（默认焦点「取消」）/ **L2 双步确认 + 长按 1.0 s**（危险色 `#FF6B6B`）/ **L2+ 追加 `WarnBanner`**（本次改动涉及的字段中任一 `requires_reconnect == true`）。落点为 `ConfirmDialog` 的 `level` 参数，**调用方必须显式传入（无默认值，防漏配）**；各页映射见 §6.2（保存 = L1 或 L2+、恢复默认值 = L2）与 §6.4（释放 / M1 授权 = L2） |
...
| **时延拆解回归**（F6.3 / F7.3 / F16.5） | 断言 §4.2.1 的落地约束全部成立：`alarm_poll_ms ≤1000`、`interlock_poll_ms ≤1000`、`device_poll_ms ≤4000`、`publish_ms ∈ [100, 4000]`（**B3-2d 裁定：上界 = 契约 `MAX_PUBLISH_MS` = 4000**，对应 §4.2.1 约束 6；成对边界用例 `publish_ms_upper_bound_4000_paired_boundary`：**4000 过 / 4001 拒**）、`min_publish_interval_ms ∈ [250, publish_ms]`（**A-2 裁定：以契约 `MIN_MERGE_WINDOW_MS` = 250 为准**，本节原文 `[100, …]` 与 §4.2.1 约束 2 冲突，已订正）；并断言**慢拍写缓存后被唤醒组帧**（假时钟下「写入 → 发布」间隔 < 合并窗口上限），防退化回纯 1 Hz 主拍（该退化会使 F7.3/F16.5 变为 2.1 s，超差） | `display_host` 单测（假时钟）+ 配置校验单测 | 全平台 |
...
| **R-24** | **`LV_MEM_SIZE`：已定稿 1 MB（实施期实测，2026-09-15）**；**`lv_mem_monitor_t` 真机堆占用实测仍待** | **真机** | **低**（原「中」）：口径已由"未压测"收敛为「实测区间 + 上锁不变量」 | A.13 实测：**256 KB 只够 P1+P6+P2 三页**（建第 4 页 P4 即 `lv_realloc` 失败）→ **512 KB 亦不足** → **1 MB 通过** ⇒ 取 1 MB（1 MB 是**实测通过值**、非最小理论值）。不变量由 `ui/tests.rs::pages_chain` 的「6 页共存」用例锁住（**再调小或某页膨胀即红**）。**残余**：本机 `lvgl-sys/allowlist.txt` **无 `lv_mem_monitor`** ⇒ 未读到 `free / max_used / frag`，故 **1 MB 的余量倍数未量化**；真机可用 `ps`/`smaps` 复核 BSS 占用 |
...
### A.13 实施期实测收口：`LV_MEM_SIZE` 定稿 1 MB（2026-09-15）

> **性质**：本节**不是设计变更**，而是**闭合本文档自己延后到实施期的待测项**（§10 / §14 **R-24** 原文即"实施期须 … 实测后定稿"）。视觉规范、分区、控件语义与强确认、双通道、6 页 IA **均未改动**。

**① 触发事实（外壳的硬前提）**：B2c-3 应用外壳要求**同时持有 6 页**（P1–P6，靠显隐切换；六页已分别由 B2a / B2c-1 / B2c-2 交付）。而 `LV_MEM_SIZE = 256 KB` 下**实测只够 P1+P6+P2 三页**：按 `P1→P6→P2→P4→P5→P3` 顺序构建时，**建第 4 页（P4）即 `lv_realloc: couldn't reallocate memory`（`lv_mem.c:162`）**，紧接着 `lv_array_resize` 断言失败（`lv_array.c:177`）。
> ⚠️ **其失败形态是「挂死」而非「变红」**（测试被超时杀掉，`exit 143`）—— 故这方面的探针**一律须加 `timeout`**。

**② 实测区间（本次标定）**：

| `LV_MEM_SIZE` | 结果 |
|---|---|
| 256 KB | ❌ 只够 3 页（建第 4 页 P4 失败） |
| **512 KB** | ❌ **仍不足**（0.695 s 即 `lv_realloc` 失败 + `lv_array_resize` 断言 ⇒ 挂死被超时杀掉） |
| **1 MB** | ✅ **通过**（279 lib + 7 集成 + 2 doc 全绿，含「6 页共存」用例） |

⇒ **需求区间 = (512 KB, 1 MB]，取 1 MB**。**1 MB 是「实测通过值」，不是「最小理论值」**（余量倍数未量化，见 ④）。

**③ 为什么调大是安全的（不是"用空间换功能"）**：`LV_USE_STDLIB_MALLOC = LV_STDLIB_BUILTIN` ⇒ 该池**只装 LVGL 对象树 / 样式 / 定时器**；**绘制缓冲走系统堆**（`src/lvgl/display.rs` 用 `std::alloc::alloc_zeroed` 分配，PARTIAL 双缓冲约 628 KB），**两者不竞争** ⇒ 调大本值**只增加 BSS**（RK3588 上 1 MB 可忽略），**不牺牲任何功能**：不丢页内滚动位置、不丢 P2 未保存草稿（EDGE-11）、不影响「任意页 1 次触摸」与页切换 ≤300 ms（§7.1）。相比"懒建 + 离页销毁"的方案，本方案**代价最低且不留功能缺口**。

**④ 上锁与残余**：
- **不变量已上锁**：`src/ui/tests.rs::pages_chain` 的「**6 页共存**」用例 —— 按 `P1→P6→P2→P4→P5→P3` 顺序建六页（**该顺序有意复现"第 4 页失败"的原始序列，不得改**）、**每页建完即验「存活 ∧ 子对象数 > 0」**（不是只验"构造没报错"——本项目出过"局部句柄被 drop ⇒ 屏上空白而测试仍绿"的缺陷类）、六页同屏渲染一次、析构后断言宿主仍存活。**再调小 `LV_MEM_SIZE`、或某页常驻对象膨胀，该用例即红**。
- **残余（真机项）**：本机 `lvgl-sys/allowlist.txt` **未收录 `lv_mem_monitor`** ⇒ 未读到 `free_size / max_used / frag_pct`，**1 MB 的余量倍数未量化**；真机可用 `ps` / `smaps` 复核 BSS 占用，必要时再调。**不得据此标注"内存预算已验证"**。
- **一处实施期环境事实（与本节结论无关，仅防误判）**：本机**全量重编 LVGL C（约 409 个 `.c`）时出现过一次 `cl.exe` 静默失败（exit 2、无 stderr）**，**重试即通过** ⇒ 属**低内存机器上的并发编译资源竞争**，非代码 / 配置问题。

**⑤ 影响面**：仅 `lvgl-sys/lv_conf.h` **一行**（`LV_MEM_SIZE` 256 KB → 1 MB）+ 一条新增回归用例；**无 API 变化、无调用方改动**。B2c-3 外壳可据「6 页常驻」直接实施（**无需懒建、无需离页销毁**）。

---

*（End of Design **v2.0-r4（LVGL v9.5.0 API 事实订正）** — ✅ **`[DESIGN_APPROVED: 2026-09-11, 设计评审员]`（第四轮复评 · 增量 · 终审通过）**；前序「第三轮复评 · 终审通过」标记曾因 r4 修订失效、现经复评恢复（v2.0-r4 的 `[PENDING_REVIEW]` 行历史保留）；r1 评审整改见附录 A.4，Slint→LVGL 变更决策见附录 A.5，二次整改见附录 A.6，LVGL spike 实测整改见附录 A.7，**v2.0-r3 复评问题清单见附录 A.8**，**P-1~P-5 整改见附录 A.9**，**第二轮复评残余问题见附录 A.10**，**P-6~P-8 整改见附录 A.11**（含**复评结论**），**v9.5.0 API 事实订正见附录 A.12**，**实施期实测收口（`LV_MEM_SIZE` 定稿 1 MB）见附录 A.13**）*
