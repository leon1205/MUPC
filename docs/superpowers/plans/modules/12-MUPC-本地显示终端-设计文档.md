...
> ✅ **`[DESIGN_APPROVED: 2026-09-23, 设计评审员]`**（**仅对 §15 增量有效**；文首 v2.0 标记原文未动、亦不覆盖 §15）—— 2026-09-23 追加 **§15 外设数值上屏（U-73 增量）**，对应 PRD v2.2 的 **§3.9 F20–F26**（`[REVIEWED: PASS: 2026-09-23]`）与 **T-8 裁定（两处拆）**。**该增量已于 2026-09-23（三审 / v2.1-r3 末轮）通过，对应实现可启动**；文首 `[DESIGN_APPROVED: 2026-09-11]` 与本增量**互不覆盖**。**§1–§14 的正文、门禁标记与 §14 条目一律未改。**
>
...
> - 关联 UI 设计：[`12-MUPC-本地显示终端-UI设计文档.md`](12-MUPC-本地显示终端-UI设计文档.md)——视觉与交互规范的权威。**分工**：本文件给**功能与结构**（分区顺序 / 控件类型 / 常量断言），UI 文档给**版面几何**（矩形 / 间距 / 字号 / 行高）。本增量按 **R-36** 的裁定**追加了 UI 文档的 §6.4.1 / §6.6.1**（该追加节**不在** UI 文档 `[DESIGN_APPROVED: 2026-09-10]` 的覆盖范围内，**已于 2026-09-23 加 `[DESIGN_APPROVED: 2026-09-23, UI 评审员]` 增量标记**），**UI 文档 v2.0 正文未改**
> - 权威点表：协议 V1.3（EMS）3 区只读点表（FC04）；`REG_SOC=1010` / `REG_RUN_STATE=1013` 在 `mupc/crates/mupc-southd/src/pcs/regs.rs:20` / `:22`，`REG_I_A=1022` / `REG_P_A=1029` / `REG_P_TOTAL=1032` 在**同一文件**（`:30` / `:31` / `:32`，**已是公有常量**）；三相读在 `mupc/crates/mupc-southd/src/pcs/collect.rs`（`read_three_phase`）。⚠️ **2026-09-26 改注**：原指向 `mupc/crates/intercore/src/pcs.rs:20/22` 与 `mupc/crates/intercore/src/transport/modbus.rs:170-171`（当时后者为文件私有常量）——该两文件已随 **PCS 通信与控制整体迁入南向**（02 号设计 §13 / ADR-014）**删除**，现址如上
...
- **码表**：真源为 **UI 设计 §3.6 的全屏用字表**，由 `fonts/extract_charset.py` **自动提取**（不做猜测；脚本内置**漂移断言**：表末声明的每个字形/字串——`MUPC`/`ERROR`/`kW`/`0–9`/`Σ`/`⚠`/`▲` 等——必须真的出现在 §3.6 正文，否则**直接报错退出**；`🔒` 按 UI 文档"以几何锁形替代"**不纳入**）。实测产出：**271 汉字 + 32 拉丁/数字 + 35 符号几何 = 326 字符**（`font_subset_charset.txt`，901 B）。漏字即屏上出现豆腐块，故 §11.1 保留**码表覆盖率测试**。
  ...
  > ⚠️ **口径订正（2026-09-25）**：**上句的「326 字符」是本节所有字体体积实测的（历史）前提，不得与本行之后的码表规模并排比较。** U-73 外设上屏引入短标签白名单 + `ui_text` 后，码表已扩至 **464 字符**（`font_subset_charset.txt`；10 档 cmap 交集 **462** 码位，缺口 2 = `✕` U+2715 / `❚` U+275A——字库源无字形、生产未用）。⇒ **本节（§12.3）下列所有体积/字节数（`1,776,080 B` / `131,793 B` / `830,590 B` / `136,892 B` 等）与「10 档位图合计 1.69 MB」均为「**326 字符**」口径下的单机实测，**未按 464 字符重测**。按线性外推的新口径见 **§15.6.3**（10 档位图 **≈1.7–2.7 MB**，磁盘预算 ≤100 MB 不破）。**真机/构建机重跑 `gen_fonts.sh` 后应以新实测值替换本节数字**（本行只做口径标注，**不伪造未测数字**）。
...
> ⚠️ **本节落地现状（2026-09-26，如实登记）**：环的**生产侧**已有三个投递方（`SouthSink` 系统事件、安全联锁、**03 设计 §9.3 缺口 1 的丢弃告警巡检**——后者投 `FeedItem.subtype == "major"`）；但 `AlertFeed::subscribe()` 本期**没有任何生产消费者**（函数带 `#[allow(dead_code)]`）。⇒ **FLS-03② 的 `major` 丢弃告警"投得进环、但生产侧无消费者"**，与本节"可选增强、非 F7 真源"的定性一致 —— **不得**把它读成"现场可观测告警"（可观测性仍以 `storage.events` + 计数读口为准）。
...
> ✅ **`[DESIGN_APPROVED: 2026-09-23, 设计评审员]`**（**本章门禁**；§1–§14 的 `[DESIGN_APPROVED: 2026-09-11, 设计评审员]` **原文未动**，两标记**互不覆盖**）
>
> **三审（末轮 v2.1-r3）结论：通过。** 上一轮 5 严重 + 6 中等**逐条返工经独立复算 + 源码取证核实属实**：LVGL `pad_column` 段间距 API（`lv_buttonmatrix.c:1024-1029` / `:1059` / `:1067-1068`）与命中区外扩（`:896`，注释 `:894`；上限 `BTN_EXTRA_CLICK_AREA_MAX = LV_DPI_DEF(130)/10 = 13 px`）⇒ `pad_column = 16` 净距 **−2 px**、`≤24` 恒 −2 px、**须 ≥42 px**；R-42 排除侧依据 = **PRD §7 #11（`:1042`）**，EX-23（`:1279`）/ F24 验收 2（`:773`）确为「不得**被标注为**」的**误用禁止**；权威点表 `REG_I_A=1022` / `REG_P_A=1029` 在 **`mupc/crates/mupc-southd/src/pcs/regs.rs:30` / `:31`**（**公有**），`REG_SOC=1010` / `REG_RUN_STATE=1013` 在 `mupc/crates/mupc-southd/src/pcs/regs.rs:20` / `:22`（⚠️ **2026-09-26 改注**：原指向 `intercore/src/transport/modbus.rs:170-171` 与 `intercore/src/pcs.rs:20/22`，两文件已随 PCS 迁入南向**删除**，现址如上；三相读现址 = `mupc-southd/src/pcs/collect.rs`）；容量表四格（**20.6 / 22.2 / 36.1 / 37.7 KiB**）与上界四格（**30,736 / 53,776 / 32,379 / 55,419 B**）逐格复算相符；反证下界 **63.96 KiB**、整帧 **87.2 / 102.7 KiB**、catalog **64–188 KiB**；`k_max = 111`；口径 A **1.35 s** / 口径 B **6.35 s**；§15.7.3 去重 **31**；P6 **428 + 装置段 11** 行。**4 项文字级订正**（§15.2.2 的 `63 %`→`59 %`、§15.4 的 `384 − ?` 残句与「x 区间不相交」、`style_seg_*` 命名、F-3 的 525 条口径）**不阻塞编码**，见评审报告。
...
>
> **门禁声明**：本节产物**不受**文首 `[DESIGN_APPROVED: 2026-09-11]` 覆盖（该标记只对 §1–§14 有效）；**本节已于 2026-09-23（三审）通过**（见本章门禁标记），**编码可启动**。
>
...
| N-15 | `south_stations.pcs` **整段注释**；`PcsHandle` 当前**仅**读 1010 / 1013 / 1022–1024 / 1029–1032 | `production.yaml:236-284`（注释段）；`mupc/crates/mupc-southd/src/pcs/regs.rs:30-32`（**公有**常量 `REG_I_A=1022`（`:30`）/ `REG_P_A=1029`（`:31`）/ `REG_P_TOTAL=1032`（`:32`），同属一路 FC04 3 区读）；`mupc/crates/mupc-southd/src/pcs/regs.rs:20` / `:22`（`REG_SOC=1010` / `REG_RUN_STATE=1013`）。⚠️ **2026-09-26 改注**：原指向 `mupc/crates/intercore/src/transport/modbus.rs:170-172` 与 `mupc/crates/intercore/src/pcs.rs:20/22`（且读取方写 `intercore`）——该两文件已随 **PCS 迁入南向**（02 号设计 §13 / ADR-014）**删除**，寄存器表现址 = `mupc-southd/src/pcs/regs.rs`、三相读现址 = `mupc-southd/src/pcs/collect.rs`、读取方改为 `mupc-southd::pcs::PcsHandle` | F24 真源 = **PcsHandle（南向）**⇒ **写入方缺口**（§15.1.3 报告项 R-28 / R-29） |
...
pub struct PeripheralsSection {
    pub ts_ms: u64,        // 本段**重建**时刻（Unix ms；0 = 未采集）
...
> **已落地（T21c-3-r1，2026-09-25）**：
>
> - **顶部提示 + 「重试」**：两页 API = `P4InterlockPage::set_catalog_stale(bool)` / `P4InterlockPage::set_on_catalog_retry(cb)` 与 `P6SystemPage::set_catalog_stale(bool)` / `P6SystemPage::set_on_catalog_retry(cb)`。**落点**：P4 = 滚动区顶部（安全总览带之下）、P6 = **段内容区最顶部**（分段页签之下，页级单件）；文案 / 组件零新增（`ui_text::CATALOG_STALE` + `ui_text::RETRY`；`WarnBanner` 928×56 非交互行 + `TextButton` 48×48，净距 16）。**占位**：只在 stale 时显（默认不显、不占位）；stale 时滚动视口整体下移 72 并等量变矮（**底缘不动**、不遮住既有内容），解除后逐像素复原（几何单一真源 = **UI §6.4.1 / §6.6.1**）。
> - **接线**（`app.rs`）：`RouteDecision::PeripheralCatalog`（成功）⇒ `on_catalog_read_ok()`（清预置 / 清失败抑制 + 两页 `set_catalog_stale(false)`）；catalog 的两条失败入口（传输 / 非 2xx / 超时，以及回执形态 / 解码）⇒ `on_catalog_read_failed()`（两页 `set_catalog_stale(true)`，**不调 `clear_catalog()`** ⇒ 保留旧 catalog）。「重试」= `ControlIntent::CatalogRetry`（**回调只投意图**）⇒ 复位 `periph_page_entered` 与失败 rev 抑制 ⇒ 下一拍由 `catalog_due` 重新发起**一次**。
> - **W-1 收口**（评审 2026-09-25 登记项）：发起时的预置 `catalog_rev` 在回执**失败**时**回滚**为失败前的值（`App::catalog_preset`），并把该帧内 rev 记进 `App::catalog_failed_rev` 作"同 rev 不自动重发"抑制（`catalog_due` 的 `failed_rev` 入参）—— 既让"帧内 rev 变化 ⇒ 自动重取"重新有效，又**不会**因回滚造成每拍反复发起。判据与用例：`control_route::catalog_due` + `control_route::tests::catalog_failure_suppresses_the_same_rev_and_retry_forces_exactly_one_more_fetch`。
...
pub struct BitMeta {
    ...
    ///
    /// **T21c-2-r1（F3；产品裁定 2026-09-25）**：与 `active_text` 成**一对** —— 两态词
    /// **都由 catalog 承载**（"屏侧不猜语义"，D22），需要双态词的位须把两侧给全。
    /// **首例 = `hvac_di_8`（系统运行）**：活跃「运行」/ 非活跃「停止」（PRD **EX-06**：
    /// 「停止 / 运行」文字 + 语义色 + 图标三重冗余）。只给 `active_text` 的位，非活跃侧
    /// **回退**通用「非活跃」（与本字段引入前的行为逐字相同）。
    ///
    /// **兼容性**：`#[serde(default)]`（`Option` 缺键亦解析为 `None`）⇒ 既有 catalog
    /// （无本字段）反序列化得到 `None`、不解析失败；`display-proto` 的 DTO 契约用例含
    /// 一条"删掉该键仍能解析且其余字段逐值不变"的对照断言。
    #[serde(default)]
...
| 点 | `bits` 内容 | 依据 |
|----|-------------|------|
| `hvac_di_{at}`（离散位，每点 1 位） | `[{ index: at−1, label: <登记 label>, class: <BitClass 映射>, defined: class != Reserved, active_text: …, inactive_text: …, inverted: false }]` —— 两态词**只对 `hvac_di_8`（系统运行）登记**（`运行` / `停止`，EX-06）；其余位两侧俱 `None` ⇒ 屏侧显通用「活跃 / 非活跃」（**不臆造**，见 §15.5.2 的 `hvac_run` 行） | `point_table.rs:833-863`（`BitClass::Alarm` 20 条 / `State` 10 条 / `Reserved` 1 条，位 25 保留）；两态词登记 = `console_host.rs::bit_state_words`（文案取自 `display-proto::ui_text`） |
...
│    │ 「已联锁 / 未联锁 / 不可用」96 px（※见下）│  │ 枚举文案 96 px + 图标   │
│    └──────────────────────────────────────────┘  └────────────────────────┘
...
> **⚠️ 「既有联锁区…一律不动」的三条例外（T21c-1 / `IL29`；产品裁定 2026-09-25）** —— 本节与 UI §6.4.1 的两句「不变」声明，在 **488 px 卡宽**下**数学不可同时成立**：卡内容区 **454 px** < 既有通道之和（标题 112 + 主值图标 72 + 96 px 主值词 + latch 胶囊 236 + 文字冻结角标 192；单是「标题 + 角标 + 胶囊」= **540**；评审 **G-13** 即该冲突）。逐条处置（实现侧的逐条登记见 `mupc/crates/local-display/src/ui/pages/p4_interlock.rs` 的 **IL29①–⑦**）：
> 1. **不可用态的总态词缩短为「不可用」**（IL29①）：「联锁状态不可用」7 字 × 96 px = **672 px** > 该槽 **366 px**（= 454 − 72 − 16）⇒ `DOTS` 下只剩约 2 字 + `…`。**完整语义不减**：由**卡标题「联锁状态」+ 就地原因带全串（992 px）+ 三态色 / 图标冗余**三重承担（`available = false` ⇒ 右槽不显 ⇒ 左槽扩到 992 并置全串）；两条常态词「已联锁 / 未联锁」（各 288 px ≤ 366）**逐字不变**。
> 2. **EDGE-03 / EDGE-20 的区块角标改「图标-only」形态**（IL29②）：**要求本身保留**（§8.3「每区块打标」对 P4 仍成立、可见性判据仍是 `frame_mark` 单一真源），只把 192 px 文字胶囊换成 **28 px 图标徽标**（`⚠` = 冻结 / `!` = 数据过期，与 P1 的两个角标同源），落在标题与 latch 胶囊之间的空槽（112 < 174 < 202 < 218 ⇒ 三件互不重叠）。**代价（如实）**：失去「冻结 / 数据过期」的**文字**区分，改由图标通道承担。
> 3. **栅格 / 行高偏差**（IL29③⑤）：单卡外缘高实现 **162**（UI 写 164）、滚动视口 **342**（UI 写 340）—— root cause = `theme` 缺 2 px 档，实现坚持「不写裸规格值」；§A 四组行高统一取 `ROW_LOG_H`(44)（UI 写 36 / 40）。**关键不变量不破**：视口 342 ≥ 340，且 §B 既有全量 `202 + 16 + 106 = 324 ≤ 342` ⇒ 既有联锁区仍**首屏全可见**（UI §6.4.1 的核心承诺）。
>
> **同步回写**：**12-UI 设计文档** §6.4.1（三条例外 + 表头位名 + 「收起」纵向实测）与 §8.3（角标形态）。
...
| `fire_sys_status` | 消防系统状态 | `fire_sys_1`（整字位图） | — | — | 24 px（标注） | **逐位 16 行**：已定义 6 位（主电故障 / 备电故障 / 驱动电路 / 压力传感器 / 电磁阀 / 喷洒标记）显「位名 + 活跃 / 非活跃」；**未定义位显「未定义位 n」**，不猜语义（F21.1 / EX-09）。⚠️ **段级 / 站级降级时通告另占一行**（卡头之下 **段顶状态条**，§15.6.2 ① 的口径）—— **位行恒 16 且恒从 bit0 起**，通告**不占**位行（T21c-1 订正，评审 W-3②：此前通告占用第 0 行 ⇒ bit0 语义在本态不可判读） |
...
**下钻明细的行区窗口化（T21e，2026-09-26 已落地）**：`fire_detector` 的**分页表行区**按 §15.5.3 的窗口化要求实现 —— 行池 = **可视行 ×1.5 + 1**（`DRILL_ROWS_H / DRILL_ROW_H` = 4 ⇒ **7 行**，范式与 `BmsDrill` 同源），按 `SCROLL` 重绑到页内行 `[start, start + 7)`；行区另有**高度占位器**使滚动范围覆盖**整页**（页尾行可滚到）。**「对象数与页行数无关」由此成立**：`page_size` = 20（缺省）或 50（上限）时行区对象数**相同**（`ui/tests.rs` 实测：注入 50 行页新建 LVGL 对象 = 0 件）。`page_size = 20` 的**分页契约本身不变**（本句只关行区渲染方式）。⇒ 对象预算随之由 1031 降至 **922**（§15.9 **R-34**）。
...
| **新增只读控件（下钻「收起」y260–308）↔ 危险按钮** | `Dimens::GAP_DANGER` | **≥48 px** | 本表按**线框** y 复算 = 纵向 **316 px** ⇒ ✓；**实现实测 = 322 px**（视口顶在实现里是页内 y186 而非线框 y260 ⇒ 收起底 234 → 危险按钮顶 556；T21c-1-r1 已加断言，见 `ui/tests.rs` 的 T-25 段）|
...
| 行类型 | 呈现 | 字号 | 降级 |
|--------|------|------|------|
| 站点状态条（每段顶部） | 「站 `<role 中文名>` │ 在线 / **站离线** │ **成功** 12:03:44 │ **更新** 12:03:46」（**四列**；标签取**缩短形态**，见下注） | 站名 26 px / 其余 24 px | 离线 ⇒ 状态条警示色；**段内全部行** `--` +「站离线」 |
...
> **站点状态条的标签缩短（T21c-2-r1 / F2；产品裁定 2026-09-25）**：**四列口径不变**（UI §6.6.1
> 原文的「最后成功 … │ 最近更新 …」两列**仍是两列**），**缩短的是标签** —— 由「最后成功」/
> 「最近更新」改为 **「成功」/「更新」**（`ui_text::LAST_OK_SHORT` / `LAST_UPDATE_SHORT`），
> 值仍是 **`hh:mm:ss`**。**理由（量化）**：`last_ok_ms` 为**真值**时合并串
> 「最后成功 12:03:44 · 最近更新 12:03:46」= **424 px**（实测 `adv_w` 求和），而段「装置」
> 表的右槽只有 **398 px** ⇒ `DOTS` **必截断**（R-38 落地即现）；拆成四列后每个时刻列只需
> **146 px**（槽 165 px）。**段「装置」的 5 行表与各段顶同款**（同一条 `PoolRow::bind`）。
> 版面断言（列不重叠 + 不越槽，含 `23:59:59` 的上界形态）见 `ui/tests.rs` 的 `pages_chain` ⑰。
...
| 分组键 | 分组标题 | 内容 |
|--------|----------|------|
| `station_status` | 外设站状态 | 5 行站状态条（`hvac` / `fire` / `bms` / `meter_batt` / `pcs`）：站 id │ role 中文名 │ **在线 / 站离线 / 未启用** │ **最后成功时刻** │ 最近更新（F25.4 的**结构化字段**，非告警文本；EX-28）。**字段来源**：在线 ← `latest_values::station_is_active()`（**已存在**）；最后成功时刻 ← `station_last_poll_ms()`（**新增接口要求 R-38**；未落地 ⇒ 该列显 `--`，**不臆造、不由本侧自维护**）；最近更新 ← 各块 `ts_ms`。**版面**：行高 48、字号 24 px、同一行四段（UI §6.6.1 —— **四列**：站名 / 状态 / **成功** / **更新**，后两列的标签取**缩短形态**，见 §15.5.1 的注）；**行集合由 catalog 驱动**（`CatalogStation.enabled`，= **5 个 role 全集**），**行数与在线状态无关**（离线行**不消失、不折叠**，F25.5）。**缺席站（`enabled=false`）⇒ 该行显「未启用」**——与「**站离线**」**互异**（前者 = 该 role **未配置**、无任何数据可谈；后者 = 已配置但**当前**不可达，§15.6.2 情形 ⑦）；该站**各段**（空调 / 电池 / 储能表 / PCS）显「**站点未启用**」（**不是**「外设数据不可用」，那是整段源不可得，§15.6.2 情形 ⑤） |
...
| `hvac_run` | 运行状态 | 系统运行 | `hvac_di_8`（位 7） | — | — （「停止 / 运行」文字 + 语义色 + 图标，EX-06）。**两态词由 catalog 承载**（T21c-2-r1 / F3，产品裁定 2026-09-25）：`BitMeta.active_text = 运行` / `inactive_text = 停止`（生产者 = `console_host.rs::bit_state_words`，文案取自 `display-proto::ui_text` 的 `ENUM_RUNNING` / `ENUM_STOPPED`）；屏侧 `state::bit_text` 逐位取用，**缺 `inactive_text` 时回退**通用「非活跃」 |
...
**P6 容器策略（承 §5.4 的 `lv_tabview` / 自建二选一）**：段内容采用 **`SegmentedTabs`（Flex + `lv_button` 单选组，见 §15.5.1）+ 各段独立滚动容器**（**不用 `lv_buttonmatrix`**，理由 = F14.2 的 16 px 触摸目标间距，见 §15.5.1 ②）；**分段控件惰性创建、切换仅改可见性**（不销毁重建，避免 LVGL 内存池碎片）。**含数据行驱动的段一律窗口化**（行池 = **可视行 ×1.5 + 1**，沿用 §5.7 / R-22 范式；其「×1.5」是**下界**——任意滚动偏移下与视口相交的行最多为「可视行 ×1.5」行，`+1` 是为此**边界行**留的一行余量，见下）——理由：P6 全量展开 **428 行 + 装置段 20 行**（白名单口径 = §15.2.4 表：`hvac` 33 + `bms` 330 + `meter_batt` 38 + `pcs` 27 = **428**；装置段 = 站状态条 5 行 + F8 三卡 14 行 + 说明行 1 行 = 20 —— ⚠️ 本节原按「F8 **六行**」记 **11** 行，**该口径差（N-2）仍待随 §15.5.1/§15.5.2 的回写收口**，实现取 UI §6.6.1 的"三卡全部保留"侧），**单段最大 = 段「电池」330 行**（其中 `bms_alarm` 288 位走下钻，段内实际渲染 ≈44 行）⇒ 若逐行建对象会冲击 `LV_MEM_SIZE = 1 MB`（R-24 实测口径）。落地后的余量须真机复核（§15.9 **R-34**）。

> **⚠️ 窗口化的适用范围（T21c-2-r1，2026-09-25 订正 —— 原文写「每段列表**一律**窗口化」）**：
> **段「装置」不窗口化**（实现侧登记为 **P6-5**，本次**回写**为设计侧的**豁免**，理由是"一律"这一
> 字面的原意）：该段的**行集是固定件** —— 站状态表 5 行（= 5 个 role 全集）+ F8 既有三卡
> 的固定行集（`INFO_ROWS 4` / `RUN_ROWS 7` / `ABOUT_ROWS 3` + 说明行），**与点数、与 catalog
> 无关**（对象数由**常量**钉死，无"随数据增长"的风险）⇒ 与 §15.5.3 写"窗口化"的**理由**
> （428 行冲击 1 MB 池）不冲突。**4 个外设段与 BMS 288 位下钻仍**一律窗口化（固定行池 +
> `SCROLL` 重绑，见 §15.5.1）；**P4 探测器下钻亦同**（T21e，2026-09-26 已落地 —— 见本节末尾 ✅ 条与 §15.4）。
>
> **⚠️ 「窗口化」的适用范围订正（T21c-1-r1 原文 —— 其"根因"判断已被证伪，见下一条；**其陈述亦已由 T21e 收口**，见再下一条 ✅）**：**P4 的探测器下钻明细未做窗口化**（`DRILL_ROW_POOL` = 服务端 `page_size` 缺省 **20** ⇒ **20 行 × 9 格 = 180 个格对象一次性建齐**）。该池的行数由**服务端页大小**（≤50）钉住、并如实计入对象预算（§15.9 **R-34**）。
>
> **❌ 订正（T21c-2-r1，2026-09-25）—— 上一段写的"根因 = 薄层 `EventCode` 未镜像 `LV_SCROLL`" 是错误依据**：该事件码**自 B4b（`297b51b`，2026-09-16）起即已镜像**（见 `lvgl/mod.rs` 的 B4b 条目），且 P5 的 **AU6** 早已消费并附探针实测 ⇒ 在 T21c-1-r1 写下该句之日，该通道**已存在 9 天**。错误来源 = `ui/pages/mod.rs` 的 **R2** 陈旧串（B4b 之后未同步）。⇒ **P4 下钻的"不窗口化"是一处无依据的降级**（T21b2 的 R-3 回写明写"本行**不缩小**对 P6 各段的要求"），**P6 已给出可复用范式**（固定行池 + `SCROLL` 重绑 + `adopt_special`）⇒ 当时**登记为待单独立项**（属 **P4 的改动面**，**本批不改 P4**；实现侧登记 = `p4_interlock.rs` 的 **IL29⑥**）。同源陈旧串（`pages/mod.rs` 的 R2、`p3_logs.rs` 的 LG11/R2 注、`ui/tests.rs` 的两处注释）已在本批**一并订正**。
>
> **✅ 已落地（T21e，2026-09-26）—— 上述"待单独立项"已收口**：P4 探测器下钻明细**已按 `BmsDrill` 同一范式窗口化**（实现侧 = `p4_interlock.rs` 的 `Core::wire_scroll`（`SCROLL` 唯一注册点）+ `refresh_drill` 按 `scroll_y` 重绑 + `drill_spacer` 高度占位器）。**池 = 可视行 ×1.5 + 1 = 7 行**（**与页行数无关**：页 20 / 50 行时对象数**不变**，用例实测"注入 50 行页新建 LVGL 对象 = 0 件"）；**占位器是必要条件**（否则行池随窗口移动 ⇒ 容器内容高度缩回视口高 ⇒ 滚不到页尾、页尾行永不进窗口）。**对象预算因此由 1031 降到 922**（§15.9 **R-34**；**−109 是窗口化的直接结果，不是"调数凑绿"**）。⇒ 本节上文「P4 的探测器下钻明细未做窗口化」与「登记为待单独立项」两句**均已过期**，实现侧登记 = `IL29⑥` 的 ✅ 段。
>
> ⚠️ **订正 v2.1-r2（两处）**：① 该版写「P6 全量展开 **555 行**」——**555 含 `fire` 127，而消防落 P4（§15.4）**，P6 应为 **428 行 + 装置段 20 行**（**2026-09-25 / T21c-2-r1 订正**：此处原写「装置段 **11** 行」，其口径 = 「F8 三卡按**六行**」；本节正文已取 **UI §6.6.1「三卡全部保留」**侧 = 站状态条 5 + F8 三卡 14 + 说明行 1 = **20 行**，故本句按现状订正为 **20**，与正文一致；§15.5.1 / §15.5.2 的「F8 既有**六行**」口径差仍按本节 N-2 登记待收口）；② 该版写「n=100 时 1035 行」——**P6 与 n 无关**：唯一随 n 增长的块是 `fire_det`，而它在 **P4**（P4 侧 n=100 时 = 13 + 6×99 = **607 行**）。**"≤ n=100 不裁"的容量结论不受影响**（它针对整帧，§15.2.4）。
...
| **③** | **帧版本不匹配**（EDGE-21 / F26.3） | 帧 `version = 3`（单一真源）；两端同版本发布（部署口径） | **≤3 s** 进入「版本不匹配」降级画面：灰底 + 「**屏与主进程版本不匹配：屏 v3 / 主进程 v2，请刷同版本固件**」+ 「最后成功 12:03:44」；**不黑屏、不显示半帧 / 混版帧、不显示任何数值**。⚠️ **该整屏画面的落地点在本设计的 §15.11 清单里缺位**（本批**只登记**；现行为 = 页眉胶囊取红 + 状态层承担文案，见 §15.9 **R-46**） | 文案与④**字符串不相等**（现场排障需区分：一个刷固件、一个查进程）；`Incompatible` 为**粘性**态，须**一次成功帧**才清除（避免与偶发超时之间抖动） |
...
| **R-34** | `LV_MEM_SIZE = 1 MB` 在 P4/P6 新增对象后的**余量未量化** | 真机（承 R-24） | 中 | 分段惰性创建 + 窗口化列表已按此约束设计；真机 `lv_mem_monitor` / `smaps` 复核（D-5）。**[2026-09-25 · T21c-1-r1 补记账；**经 T21c-2-r1 / T21c-3-r1 / T21e 三次订正**]** 外壳 + 6 页**常驻对象实测 = 1031**（`ui/tests.rs::SHELL_OBJECT_BUDGET`，逐项账见该行注释）：**981**（742 基线 + P4 消防区 ≈239，其中含**总态卡图标徽标 2 件**与 **A1 段顶通告行 1 件**）+ **+35**（T21c-2：P6 段「装置」新增件 = `SegmentedTabs` 11 + 段内容容器与滚动宿主 2 + 站状态表首版 22 = **1016**）+ **+5**（T21c-2-r1 的四列返工：站状态表 5 行**各 +1 个文字槽**）= **1021**（+40）+ **+10**（T21c-3-r1：两页各一枚「名称表可能过期」提示条 = `WarnBanner` 3 件 + `TextButton` 2 件 ⇒ 5 件/页）= **1031**（+50）。⚠️ **原句内的"余量 ≈ 0.8 %"已订正（T21c-2-r1 / 评审 W-g）**：该值与它**自己的算式**（150–200 KB / 1 MB）**差约两个数量级**，属单位 / 数量级笔误，现按下式为准。<br>**另**：P4 下钻行池原先**未做窗口化** ⇒ **20 行 × 9 格 = 180 个格对象整池建齐**。其原先的理由（**IL29⑥**）**已被证伪**：事件码 `EventCode::SCROLL` **自 B4b（`297b51b`，2026-09-16）起即已镜像**，P5 的 **AU6** 早已消费并附探针实测 ⇒ 该处"不窗口化"是**一处无依据的降级**（**不是**结构限制）。原文与逐条订正见 **§15.5.3 的 ❌ 订正块**与 `p4_interlock.rs` 的 **IL29⑥**。<br>✅ **[2026-09-26 · T21e 收口]** **P4 下钻已窗口化**（池 = 可视行 ×1.5 + 1 = **7 行**；`SCROLL` 重绑 + 高度占位器）⇒ 常驻对象 **1031 → 922**（**−109 是窗口化的直接结果**：下钻视图 201 → 92 = `20 行 ×9 格`(180) 改 `7 行 ×(行容器+9 格)`(70) + 占位器 1；**不是"调数凑绿"** —— 同批该预算**降**）。**余量（随实测同步）**：`lv_obj` **≈150–200 B/对象** × **922** ⇒ 常驻 ≈**138–184 KB** vs 1 MB 池 ⇒ **占用 ≈14–18 %、余量 ≈82–86 %**；再叠加"用户把 5 段都点过 + 开过一次下钻"的**惰性半区 694**（`ui/tests.rs::P6_ALL_SEGMENTS_BUDGET`，T21e 同批实测**未变**）⇒ **稳态上界 1616 件 ≈237–316 KB** ⇒ 占用 ≈23–31 %、**余量 ≈69–77 %**（**账上推算、非实测**）。**验收 = 真机 `lv_mem_monitor`** |
| **R-46** | **「版本不匹配」的整屏降级画面至今无落点**：§15.6.2 ③ 要求 ≤3 s 进入灰底整屏「屏与主进程版本不匹配：屏 v3 / 主进程 v2，请刷同版本固件」+「最后成功 …」且**不显示任何数值**；而 §15.11 的 19 条落点里**没有它**，`shell.rs::apply_overlay` 现以 `HeaderChannel::Down` 为**唯一**判据 ⇒ 版本不匹配时屏上是「与主进程数据通道断开（重试中）」+ **保留的旧帧数值**（**错文案 + 违"不显示任何数值"**） | **产品 / UI 指派落点**（本批**只登记**） | 中：现场排障需区分「刷固件 vs 查进程」（§15.6.2 ③ 的不变量） | **本批（T21c-1）的实际行为（如实登记）**：**不是整屏覆盖层**，而是 ① **页眉通道胶囊取红**（`ChannelStatus::Incompatible` 经 `shell.rs::HeaderChannel::from_status` 归一为 `Down` ⇒ 红胶囊「与主进程数据通道断开（重试中）」）+ ② **状态层承担文案**（`state.rs` 的 `ScreenMode::VersionMismatch` / `ChannelStatus::Incompatible{got,expected}` 已就位并有单测，`T-16` 断言两条降级文案**互异**且 `Incompatible` **粘性**）。**缺口 = 整屏落点**（灰底 + 双版本号 + 不显数值）。**为什么不在本批实现**：整屏覆盖层是新增的全局件（跨 `shell.rs` 的图层与 6 页），超出 T21c-1 的窄面；**修复面极小**（改 `apply_overlay` 的判据一处 + 复用既有整屏遮罩件），指派落点后即可做 |
| **R-35** | **字库缺口最终值**：下界 41（PRD）/ 上界 189（误用登记 label 的实测） | 设计 → 实现 | 中 | 以 `T-23` 用例输出为准重算并补 `font_subset_charset.txt`（§15.7） |
| **R-36** | **12-UI 设计文档需补 P4 / P6 版面**（视觉规范权威在 UI 文档） | **UI 评审员 / 产品** | 中：P4/P6 的视觉定稿 | ✅ **本版已补齐**：可实施版面规格落 **UI 设计文档 §6.4.1（P4）/ §6.6.1（P6）/ §5.1 #21（`SegmentedTabs`）/ §2.1 补注（段间距适用范围）**，本设计 §15.4 / §15.5.1 只引其常量；机械断言见 **T-25**。**已于 2026-09-23 UI 复审通过**（该文档的文首 v2.0 标记 `[DESIGN_APPROVED: 2026-09-10]` 原文未动，本次另加 v2.1 增量标记，见其文首） |
...
| **R-45** | **F25.4「站离线变化 ≤2 s 上屏」的端到端口径依赖 02 号南向的「快采」能力**（v2.1-r2 **漏给时延落点**）：判定侧下界 = `stale_timeout_s` = 5 s（**物理**）⇒ 端到端 **6.35 s**（§15.6.1 约束 3） | **跨文档（02 号）+ 产品** | 中：决定 F25.4 按哪条口径验收 | **本设计可保证**：口径 A（**结论成立 → 屏上可见**）= `0.5 + 0.001 + 0.25 + 0.6 = 1.35 s`，**≤ 2 s ⇒ 达标**（**四段**定值 ②–⑤ 见 §15.6.1 表；断言见 T-22）。**口径 B（端到端）= `5 + 1.35 = 6.35 s` ⇒ 不达标**，需 02 号提供**独立于站轮询周期的告警位 / 站活性快采**（用户裁定）。**该能力已在 02 号侧落地** ⇒ **[2026-09-24 状态回写：02 号侧已实现 —— 02 PRD §10（v1.13）/ 02 设计 §12（v1.13-r4）；T8–T10：块级 `interval_ms` + 调度器读组化；首例 hvac 位块 1000 ms 已入生效配置 `mupc/deploy/config/mupc_core_config.production.yaml`]** 02 号已就本行（12 号 R-33 的裁定）新增「块级采集周期覆盖」能力（块级 `interval_ms`；首例 = HVAC 位块 1000 ms、标量维持 5000 ms），**并同时声明两条边界**：① 该能力**只承诺"告警位以更快周期被采集"**（首例 ≤1 s），**不承诺** F25.3 的**端到端** ≤2 s（"兜底 tick 路径"一档 `1.0 + 0.5 + 0.85 = 2.35 s` 由屏侧 `periph_poll_ms` 决定，属本设计 §15.6.1 约束 1 的 **R-33** 口径）；② **站离线（本行 F25.4）的判定下界 = `stale_timeout_s` = 5 s 是阈值语义、与轮询节奏无关** ⇒ 该能力对口径 B **无改善**（仍 6.35 s），须由本设计/产品**就 `stale_timeout_s` 取值另行裁定**（02 号无权改该阈值语义；02 PRD §10.8.1 建议把本行拆为 **R-45a**（告警位快采，02 号覆盖）/ **R-45b**（站离线阈值裁定））。**能力已实现（02 号侧）** ⇒ ① 本设计**仍只登记依赖，不登记其接口签名**（02 号 §12.9 亦声明**不新增任何接口签名**，唯一对消费方可见的变化是既有通道上的**上送频次**）；② **降级口径**：屏侧 / 组帧侧**不得**为此新造第二套离线判据（禁止 `now − last_ok_ms > 2 s`，违反 F25.1 单真源），口径 B 如实登记为不达标；③ **不阻塞**本次实现（02 号落地后口径 B 自动收敛，**屏侧零改动**） |
...
| **v2.1-r14（2026-09-26，PCS 真源迁址订正；未加任何门禁标记）** | **只订正文首「权威点表」行（`:13`）、§15 三审结论注（`:1739`）、§15.1.2 的 N-15 行（`:1775`）三处文件引用，不改任何视觉规格 / 容量数字 / 判据**。来源 = **PCS 通信与控制整体迁入南向**（02 号设计 §13 / ADR-014）后，`mupc/crates/intercore/src/pcs.rs` 与 `mupc/crates/intercore/src/transport/modbus.rs` **已删除** ⇒ 三处原把该两文件当**现行真源**，改指 `mupc/crates/mupc-southd/src/pcs/regs.rs`（寄存器表：`REG_SOC` `:20` / `REG_RUN_STATE` `:22` / `REG_I_A` `:30` / `REG_P_A` `:31` / `REG_P_TOTAL` `:32`；`REG_PHASE_P_A` `:17` / `REG_PHASE_Q_A` `:18`）与 `mupc/crates/mupc-southd/src/pcs/collect.rs`（三相读 `read_three_phase`），并按本文既有「2026-09-26 改注」体例如实标注原指向。**未改**：§15 的任何容量 / 上界 / 时延数字与判据、PRD、任何代码；**既有门禁标记原文与覆盖范围不变**（`[DESIGN_APPROVED: 2026-09-11]` 覆盖 §1–§14、`[DESIGN_APPROVED: 2026-09-23]` 覆盖 §15，**原文均未动**）；**本次未加任何门禁标记**。 |
| **v2.1-r13（2026-09-26）** | **T21e 评审 W-1 / W-2 收尾（只改 §15.5.3 的一处规范句 + 测试侧补几何读回；不重构、不改任何视觉规格与门禁标记）** —— ① **W-1（口径自相矛盾，二选一收口）**：**§15.5.3 的规范句**原写「含数据行驱动的段一律窗口化（**可视行 ×1.5**）」（= 6），而同批 §15.4 / §15.5.3 的 ✅ 块 / §15.9 **R-34** 三处均写「×1.5 **+ 1**」（= 7，与实现 `DRILL_ROW_POOL` 一致）⇒ 现把规范句**收口为「行池 = 可视行 ×1.5 + 1」**：明确 §5.7 / **R-22** 的「×1.5」是**下界**（任意滚动偏移下与视口相交的行**最多**为「可视行 ×1.5」行），`+1` 是为此**边界行**留的**一行余量**。**未改实现**（两处 `DRILL_ROW_POOL` 本就是 `×1.5 + 1`；P6 同款早已如此）。② **W-2（断言偏自比）**：`ui/tests.rs` ④‴ 段的 `drill_bound_rows() == 页行数 − start` 两侧同源于 `start` ⇒ **补几何读回**：`p4_interlock.rs` 新增两个只读回口（`drill_row_y` / `drill_viewport_top`，均读行容器/滚动宿主的实测 `coords()`），用例断言「**视口顶**几何上落在哪个池行内 ⇒ 该池行的**页内行号**（由逐格文本读出、不来自 `start`）必须 = 窗口起点」——把判据从数据面扩到**像素面**。③ **§4.7 补一节落地现状注**：`AlertFeed` 生产侧三投递方（`SouthSink` / 联锁 / 03 设计 §9.3 缺口 1 巡检投 `subtype == "major"`）在投，但 `subscribe()` 本期**无生产消费者** ⇒ **"投得进环、无消费者"，不得读成现场可观测告警**。**未改**：PRD、01 / 02 / 03 号设计、§15 任何**视觉规格**（行高 / 视口 / 分页条 120×48 / 「收起」位置 / 间距 16）、`page_size = 20` 契约、任何既有门禁标记（文首两枚 `[DESIGN_APPROVED]` **原文未动**，与 r4–r12 先例一致） |
| **v2.1-r12（2026-09-26）** | **T21e：P4 探测器下钻的窗口化收口（§15.4 / §15.5.3 / §15.9 R-34；只改文字与数字，不重构、不新建落点）** —— 落 **IL29⑥** 的 ✅ 收口（该处"不窗口化"的依据早经 T21c-2-r1 判为**错误依据**、并**登记为待单独立项**；本批把"待单独立项"改写为**已落地**）：<br>① **§15.5.3** 在 ❌ 订正块后新增 **✅ 已落地（T21e）** 条：P4 下钻已按 `BmsDrill` 同一范式窗口化（`Core::wire_scroll`（`SCROLL` 唯一注册点）+ `refresh_drill` 按 `scroll_y` 重绑 + `drill_spacer` 高度占位器），**池 = 可视行 ×1.5 + 1 = 7 行**；**占位器是必要条件**（否则行池随窗口移动 ⇒ 容器内容高度缩回视口高 ⇒ 滚不到页尾、页尾行永不进窗口）；上文两句（"未做窗口化"/"待单独立项"）**标记为已过期**；豁免段同时补"P4 探测器下钻亦同"。<br>② **§15.4** 在明细列规则表后新增「**下钻明细的行区窗口化（T21e，已落地）**」段：池 7 行 + `SCROLL` 重绑 + 占位器；**「对象数与页行数无关」**由此成立（`page_size` 20 / 50 时行区对象数**相同**，实测"注入 50 行页新建 LVGL 对象 = 0 件"）；`page_size = 20` 的**分页契约不变**（只关行区渲染方式）。<br>③ **§15.9 R-34**：常驻对象 **1031 → 922**（**−109 是窗口化的直接结果**，逐项账 = 下钻视图 201 → 92 = `20 行 ×9 格`(180) 改 `7 行 ×(行容器+9 格)`(70) + 占位器 1；**不是"调数凑绿"** —— 同批该预算**降**）；稳态上界 **1725 → 1616**；余量口径随实测同步（922 件 × `lv_obj` ≈150–200 B ⇒ ≈**138–184 KB** ⇒ 占用 ≈14–18 %、余量 ≈**82–86 %**；稳态上界 1616 件 ⇒ ≈23–31 % / ≈**69–77 %**；**账上推算、非实测**）；`P6_ALL_SEGMENTS_BUDGET` 同批**实测未变（694）**。<br>④ **实现侧（同批）**：`p4_interlock.rs` —— `DRILL_ROW_POOL` 由常量 `20`（= 页大小）改为 **可视行 ×1.5 + 1**、新增 `drill_spacer` / `drill_window_start` / `Core::on_scroll` / `Core::wire_scroll`（**唯一** `SCROLL` 注册点）、`DrillRow` 增行容器、新增只读回口（`drill_pool_size` / `drill_window_start` / `drill_bound_rows` / `drill_visible_row_count` / `scroll_drill`）；`ui/tests.rs` 的 `shell_chain` 新增 ④‴ 段（含"对象数与行数无关"断言：注入 50 行页 ⇒ `PROBE_MOUNTS` 增量 **0**）并重钉 `SHELL_OBJECT_BUDGET = 922`（**零余量、实测**）+ IL29⑥ 的 ✅ 收口 + 台账差额说明。**未改**：PRD、01 / 02 / 03 号设计、P4 下钻的**视觉规格**（行高 / 列 / 分页条 120×48 / 「收起」位置 / 间距 16）、`page_size = 20` 契约、任何既有门禁标记（文首两枚 `[DESIGN_APPROVED]` **原文未动**，与 r4–r11 先例一致）；`p6_system.rs` 的 P6-6 行与 `ui/pages/mod.rs` 的补充段仍留"待单独立项"的**过期串**（本批任务书明令不改 P6 / `mod.rs`）⇒ **如实登记、待各自改动面收口** |
| **v2.1-r11（2026-09-25）** | **§12.3 字体体积的「326 字符」口径标注（只加注、不改任何实测数字，不构成重新评审）** —— T21d 评审把 §12.3 的「326 字符」登记为"陈旧值"。**核查后改判**：该数字**不是笔误而是本节全部体积实测的（历史）前提** —— `1,776,080 B`（10 档位图合计）/ `131,793 B`（32 档未压缩）/ `830,590 B`（`.c` 文本）/ `136,892 B`（`liblvgl_fonts.a`）**都是在 326 字符码表下测的**。若直接把「326」改成「464」，等于**把未重测的数字挂到新前提上 ⇒ 伪造实测**。⇒ **处置**：在 §12.3「码表」段下**加一条口径订正注**（`⚠️ 口径订正（2026-09-25）`），写明 ① 现码表为 **464 字符**（10 档 cmap 交集 **462**、缺口 2 = `✕ U+2715` / `❚ U+275A`，字库源无字形、生产未用）；② 本节下列体积数字**均为 326 字符口径、未按 464 重测**；③ 新口径的线性外推见 **§15.6.3**（10 档位图 **≈1.7–2.7 MB**，磁盘预算 ≤100 MB 不破）；④ **真机/构建机重跑 `gen_fonts.sh` 后应以新实测值替换本节数字**。**本行只做口径标注，不伪造未测数字。**<br>**未改**：§12.3 的任何体积/字节数、§1–§11、§13–§15 正文、文首两枚 `[DESIGN_APPROVED]` 标记（**原文未动**，与 r1–r10 先例一致） |
| **v2.1-r10（2026-09-25）** | **T21c-3-r1：§15.3.1 第 2 / 3 句落地（顶部「名称表可能过期」提示 + 「重试」按钮）+ W-1 收口（只改文字 / 表格注 + 两页与接线层的窄面实现）** —— ① **§15.3.1** 在屏侧行为那段下补「**已落地**」块：两页 API（`set_catalog_stale` / `set_on_catalog_retry`）、落点（P4 = 滚动区顶部；P6 = 段内容区最顶部，页级单件）、占位口径（只在 stale 时显；stale 时滚动视口下移 72 并等量变矮、**底缘不动**；几何单一真源 = UI §6.4.1 / §6.6.1）、接线（成功 ⇒ `on_catalog_read_ok`；失败 ⇒ `on_catalog_read_failed`，**不调 `clear_catalog`**）、「重试」= `ControlIntent::CatalogRetry`（回调只投意图）。② **W-1 收口**（评审 2026-09-25 登记）：失败时**回滚**预置的 `catalog_rev` 到失败前值 + 记 `catalog_failed_rev` 作"同 rev 不自动重发"抑制（`catalog_due` 新增 `failed_rev` 入参）—— "帧内 rev 变化 ⇒ 自动重取"重新有效，且**不会**每拍反复发起。③ **§15.9 R-34 数字同步**：常驻对象 `1021 → 1031`（`+10` = 两页各一枚提示条 5 件；逐项账 = `ui/tests.rs::SHELL_OBJECT_BUDGET`）、稳态上界 `1715 → 1725`。④ **UI 侧同步**：12-UI **§6.4.1 / §6.6.1** 各增「名称表可能过期 提示条」行（矩形 / 尺寸 / 显隐条件 / 连带后果）+ **v2.1-r7** 版本行。**未改**：PRD、01 / 02 / 03 号设计、`mupc-core-bin` / `mupc-southd` / 组帧侧 / `display-proto` 契约结构（**零改动**）、任何既有门禁标记（文首两枚 `[DESIGN_APPROVED]` **原文未动**，与 r4–r9 先例一致） |
| **v2.1-r9（2026-09-25）** | **T21c-2-r1 收口（评审 F4 / W-f / W-g + 提示 N-1…N-5；只改文字 / 表格注；不重构、不新建落点）** —— ① **§15.9 R-34 三处订正**：(a) **前提订正** —— P4 下钻"不窗口化"的原理由（**IL29⑥**：薄层 `EventCode` **未镜像** `LV_SCROLL` ⇒ 窗口化**结构性不可实现**）**已被证伪**（该事件码自 **B4b `297b51b`，2026-09-16** 起即在仓、P5 的 AU6 早已消费）⇒ 改为**"一处无依据的降级 + 待单独立项"**，与 §15.5.3 的 ❌ 订正块同口径；(b) **常驻对象 `981 → 1021`**（`+40` = T21c-2 的 P6 段「装置」`+35`（`SegmentedTabs` 11 + 段内容容器与滚动宿主 2 + 站状态表首版 22）+ T21c-2-r1 四列返工 `+5`（5 行各 +1 个文字槽）；逐项账 = `ui/tests.rs::SHELL_OBJECT_BUDGET`）；(c) **余量口径自洽（W-g）** —— 1021 件 × `lv_obj` ≈150–200 B ⇒ ≈**150–200 KB** vs 1 MB ⇒ 占用 ≈**15–20 %**、**余量 ≈80–85 %**（**不是 0.8 %**；稳态上界 1021 + 694 = **1715 件 ≈257–343 KB** ⇒ 余量 ≈**67–75 %**）。② **§15.5.3 的「订正 v2.1-r2」句**「装置段 **11 行**」→ **20 行**（按现状 = 站状态条 5 + F8 三卡 14 + 说明行 1；`11` 是「F8 六行」旧口径；N-5①）。③ **v2.1-r7 行的 ④ / ⑥** 各加"**已在 r8 / r9 订正**"的历史标记（**不改其正文**，仅使读者不被其陈旧前提 / 0.8 % 绊住）。④ 同批**实现侧**（窄面、一行级）随本版一并落：`ui/tests.rs` 的白名单过滤点**计数面 `src → code`**（**W-f**：行注释掉过滤点 ⇒ 计数由 2 变 0、本网**即红**；此前数 raw `src` ⇒ 静默失效）｜对象台账**补全 4 个段内容容器**（`36 + 4 + 456 + 9 + 189 = 694`，**N-1**；总数 694 本就是实测）｜字库**自述订正**（charset 实测 = **464 字符、单行无尾换行** ⇒ 缺口 2、10 档 cmap 交集 **462**，**N-4**/提示 4）｜`display-proto` 契约用例的 `serde(default)` **反证条件订正**（原写"去掉 `#[serde(default)]` ⇒ 缺键解析 `Err`"**不实** —— 对 `Option<T>` serde 缺键本就 `None`；真条件 = 改成**非可选**，**N-3**）｜`p4_interlock.rs` **IL29⑥** 的 `981` / "余量 ~0.8 %" 同步订正为 `1021` / ≈80–85 %。**未改**：PRD、01 / 02 / 03 号设计、任何既有门禁标记（文首两枚 `[DESIGN_APPROVED]` **原文未动**，与 r4–r8 先例一致） |
| **v2.1-r8（2026-09-25）** | **T21c-2-r1「F2 四列回写 + IL29⑥ 错误依据订正 + 窗口化适用范围回写」（只改文字 / 表格注；不重构、不新建落点）** —— 落 **T21c-2 评审**（`[CODE_REVIEWED: FAILED]` 的 F2 / F3 + 风险 B + W-4）：<br>① **站状态条恢复四列 + 标签缩短（F2，产品裁定 2026-09-25）**：§15.5.1 的行规格表与 §15.5.2 的 `station_status` 行同步为「**站名 │ 状态 │ 成功 `hh:mm:ss` │ 更新 `hh:mm:ss`**」——**四列口径不变**（UI §6.6.1 原文的两列仍是两列），**缩短的是标签**（「最后成功 / 最近更新」→「成功 / 更新」，`ui_text::LAST_OK_SHORT` / `LAST_UPDATE_SHORT`）。**量化依据**：`last_ok_ms` 为真值时合并串「最后成功 12:03:44 · 最近更新 12:03:46」= **424 px** > 段「装置」右槽 **398 px**（R-38 落地即现的**必截断**）；四列后每列 **146 px**（槽 165 px）。**段「装置」5 行表与各段顶同款**。<br>② **§15.3.2 `BitMeta` 增 `inactive_text`（F3）**：两态词**由 catalog 承载**（"屏侧不猜语义"）；首例 = `hvac_di_8`（活跃「运行」/ 非活跃「停止」，PRD **EX-06**），生产者 = `console_host.rs::bit_state_words`（文案取自 `display-proto::ui_text`）；`#[serde(default)]` ⇒ 既有 catalog 兼容；`None` ⇒ 回退通用「非活跃」。§15.3.2 的 `bits` 具名用例表与 §15.5.2 的 `hvac_run` 行同步。<br>③ **§15.5.3 订正 IL29⑥ 的**错误依据****：原文「根因 = 薄层 `EventCode` **未镜像** `LV_SCROLL`」**不实** —— 该事件码**自 B4b（`297b51b`，2026-09-16）起即已镜像**，且 P5 的 **AU6** 早已消费并附探针实测（写下该句之日该通道已存在 **9 天**；错误来源 = `ui/pages/mod.rs` 的 R2 陈旧串）。⇒ **P4 下钻的"不窗口化"是一处无依据的降级**，**登记为待单独立项**（属 P4 的改动面，本批不改 P4）。同源陈旧串（`pages/mod.rs` R2 / `p3_logs.rs` LG11 与 R2 注 / `ui/tests.rs` 两处注释 / `p4_interlock.rs` IL29⑥ / `p6_system.rs` 的 P6-6 行）一并订正。<br>④ **§15.5.3 窗口化适用范围回写（W-4）**：「**每段列表一律窗口化**」→「**含数据行驱动的段**一律窗口化」，并写明**段「装置」的豁免**（行集 = 5 行站状态表 + F8 既有三卡的**固定行集**，与点数 / catalog 无关 ⇒ 对象数由常量钉死，无"随数据增长"风险）。<br>**未改**：PRD、01 / 02 / 03 号设计、任何既有门禁标记（文首两枚 `[DESIGN_APPROVED]` **原文未动**，与 r4–r7 先例一致）。**另**：`N-2`（§15.5.1/§15.5.2 的「F8 既有**六行**」口径差）本批**只如实标注待收口**，未改其正文 |
| **v2.1-r7（2026-09-25）** | **T21c-1-r1「返工落裁定 + 回写」（只改文字 / 表格注；不重构、不新建落点）** —— 落 **T21c-1 评审**（`[CODE_REVIEWED: FAILED]` 的 F1/F2/F4 + W-1…W-5）的**产品 / UI 裁定**与**逐条回写**：<br>① **IL29 三条例外入正文**（§15.4）：在「既有联锁区…一律不动」句后新增例外块 —— (a) **不可用态总态词缩短为「不可用」**（几何：全串 7 字 × 96 = 672 px > 槽 366 px；完整语义由卡标题 + 就地原因带全串 992 px + 三态色/图标冗余承担）；(b) **EDGE-03/EDGE-20 区块角标改「图标-only」形态**（要求保留、28 px 徽标落标题与 latch 胶囊之间的空槽，代价 = 失去文字区分、改由 ⚠/`!` 图标通道承担）；(c) **栅格 / 行高偏差**（卡高 162 vs 164、视口 342 vs 340、§A 行高 44 vs 36/40；`theme` 缺 2 px 档）。线框图内该卡的 96 px 总态词同步由「联锁状态不可用」改为「不可用※」。**实现侧逐条登记 = `p4_interlock.rs` 的 IL29①–⑦**（本批补上，此前 26 处引用、0 处定义）。<br>② **§15.4 `fire_sys_status` 行补「段顶通告另占一行」**：段级 / 站级降级时通告落卡头之下的**段顶状态条**，**位行恒 16 且恒从 bit0 起**（订正实现的"通告占用第 0 行"，评审 W-3②；行数由 `ui/tests.rs` 断言锁）。<br>③ **§15.4 相邻关系表**「下钻『收起』↔ 危险按钮」行补**实现实测 322 px**（本表原按线框复算 316 px；两者均 ≫ 48，评审 E 表的"自报 392 px 不可复现"由此收口）。<br>④ **§15.5.3 窗口化适用范围订正**：**P4 探测器下钻明细未做窗口化**（20 行 × 9 格 = 180 件整池建齐），根因 = 薄层 `EventCode` 未镜像 `LV_SCROLL` ⇒ 实现层结构性不可实现；补 `LV_SCROLL` 后启用。**不缩小对 P6 各段的要求**（评审 W-5 / IL29⑥）。（**⚠️ 该「根因」已在 r8 / r9 订正为不实**：事件码 **自 B4b（`297b51b`，2026-09-16）起即在仓**、P5 的 AU6 早已消费 ⇒ P4 的"不窗口化"是**一处无依据的降级**，已**登记为待单独立项** —— 见 **§15.5.3 的 ❌ 订正块**与 **§15.9 R-34**。**本行保留为历史记录**。）<br>⑤ **§15.6.2 ③ + §15.9 新增 R-46**：「版本不匹配」的**整屏降级画面无落点**（§15.11 的 19 条里缺位）⇒ 本批**只登记**，现行为 = 页眉胶囊取红 + 状态层承担文案（`ScreenMode::VersionMismatch` / `Incompatible{got,expected}` 已就位且 T-16 锁"两条降级文案互异 + 粘性"）；**不实现整屏覆盖层**。<br>⑥ **§15.9 R-34 补记账面依据**：常驻对象实测 **981**（742 → 981 的逐项账含总态卡图标徽标 2 件 + A1 段顶通告行 1 件），按 `lv_obj` 150–200 B 估算 ⇒ 1 MB 池**余量 ≈0.8 %**（**账上推算、非实测**）；真机 `lv_mem_monitor` 为验收项。（**⚠️ 该数字与口径已在 r9 订正**：`981 → 1021`、余量应约 **80–85 %**（稳态上界 1715 件 ⇒ ≈67–75 %）—— 见 **§15.9 R-34**。**本行保留为历史记录**。）<br>**同时**：**12-UI 设计文档** §6.4.1（三条例外 + 表头位名 + 「收起」纵向实测 + A4 汇总分隔符口径确认）/ §8.3（角标形态）同步。**未改**：PRD、01 / 02 / 03 号设计、任何代码、任何既有门禁标记 |
