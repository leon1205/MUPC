...
| **R-45** | **F25.4「站离线变化 ≤2 s 上屏」的端到端口径依赖 02 号南向的「快采」能力**（v2.1-r2 **漏给时延落点**）：判定侧下界 = `stale_timeout_s` = 5 s（**物理**）⇒ 端到端 **6.35 s**（§15.6.1 约束 3） | **跨文档（02 号）+ 产品** | 中：决定 F25.4 按哪条口径验收 | **本设计可保证**：口径 A（**结论成立 → 屏上可见**）= `0.5 + 0.001 + 0.25 + 0.6 = 1.35 s`，**≤ 2 s ⇒ 达标**（**四段**定值 ②–⑤ 见 §15.6.1 表；断言见 T-22）。**口径 B（端到端）= `5 + 1.35 = 6.35 s` ⇒ 不达标**，需 02 号提供**独立于站轮询周期的告警位 / 站活性快采**（用户裁定）。**该能力尚未落地** ⇒ **[2026-09-23 状态回写：需求已立 —— 02 PRD §10（v1.12）/ 02 设计 §12；实现待 S3b-3]** 02 号已就本行（12 号 R-33 的裁定）新增「块级采集周期覆盖」能力（块级 `interval_ms`；首例 = HVAC 位块 1000 ms、标量维持 5000 ms），**并同时声明两条边界**：① 该能力**只承诺"告警位以更快周期被采集"**（首例 ≤1 s），**不承诺** F25.3 的**端到端** ≤2 s（"兜底 tick 路径"一档 `1.0 + 0.5 + 0.85 = 2.35 s` 由屏侧 `periph_poll_ms` 决定，属本设计 §15.6.1 约束 1 的 **R-33** 口径）；② **站离线（本行 F25.4）的判定下界 = `stale_timeout_s` = 5 s 是阈值语义、与轮询节奏无关** ⇒ 该能力对口径 B **无改善**（仍 6.35 s），须由本设计/产品**就 `stale_timeout_s` 取值另行裁定**（02 号无权改该阈值语义；02 PRD §10.8.1 建议把本行拆为 **R-45a**（告警位快采，02 号覆盖）/ **R-45b**（站离线阈值裁定））。**能力"需求已立"但实现仍未落地** ⇒ ① 本设计**仍只登记依赖，不登记其接口签名、不假定其存在**（02 号 §12.9 亦声明**不新增任何接口签名**，唯一对消费方可见的变化是既有通道上的**上送频次**）；② **降级口径**：屏侧 / 组帧侧**不得**为此新造第二套离线判据（禁止 `now − last_ok_ms > 2 s`，违反 F25.1 单真源），口径 B 如实登记为不达标；③ **不阻塞**本次实现（02 号落地后口径 B 自动收敛，**屏侧零改动**） |
