...
> **⚠️ 「窗口化」的适用范围订正（T21c-1-r1，2026-09-25）**：**P4 的探测器下钻明细未做窗口化**（`DRILL_ROW_POOL` = 服务端 `page_size` 缺省 **20** ⇒ **20 行 × 9 格 = 180 个格对象一次性建齐**）。**根因 = 实现层结构性不可实现**：行对象复用**必须先知道滚动位置**，而薄层的 `EventCode` **未镜像 `LV_SCROLL`**（`ui/pages/mod.rs` 的 R2）⇒ 收不到滚动事件。⇒ 该池的行数由**服务端页大小**（≤50）钉住、并如实计入对象预算（981，§15.9 **R-34**）；**薄层补 `LV_SCROLL` 后**再启用「可视行 ×1.5」窗口化（届时该 180 件可降到 ≈54 件）。**本行不缩小对 P6 各段的要求**（P6 段内列表**仍须**窗口化）。
>
...
| **R-34** | `LV_MEM_SIZE = 1 MB` 在 P4/P6 新增对象后的**余量未量化** | 真机（承 R-24） | 中 | 分段惰性创建 + 窗口化列表已按此约束设计；真机 `lv_mem_monitor` / `smaps` 复核（D-5）。**[2026-09-25 · T21c-1-r1 补记账面依据]** 外壳 + 6 页**常驻对象实测 = 981**（`ui/tests.rs::SHELL_OBJECT_BUDGET`，逐项账见该行注释：742 基线 + P4 消防区 ≈239 = 981，其中含**总态卡图标徽标 2 件**与 **A1 段顶通告行 1 件**）；按 LVGL `lv_obj` 量级（**≈150–200 B/对象**）⇒ 对象 ≈**150–200 KB** vs 1 MB 池 ⇒ **余量 ≈ 0.8 %**（**账上推算、非实测**）。**另**：P4 下钻行池**未做窗口化** ⇒ **20 行 × 9 格 = 180 个格对象整池建齐**（**IL29⑥**：薄层 `EventCode` 未镜像 `LV_SCROLL` ⇒ 实现层收不到滚动事件，窗口化**结构性不可实现**；§15.5.3 已同步注明）。**验收 = 真机 `lv_mem_monitor`** |
