...
| **R-34** | `LV_MEM_SIZE = 1 MB` 在 P4/P6 新增对象后的**余量未量化** | 真机（承 R-24） | 中 | 分段惰性创建 + 窗口化列表已按此约束设计；真机 `lv_mem_monitor` / `smaps` 复核（D-5）。**[2026-09-25 · T21c-1-r1 补记账；**经 T21c-2-r1 订正**]** 外壳 + 6 页**常驻对象实测 = 1021**（`ui/tests.rs::SHELL_OBJECT_BUDGET`，逐项账见该行注释）：**981**（742 基线 + P4 消防区 ≈239，其中含**总态卡图标徽标 2 件**与 **A1 段顶通告行 1 件**）+ **+35**（T21c-2：P6 段「装置」新增件 = `SegmentedTabs` 11 + 段内容容器与滚动宿主 2 + 站状态表首版 22 = **1016**）+ **+5**（T21c-2-r1 的四列返工：站状态表 5 行**各 +1 个文字槽**）= **1021**（+40）。按 LVGL `lv_obj` 量级（**≈150–200 B/对象**）⇒ 常驻 ≈**150–200 KB** vs 1 MB 池 ⇒ **占用 ≈15–20 %、余量 ≈80–85 %**；再叠加"用户把 5 段都点过 + 开过一次下钻"的**惰性半区 694**（`ui/tests.rs::P6_ALL_SEGMENTS_BUDGET`）⇒ **稳态上界 1715 件 ≈257–343 KB** ⇒ 占用 ≈25–33 %、**余量 ≈67–75 %**（**账上推算、非实测**）。⚠️ **原句内的"余量 ≈ 0.8 %"已订正（T21c-2-r1 / 评审 W-g）**：该值与它**自己的算式**（150–200 KB / 1 MB）**差约两个数量级**，属单位 / 数量级笔误，现按上式为准。**另**：P4 下钻行池**未做窗口化** ⇒ **20 行 × 9 格 = 180 个格对象整池建齐**。其原先的理由（**IL29⑥**）**已被证伪**：事件码 `EventCode::SCROLL` **自 B4b（`297b51b`，2026-09-16）起即已镜像**，P5 的 **AU6** 早已消费并附探针实测 ⇒ 该处"不窗口化"是**一处无依据的降级**（**不是**结构限制），已**登记为待单独立项**（属 **P4 的改动面**，本批不改 P4）。原文与逐条订正见 **§15.5.3 的 ❌ 订正块**与 `p4_interlock.rs` 的 **IL29⑥**。**验收 = 真机 `lv_mem_monitor`** |
