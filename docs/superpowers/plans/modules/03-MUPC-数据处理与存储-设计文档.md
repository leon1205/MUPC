...
> **版本：** **v1.3-r2**（2026-09-26，**开发交付登记：FLS-03② / FLS-04 落地**）—— 承 **v1.3-r1**（2026-09-23，设计评审员复审 `[DESIGN_APPROVED: 2026-09-23]`，**仅覆盖 §9 增量**）← **v1.3**（2026-09-23，按设计评审意见修订；设计评审复审通过）
...
| S-3 ② | 丢弃须产**一条 `major` 级告警/事件** | ✅ **已实现（v1.3-r2）** | `storage` crate **仍无告警/事件通道**（依赖方向不允许，原判不变）⇒ 由 **core-bin 健康巡检**承接：判据 `mupc-core-bin/src/storage_health.rs:78-92`（`StorageHealthWatch::poll`：读**增量**，增量 > 0 才出文案，含增量条数 + 累计值）、循环 `:101-129`（周期 1 s）、生产接线 `:137-163`、装配点 `startup.rs:1462-1477`（句柄入 `producers` 协作退出名单，标签 `storage_health_timer`，级别常量 `:45` = `"major"`） |
...
| S-4 | 持续失败**按周期聚合告警**、不风暴 | ✅ **已实现（v1.3-r2，与缺口 2 同源）** | 两层：① 结构性缓解沿旧判 —— `since_attempt`（`:228-238`）使"失败后每个 push 都重试"的活锁式风暴不可能发生（恢复靠定时任务 `:476-513`）；② **告警聚合** = 「只在**增量 > 0** 时投 `major`」（`storage_health.rs:78-92`）⇒ 连续失败但无丢弃的周期**一条不发**，真丢弃的周期该周期**恰好一条** |
...
|---|------|---------------|----------|------|
| 1 | S-3② 丢弃无 `major` 事件 | **storage 不发事件**（依赖方向不允许）⇒ 由 **core-bin 的健康巡检任务**读 `dropped_points()/dropped_batches()` 的**增量**，增量 > 0 时调 `AlertFeed::push_system_alert("major", 文案)`（含增量条数 + 累计值）。**只在增量 > 0 时发**⇒ 自然按周期聚合（同时闭合 S-4）。<br>✅ **v1.3-r2 已交付**：新增 `mupc-core-bin/src/storage_health.rs`（判据 `:78-92`、循环 `:101-129`、生产接线 `:137-163`；周期常量 `HEALTH_TICK_MS` `:41` = 1 s；级别常量 `:45` = `"major"`，经**既有** `AlertFeed::push_system_alert` 投递、文案为单行中文含"本周期新增 N 点（M 次超限丢弃），累计 P 点（Q 次）"）；装配点 `startup.rs:1462-1477`（句柄入 `producers` **协作退出名单**，标签 `storage_health_timer`）。<br>**取舍：新增**独立 1 s 任务**而非**复用 `grid_agg_timer` 的 tick —— 理由：聚合任务的收工契约（drain 通道 + 关闭未闭合周期 + 入队）与"读告警计数"无关，混在一个 `select!` 里会让它的停机语义变模糊；"契合既有架构"体现在**同周期（1 s）、同名单（`producers`）** | R-11.5-A3② / A4 / FLS-03 | 新增 1 个 core-bin 任务（≤ 30 行） |
| 2 | S-4 聚合告警 | 同上一条：按"增量 > 0"触发 + 一条 `major` 事件内**携带递增计数**（连续失败时段合并）。<br>✅ **v1.3-r2 已交付**：聚合判据即"增量 > 0 才投"（`storage_health.rs:85-87` 的早退）——**连续失败但无丢弃的周期 0 条告警**；真丢弃的周期该周期**恰好 1 条**（用例 `:233-354` 断言 3 次变更 ⇒ 恰好 3 条，并用"静默窗口内又跑了 ≥4 拍"排除假绿）。⚠️ **口径登记**：本判据按"**丢弃**周期"聚合；若**每个**周期都在丢（缓冲被持续顶穿），则每周期 1 条 —— 与 R-11.5-A4 的"连续失败 10 周期"（= 失败但无丢弃）不同口径，**按设计原文实现** | R-11.5-A4 | 同上 |
| 3 | S-5 采集路径仍 await | **两条路可选**（本章推荐 (a)）：(a) **把 `buffer_telemetry` 改为非阻塞**：内部改 `mpsc::Sender`（`try_send`），`flush/tick` 逻辑搬进 `spawn_flush_timer` 那个 task（旧基线 `:407-439`，本批后 `:476-513`）——但这会改 `buffer_telemetry` 的签名（`await` 仍在，只是不再干 DB 活）；**最小改法**：容量触发时**不在调用栈内 await**，而是 `tokio::spawn(flush_batch(batch))`（**保住签名、去掉阻塞**，代价是失败回填的时序略变，`BatchGuard` 已覆盖）；(b) 维持现状 + 在装配期给 `capacity` 配足够大（把 await 概率降到"每 1000 点一次"）。**推荐 (a) 的最小改法**。<br>✅ **v1.3-r2 已交付（取 (a) 的等价形态，未取 `tokio::spawn` 最小改法）**：`buffer_telemetry`（`services.rs:313-353`）容量触发**不再 drain、不再 await**，只投一次**非阻塞**唤醒（`request_flush` `:355-367`，`mpsc::Sender::try_send`，容量 1、满即合并）；DB 活在**已注册**的 `spawn_flush_timer` 任务里干（`select!` 第三臂 `:501-502` + `flush_wake_once` `:621-632`）。**为什么不用 `tokio::spawn(flush_batch(batch))`**：`flush_batch` 需 `'static` 而 `&self` 拿不到 `Arc`（要改签名），且游离 spawn **不登记在退出编排里** ⇒ 正是 T15/T16 刚修掉的"退出期窄竞态"形态（`startup.rs:459-478` 有同一条裁决）。**代价（如实登记）**：`buffer_telemetry` 的 `Err` 分支退化为永不触发（返回值恒 `Ok`，签名保留 ⇒ 调用点零改动）；容量触发的提交延迟从"调用栈内即刻"变为"已注册任务下一次 `select!` 醒来"（微秒级，且**不再计入采集耗时**） | R-11.5-A5 / FLS-04 | 对接方 |
...
| 11 | `storage/src/services.rs` 失败语义 | **已由 `724225c` 落地**（回填/有界/计数）；**残留 S-3②/S-4/S-5 已由 v1.3-r2 落地**：S-5 落 `services.rs:313-353`（`buffer_telemetry` 无 await）+ `:355-367`（`request_flush`）+ `:476-513`/`:621-632`（已注册 flush 任务接走唤醒）；S-3②/S-4 落新增 `mupc-core-bin/src/storage_health.rs`（§9.3 缺口表逐条给行号） |
| 12 | `mupc-core-bin/src/startup.rs`（`alert_feed` 构造之后） | **v1.3-r2 新增**：`producers.0.push(("storage_health_timer", spawn_storage_health_timer(write_buffer, alert_feed, stop_rx)))`（`:1462-1477`）—— 03 设计 §9.3 缺口 1 的装配点；**与 `flush_timer`/`grid_agg_timer` 同名单**（协作退出），**不得**放入 abort 名单 |
...
| **FLS-03** | 集成 | ✅ **已交付（v1.3-r2 收口）**：① error 日志 ✅（`log_dropped` 旧基线 `:322-330` → 本批 `:385-399`、失败批日志旧 `:484-499` → 本批 `:558-573`）；② `major` 事件 ✅ **新增** —— `mupc-core-bin/src/storage_health.rs:78-92`（增量判据 + 文案含增量与累计）+ `:101-129`（1 s 巡检）+ 装配 `startup.rs:1462-1477`；③ 计数可观测 ✅（`dropped_points/dropped_batches/requeued_batches` 旧基线 `:368-383` → 本批 `:431-447`）。**判据（判别锚）**：`storage_health.rs` 的 `drop_watch_is_silent_when_there_is_no_increment`（`:199`，③ 无增量不发）、`health_loop_emits_exactly_one_major_alert_per_changed_cycle`（`:233`，①② 每周期恰好 1 条 + 级别字面量 `"major"` + 静默窗内 ≥4 拍）、`drop_alert_level_is_major`（`:190`）、`startup_wires_the_health_timer_into_the_cooperative_list`（`:366`，装配接线/协作名单）；**行为级证据**另在 `storage/tests/integration.rs:418`/`:458` 覆盖"丢弃 → 计数 → 告警"的上游链。告警按周期聚合（`连续失败 10 周期不得 ≥10 条告警`）由"仅在增量 > 0 时发"满足（缺口 2）；⚠️ 口径见 §9.3 缺口 2 行的登记（"失败但无丢弃"0 条 / "每个周期都在丢"每周期 1 条） |
| **FLS-04** | 性能测试 | ⚠️ **结构已交付、性能验收未取证（v1.3-r2）**：实现 = §9.3 缺口 3（`buffer_telemetry` 容量触发**不再 await**，改投**非阻塞**唤醒，DB 活搬进**已注册**的 `spawn_flush_timer` 任务；**未取**设计推荐的 `tokio::spawn` 最小改法，理由见该行）。**结构证据**：`services.rs:313-353` 函数体内**无任何 `.await`、无 `flush_batch`**（判别锚 `services.rs` 的 `telemetry_ingest_path_has_no_db_call_in_its_call_stack` `:1050`）；**行为证据**：`storage/tests/integration.rs:458`（正常库上容量触发后**库里 0 行**、且 `pool.close()` 后采集侧**仍拿不到 `Err`** —— 旧实现两种情形都红，已用探针 P1 实测）与 `:418`（唤醒由已注册任务接走并落库，`flush_interval_ms=60_000` 排除周期触发）。**验收缺口（如实登记）**：**采集侧写入 p99 ≤ 10 ms 未在本机实测**（本机无南向真源与 NPU/串口链路，压测环境不具备）⇒ 该验收项**仍需真机/压测**；本批以"**采集调用栈里已经没有 DB 调用点**"这一**结构性**判据替代，**不得**据此宣称 p99 已达标（HST-ELEC-02 的回归需在同一真机压测中一并复核） |
