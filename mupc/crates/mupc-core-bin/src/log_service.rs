...
//!   只作"还有更早的"这一**事实**的载体。**需 PM 裁定**：要恢复"向前翻历史"，须给契约**新增**
//!   一个独立参数（如 `before_seq`）——这属**契约变更**（跨单元动作），本单元不许改。
//!   （原写"见交付报告的「未决」"——该报告不随仓库分发 ⇒ 悬空引用，K 收尾 Q-4 改为就地写清依据。）
//!
...
mod tests {
    ...
    /// **恰在闸门处**停住（实测数字 = `SCAN_READ_BUDGET_LINES + 1` = **200 001**，即"越闸 1 行
    /// 即停"——见下面 `stats.lines_read` 的断言；**不是**读满 300 000 行）。
    /// （原写"实测数字见交付报告"——该报告不随仓库分发 ⇒ 悬空引用，K 收尾 Q-4 改为自足表述。）
    ///
