...
//! PRD / UI 设计 / 技术设计三处口径与渲染层文案均已回写（单元 **B3-2c 文案收口**，
//! 偏差登记 **PD24**）⇒ 本处不再列为"待裁项"。
//!
//! ⚠️ **跨模块缺口（评审重要 4）——✅ 已裁定并修复（2026-09-16）**：上面这张"如实结论"表
//! 曾**到不了用户眼前** —— 渲染端 `local-display/src/ui/pages/p2_config.rs` 的常驻说明与
//! `TEXT_IMPACT_SAVE` 都写「**无需重启装置**」（**反向陈述**），且成功分支**丢弃**后端
//! `message`（只用固定 `TEXT_TOAST_OK` =「保存成功 · 已生效」）。**PM 裁定（2026-09-16）：
//! 接受降级**（不投入"把 6 个字段做成真热生效"的改造）**+ 改屏上文案 + 回写 PRD（CF-04 降级）**。
//! 渲染层已同步（单元 **B3-2c 文案收口**，其偏差登记 **PD24**）：两处文案改为分级口径
//! 「日志级别立即生效 · 连接类参数需重启进程生效」，成功分支**并入回执 `message`**
//! （`p2_config::success_toast_text`，逐字点名需重启的键）⇒ 后端**三处**（回执 `message` /
//! 审计 `reason` / `tracing::warn!`）的真话**已直达屏面**。**本模块（分发行为与上表）未改一行**。
