...
impl BsdiffApplier {
    ...
    fn apply_via_system_bspatch(old_data: &[u8], patch_data: &[u8]) -> Result<Vec<u8>, OtaError> {
        let dir = tempfile::tempdir().map_err(|e| {
            OtaError::DecompressionFailed(format!("创建临时目录失败: {}", e))
        })?;
        ...
        std::fs::write(&old_path, old_data).map_err(|e| {
            OtaError::DecompressionFailed(format!("写入临时旧文件失败: {}", e))
        })?;
        std::fs::write(&patch_path, patch_data).map_err(|e| {
            OtaError::DecompressionFailed(format!("写入临时补丁文件失败: {}", e))
        })?;
        ...
        let output = Command::new("bspatch")
            ...
            .output()
            .map_err(|e| {
                OtaError::DecompressionFailed(format!("bspatch 命令执行失败: {}", e))
            })?;
        ...
        std::fs::read(&new_path).map_err(|e| {
            OtaError::DecompressionFailed(format!("读取补丁结果失败: {}", e))
        })
    }

    /// 纯 Rust bspatch 实现（简化版，用于系统无 bspatch 命令的回退方案）
    fn apply_bspatch_internal(
        old_data: &[u8],
        patch_data: &[u8],
    ) -> Result<Vec<u8>, OtaError> {
        ...
        if &patch_data[..8] != b"BSDIFF40" {
            return Err(OtaError::VerificationFailed("不是有效的 bsdiff 格式".into()));
        }
        ...
        for i in 0..(ctrl_len / 24) {
            ...
            let diff_len = i64::from_le_bytes(patch_data[offset..offset + 8].try_into().unwrap()) as usize;
            let extra_len = i64::from_le_bytes(patch_data[offset + 8..offset + 16].try_into().unwrap()) as usize;
            let seek: i64 = i64::from_le_bytes(patch_data[offset + 16..offset + 24].try_into().unwrap());
