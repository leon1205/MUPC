...
pub async fn get_rewards(
    ...
    for r in &records {
        let action: serde_json::Value = serde_json::from_str(&r.action_json).unwrap_or_default();
        let total_reward = action.get("reward").and_then(|v| v.as_f64()).unwrap_or(0.0);
