...
///
/// 本用例同时给两件事作证：
/// 1. 容量触发**确实**把落库带到了 flush 任务里（判据：`flush_interval_ms = 60_000`，周期触发
///    在本用例期间不可能到点 ⇒ 库里出现行只可能是那次唤醒驱动的）；
/// 2. 触发**不在采集调用栈里**：5 个点只触发一次容量，修复前第 3 个 push 会在**调用栈内**
///    提交掉 3 条（那正是被禁止的阻塞点）。此处改为"先投唤醒、点仍留缓冲"，故按下这几次
///    push 的瞬间库里还是 0 行。
///
