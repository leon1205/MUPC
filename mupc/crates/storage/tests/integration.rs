...
async fn telemetry_insert_and_query() {
    ...
    let id = svc.telemetry.insert(&make_telemetry("dev1", "voltage", 220.0)).await.unwrap();
    assert!(id > 0);
...
async fn telemetry_query_range() {
    let (_, svc) = setup().await;
    let now = Utc::now();
    let t1 = TelemetryPoint { id: None, device_id: "dev1".into(), timestamp: now - Duration::hours(1), metric_name: "v".into(), value: 1.0, quality: 0 };
    let t2 = TelemetryPoint { id: None, device_id: "dev1".into(), timestamp: now, metric_name: "v".into(), value: 2.0, quality: 0 };
    svc.telemetry.insert(&t1).await.unwrap();
    svc.telemetry.insert(&t2).await.unwrap();

    let results = svc.telemetry.query_range("dev1", now - Duration::hours(2), now + Duration::minutes(1)).await.unwrap();
...
async fn telemetry_delete_older_than() {
    let (_, svc) = setup().await;
    let old = TelemetryPoint { id: None, device_id: "dev1".into(), timestamp: Utc::now() - Duration::days(100), metric_name: "v".into(), value: 0.0, quality: 0 };
    svc.telemetry.insert(&old).await.unwrap();
    let deleted = svc.telemetry.delete_older_than(Utc::now() - Duration::days(50)).await.unwrap();
    assert!(deleted > 0);
}
...
async fn fault_insert_and_acknowledge() {
    let (_, svc) = setup().await;
    let id = svc.faults.insert(&make_fault("dev1", "overcurrent", 3)).await.unwrap();
    assert!(id > 0);

    svc.faults.acknowledge(id).await.unwrap();

    let results = svc.faults.query_range(Utc::now() - Duration::hours(1), Utc::now() + Duration::minutes(1)).await.unwrap();
    assert!(!results.is_empty());
...
async fn fault_query_empty() {
    let (_, svc) = setup().await;
    let results = svc.faults.query_range(Utc::now() - Duration::days(1), Utc::now()).await.unwrap();
    assert!(results.is_empty());
}
...
async fn decision_insert_and_query() {
    let (_, svc) = setup().await;
    svc.decisions.insert(&make_decision("AgriculturalIrrigation", r#"{"p_batt": 10}"#)).await.unwrap();
    svc.decisions.insert(&make_decision("DemandControl", r#"{"p_batt": 5}"#)).await.unwrap();

    let recent = svc.decisions.query_recent(10).await.unwrap();
    assert_eq!(recent.len(), 2);

    let by_scene = svc.decisions.get_by_scene("AgriculturalIrrigation", 10).await.unwrap();
...
async fn event_insert_and_query() {
    let (_, svc) = setup().await;
    svc.events.insert(&make_event("startup", "system", "boot complete")).await.unwrap();

    let results = svc.events.query_range(Utc::now() - Duration::hours(1), Utc::now() + Duration::minutes(1)).await.unwrap();
    assert!(!results.is_empty());
}
...
async fn event_purge() {
    let (_, svc) = setup().await;
    let old = SystemEvent { id: None, timestamp: Utc::now() - Duration::days(200), event_type: "old".into(), source: "s".into(), message: "m".into() };
    svc.events.insert(&old).await.unwrap();
    let deleted = svc.events.purge_older_than(Utc::now() - Duration::days(100)).await.unwrap();
    assert!(deleted > 0);
}
...
async fn asset_upsert_update_existing() {
    let (_, svc) = setup().await;
    svc.assets.upsert(&make_asset("dev-002", "ttu")).await.unwrap();
    let mut updated = make_asset("dev-002", "ttu");
    updated.firmware_version = "2.0".to_string();
    svc.assets.upsert(&updated).await.unwrap();

    let found = svc.assets.get_by_device_id("dev-002").await.unwrap().unwrap();
...
async fn asset_list_by_type() {
    let (_, svc) = setup().await;
    svc.assets.upsert(&make_asset("dev-a", "inverter")).await.unwrap();
    svc.assets.upsert(&make_asset("dev-b", "inverter")).await.unwrap();
    svc.assets.upsert(&make_asset("dev-c", "charger")).await.unwrap();
...
async fn writebuffer_flush_on_capacity() {
    ...
    for i in 0..5 {
        wb.buffer_telemetry(make_telemetry("dev-wb", "v", i as f64)).await.unwrap();
    }

    let results = svc.telemetry.query_range("dev-wb", Utc::now() - Duration::minutes(1), Utc::now() + Duration::minutes(1)).await.unwrap();
    assert!(!results.is_empty());
}
...
async fn writebuffer_manual_flush() {
    ...
    wb.buffer_telemetry(make_telemetry("dev-fl", "v", 1.0)).await.unwrap();
    wb.buffer_telemetry(make_telemetry("dev-fl", "v", 2.0)).await.unwrap();

    let count = wb.flush().await.unwrap();
    assert_eq!(count, 2);

    let results = svc.telemetry.query_range("dev-fl", Utc::now() - Duration::minutes(1), Utc::now() + Duration::minutes(1)).await.unwrap();
