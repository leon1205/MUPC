...
// v2.17: 安全包装器违规记录持久化函数
// 由 model_manager 调用（不直接依赖 ai-engine crate）
// 函数签名见 lib.rs 导出

/// 持久化 SafetyRLWrapper 违规记录
///
/// v2.17 新增。调用方需传入已初始化的 SqlitePool。
/// migration 已在 services::run_migrations 中添加 safety_violations 表。
pub async fn insert_safety_violation(
    pool: &SqlitePool,
    timestamp: i64,
    reason: &str,
    proposed_p_ref: f64,
    proposed_k_droop: f64,
    fallback_p_ref: f64,
    fallback_k_droop: f64,
    v_predicted: f64,
    latency_us: i64,
) -> Result<i64, StorageError> {
    let result = sqlx::query(
        "INSERT INTO safety_violations
         (timestamp, reason, proposed_p_ref, proposed_k_droop,
          fallback_p_ref, fallback_k_droop, v_predicted, latency_us)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id",
    )
    .bind(timestamp)
    .bind(reason)
    .bind(proposed_p_ref)
    .bind(proposed_k_droop)
    .bind(fallback_p_ref)
    .bind(fallback_k_droop)
    .bind(v_predicted)
    .bind(latency_us)
    .fetch_one(pool)
    .await
...
/// 查询最近 N 条违规记录
pub async fn query_recent_safety_violations(
    pool: &SqlitePool,
    limit: usize,
) -> Result<Vec<crate::models::SafetyViolation>, StorageError> {
    #[derive(sqlx::FromRow)]
    struct ViolationRow {
        id: i64,
        timestamp: i64,
    ...
    let rows = sqlx::query_as::<_, ViolationRow>(
        "SELECT * FROM safety_violations ORDER BY timestamp DESC LIMIT ?",
    )
    .bind(limit as i64)
    .fetch_all(pool)
    .await
    ...
    Ok(rows
        .into_iter()
        .map(|r| crate::models::SafetyViolation {
            id: Some(r.id),
            timestamp: r.timestamp,
            reason: r.reason,
            proposed_p_ref: r.proposed_p_ref,
            proposed_k_droop: r.proposed_k_droop,
            fallback_p_ref: r.fallback_p_ref,
            fallback_k_droop: r.fallback_k_droop,
            v_predicted: r.v_predicted,
