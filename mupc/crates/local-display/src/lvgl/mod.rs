...
//! 登记处的意义：把"做不到"从散落的注释变成**一份清单** —— 当前**不做**（本清单只登记，
//! 补能力属「薄层收口批」；`src/lvgl/**` 与 `lvgl-sys/allowlist.txt` 是另一个受审表面）。
//!
//! | # | 缺口（本层未提供的符号） | 影响（`ui/**` 侧的后果） | 候选改进 |
//! |---|--------------------------|--------------------------|----------|
//! | **G1** | **无样式读回**：`bg_opa` / `text_font` / `text_color` 三个 `lv_obj_get_style_*` 均未封装（`Obj` 只暴露 `size` / `coords` / `is_hidden` / `has_flag` 这类**几何与旗标**读回） | **"用错常量的视觉退化"在对象层不可判** —— 例：把整屏降级遮罩的不透明度由 20 % 偷换成模态的 62 %、把中央大字由 64 px 换成 24 px、或把 `#FFB020` 换成 `DANGER` 红，**对象级断言全部读不回来**（对象层只剩 `size` 可读，而"字号"若没同步喂进 `set_size` 连尺寸都不变）。`ui/**` 的既有对策是**应用标记**（`shell.rs::TabBg` / `p5_audit.rs::immutable_bg`：记下"本层送进 setter 的那个值"）+ **单一绑定**（建对象与设尺寸同源，`shell.rs::OVERLAY_TITLE_SLOT`）。**残余**：应用标记只证明"本层送出了哪一档"，不证明"LVGL 真的按它渲染"；**字色 / 真实字体**至今无任何网 | 给 `Obj` 补 `bg_opa()` / `text_font()` / `text_color()` 三个读回（各自一行 `lv_obj_get_style_*` 封装），并把对应符号放行进 `lvgl-sys/allowlist.txt`。补齐后：① `shell_chain` 可把"遮罩档位"由"常量相等"升级为"对象实际值相等"；② `ui/**` 的应用标记可退化为冗余校验 |
//! | **G2** | `ObjFlag` **未镜像** `LV_OBJ_FLAG_EVENT_BUBBLE`；`Indev` **未**暴露 `lv_indev_add_event_cb`（该符号**在** `allowlist.txt` 里，只是没封装） | `ui/shell.rs` 的「任何触摸事件重置空闲计时」（UI §4.3）只覆盖**3 个控件**：LVGL 默认不上冒 + `lv_indev_search_obj` 取"命中的最深可点对象" ⇒ 页内按压**到不了**外壳根。详见 `ui/shell.rs` 偏差表 **SH5** | 二选一：① `ObjFlag` 补 `EVENT_BUBBLE`（须由外壳**递归**给整棵子树置位，链上缺一层即断）；② 给 `Indev` 加 `on(EventCode, F)`（**推荐**：一处挂钩覆盖全屏，不碰 `pages/**`） |
//! | **G3** | `Obj` 无"滚到指定位置"封装（`set/get_scroll_dir` / `set/get_scrollbar_mode` 有，`lv_obj_scroll_to_y` 未封装、**也**未进 `allowlist.txt`） | UI §4.3「超时回归 P1 始终从顶部开始」**做不到** ⇒ 回归后 P1 保留上次滚动位置。详见 `ui/shell.rs` 偏差表 **SH12** | 新增 `Obj::scroll_to_y(y)`（或 `scroll_to(x, y)`）一行封装 + `allowlist.txt` 放行 `lv_obj_scroll_to_y`；外壳在 `Core::select` 切回 P1 时调用一次 |
//!
