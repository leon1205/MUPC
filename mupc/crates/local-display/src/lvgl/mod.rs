...
//! - **B4b「薄层能力的消费者」**：[`event::EventCode::SCROLL`] **新增镜像**（`LV_EVENT_SCROLL`；
//!   数值早已随 `lv_event_code_t` 生成 ⇒ 无需新 C 符号）。消费者 = `ui/pages/p5_audit.rs`
//!   （滚动到底 ⇒ 加载更多 **AU6**）与 `ui/shell.rs`（超时回归回顶 **SH12**）。
//!   ⚠️ **B4b 整改**：一度新增的 `indev::Indev::on`（`lv_indev_add_event_cb` 的封装）**已删除**
//!   —— 它的唯一消费者是 SH5「任何触摸重置计时」，而该钩子**在 `LV_STATE_DISABLED` 的命中
//!   对象上不触发**（`lv_indev.c:1339/1342`），SH5 已换成 **evdev 快照机制**（见 `app.rs` 的
...
//! | **G2** | `ObjFlag` **未镜像** `LV_OBJ_FLAG_EVENT_BUBBLE`；`Indev` **未**暴露 `lv_indev_add_event_cb`（该符号**在** `allowlist.txt` 里，只是没封装） | `ui/shell.rs` 的「任何触摸事件重置空闲计时」（UI §4.3）只覆盖**3 个控件**：LVGL 默认不上冒 + `lv_indev_search_obj` 取「命中的最深可点对象」 ⇒ 页内按压**到不了**外壳根。详见 `ui/shell.rs` 偏差表 **SH5** | **✅ 已收口（B4b 整改后）**：① [`obj::ObjFlag::EVENT_BUBBLE`] **已补**（B4a，镜像枚举值，**无需新 C 符号**）+ 薄层单测 —— 但**本批 `ui/**` 无消费者**（未采纳的候选路，理由见 `ui/shell.rs` 的 SH5）；② SH5 **最终取的是"evdev 快照"路**，**不是** `Indev::on`：一度新增的 `indev::Indev::on`（`lv_indev_add_event_cb` 的封装）**已删除**（本单元已无消费者）—— 那条路的钩子**在 `LV_STATE_DISABLED` 的命中对象上不触发**（`lv_indev.c:1339` 的 `is_enabled` 把 `send_event(PRESSED)`（`:1342`）包住，而 `lv_obj_hit_test` 不排除 `DISABLED` ⇒ 命中对象仍是那个禁用控件），而生产命中面真实存在（P2 置灰的「保存」/ 步进器越界 / P4 联锁）；③ `lv_indev_add_event_cb` 的 `allowlist.txt` 条目**保留**（`event.rs` 的 `EventHost for *mut lv_indev_t` 仍引用该符号 ⇒ 不是死符号） |
//! | **G3** | `Obj` 无「滚到指定位置」封装（`set/get_scroll_dir` / `set/get_scrollbar_mode` 有，`lv_obj_scroll_to_y` 未封装、**也**未进 `allowlist.txt`） | UI §4.3「超时回归 P1 始终从顶部开始」**做不到** ⇒ 回归后 P1 保留上次滚动位置。详见 `ui/shell.rs` 偏差表 **SH12** | **✅ 已补齐 + 已消费（B4b）**：[`obj::Obj::scroll_to_y`] / [`obj::Obj::scroll_y`]（+ `allowlist.txt` 放行两个符号；`LV_EVENT_SCROLL` 早已随 `lv_event_code_t` 生成，无需新增）。**消费者**：`ui/shell.rs::Core::tick` 的**强制切页分支**（超时回归那一拍）调 `Core::reset_primary_scroll()` —— ⚠️ 不是 `Core::select`（那会把「主动返回 / 点页签」也清掉滚动位置，属扩大 §4.3；理由见 SH12 行）。**残余（如实）**：本层仍**没有**「内容高 / 剩余可滚量」读回口（`lv_obj_get_scroll_bottom` 一类）⇒ 判「接近底部」只能靠上层自算（P5 走的就是自己的布局算术，见 `p5_audit.rs` 的 **AU6**） |
