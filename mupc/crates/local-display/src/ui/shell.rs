...
//! | SH5 | 【**✅ 已实施（2026-09-16，B4b；B4b 整改已换机制）**】「**任意**触摸（含**禁用态控件** / 弹层 / 空白）都重置计时」（§4.3） | **机制 = evdev 快照驱动**（`app.rs` 输入泵）：[`crate::app::apply_touch_snapshot`]（生产调用点 = [`App::pump`](crate::app::App)）喂过 `TouchSnapshot{pressed: true}` 之后调一次 [`Shell::note_activity`]（**幂等**：只置一个 `Cell`，跨多拍重复无副作用）。evdev 快照是**真实输入源**，与 LVGL 的**命中测试无关** ⇒ 天然覆盖**禁用态控件 / `lv_layer_top()` 上的弹层与 Toast / 空白**。**早先的 `Indev::on` 设备级钩子在禁用态控件上不触发**（B4b 整改实测）：`lv_indev.c:1339` 的 `is_enabled = !lv_obj_has_state(indev_obj_act, LV_STATE_DISABLED)` 把 `send_event(LV_EVENT_PRESSED, …)`（`:1342`）整个包住，而 `lv_obj_hit_test` **不排除** `DISABLED` ⇒ `lv_indev_search_obj` 照样返回那个禁用对象 ⇒ **回调根本不触发**（实测：120×120 且置 `DISABLED` 的子对象按其中点，回调计数 `2 → 2`）。生产命中面**真实存在**：`p2_config.rs`（无改动时「保存」置灰）/ `components.rs`（步进器越界禁用）/ `p4_interlock.rs` ⇒ **已按此换成快照机制**，`Indev::on` 与 `Shell::attach_indev` **一并删除**（本单元已无消费者；`lv_indev_add_event_cb` 的 allowlist 条目**未动** —— `event.rs` 的 `EventHost for *mut lv_indev_t` 仍引用它） | 为什么**对象级**挂钩（外壳根挂 `PRESSED`，[`Shell::wire`] ①）不够：① LVGL **默认不上冒** —— `lv_obj_event.c:391` 的 `event_is_bubbled` 要求**链上每层**自带 `LV_OBJ_FLAG_EVENT_BUBBLE`；② `lv_obj` 构造时**默认 `CLICKABLE`**（`vendor/lvgl/src/core/lv_obj.c:584`），而页眉（0,0,1024,72）/ 内容区（0,72,1024,624）/ 导航条（0,696,1024,72）**恰好铺满** 1024×768，`lv_indev.c:618::lv_indev_search_obj` 取「命中的**最深**可点对象」 ⇒ **真实触摸永远落在某个后代对象上**（无更深命中时返回的是**屏对象本身**，非外壳根 / 非页面子树），外壳根收不到 `PRESSED`；③ 改动前实测：只有返回键 / 6 页签 / 「放弃修改」**这 3 个控件**会重置。另两条候选未采纳：① 递归给整棵子树置 `EVENT_BUBBLE`（B4a 已镜像该枚举）—— 代价是遍历上千对象，且 `lv_layer_top()` 上的弹层 / Toast **不在**外壳根的后代链上（盖不到），且打开冒泡会改变每个页面祖先处理器的收件面；② **设备级 `Indev::on(PRESSED)`** —— 见左栏，**在禁用态控件上不成立**。外壳根那条挂钩**保留**：它承担「非控件区域」的**合成 / 直接投递**（离屏用例的 `send_event` 径） | **✅ 已实施（B4b 整改）**。回归锁（`ui/tests.rs::shell_chain` 的 ④″ 段，**真实触摸径**）：① `apply_touch_snapshot` 按在 **P1 SOC 卡**矩形中心（坐标由 `soc_card().coords()` 算出）⇒ 断言倒计时消失，**同时**断言「本次按压未落到外壳根」；② **禁用态控件**（`TextButton::set_disabled(true)`）上的按压 ⇒ 先证「启用态同一坐标能收到对象级 `PRESSED`」（命中真实性），再证「置禁用后对象级回调消失（`hits` 恒 1）」**而计时照旧重置**；③ 生产接线由源码哨 `app::tests::pump_routes_touch_snapshots_through_apply_touch_snapshot` 钉住（接线被删 / 被**注释** / 退回裸 `self.indev.feed(` / 摘掉 `if snap.pressed` 都会红 —— 哨在**去整行注释**的文本上判）。**探针实测**：摘掉 `apply_touch_snapshot` 里的 `shell.note_activity()` ⇒ ④″ 两条断言分别红（第二条原文 `left: 0, right: 1` 式的计数失败）。**残余（如实）**：`ObjFlag::EVENT_BUBBLE` 这条**未采纳**的路仍留着 ⇒ **该枚举在 `ui/**` 无消费者**（B4a 的能力与单测仍在，不删；日后可用于「父容器统一处理子事件」一类需求） |
...
//! | SH15 | **EDGE-03 原文的「秒级恢复倒计时」改实现为「秒级递增的已断开时长」**（下方 24 px 行 = `N 秒`） | 原文写"倒计时"，但**恢复时刻不可预知**（取决于 mupcd 何时起）⇒ 递减倒计时**没有分母**（既无"总时长"，也无"预计恢复时刻"这类输入）。本层只有 `ChannelStatus` 一个通道态输入，**没有**任何可推算恢复时刻的真源 ⇒ 递减会编造一个假的分母 | **PM 裁定项**：若确需递减，须先给出「恢复时刻预测源」（如后端提供重连退避进度 / 预计恢复时刻）；当前不存在 ⇒ 本层按"已断开时长"落地。文案字符全部取自 §3.6 用字表（`秒` + 数字，既有用法：`12 秒后返回主状态页` / `保持时间不足，还需 12 秒`），**未自造任何新字**（**非**"新增上屏串"：见 [`disconnect_elapsed_text`] 的说明） **✅ PM 已裁定（2026-09-16）**：见 UI 文档附录 **A.8**（SH16/SH19 的实施单独立项 **B3-2c**） |
//! | SH16 | **EDGE-03 原文的「保留最近有效帧并每区块打 `冻结` 角标」不由本层实现**（本层只做遮罩 + 中央文案 + 时长行）—— **实际缺口 = P2 / P4 / P5 / P6 四页 + P3**（**订正**：早先写成"由既有 §8.2 角标承担"，把 P3 的**构件存在**当成了**标记生效**，**不实**；见下） | 逐页核查（**2026-09-15 规格评审复核后订正**）：§3.6 用字表里确有 `冻结`，但 `ui/pages/**` 六个文件里**没有任何一页**把它上屏（`grep 冻结 src/ui/pages` 结果 = 注释）。各页现有的等价可见标记是**别的名字**：① **P1** —— 页内通道条 [`p1_status::P1StatusPage::channel_text`]（通道断 ⇒ `与主进程数据通道断开`）+ 「`数据过期`」角标（`stale_visible()`，**仅在通道正常但帧旧时**打）+ 逐相/逐字段状态点（`Palette::BORDER_CTRL` 空心 = 停更，见 `p1_status::phase_dot_color` / `state::live_dot_for`）；② **P3** —— 页内 `实时日志已断开 · 正在重连...` 条（**构件存在，但生产零调用者**：`P3LogsPage::set_channel(bool)`（`p3_logs.rs:2118`）全仓只有 `ui/tests.rs` 三条用例调它 ⇒ 读通道断时 P3 **不会**自动显示该条 ⇒ **不计入**"有等价标记"）；③ **P2 / P4 / P5 / P6 在"保留最近有效帧"时没有任何可见标记**（数值照旧显示，与实时不可区分）—— 实测：`grep -n "input.channel" src/ui/pages/*.rs` **只命中 P1**（`p4_interlock.rs:1945` 的注释亦自陈"本页不消费 `freshness` / `channel`"）⇒ 通道级降级对这四页**结构上不可见**。**★ 连带发现（如实登记，非本单元引入）**：P3 的 `connected` 缺省 `false`（`p3_logs.rs:1571`，**B2c-2** 引入）⇒ **生产上 P3 恒显"已断开"**（与实际不符的**静态**文案，方向上 fail-closed）。属**接线缺口**：应由 **B3-2b-2** 把真实读通道态喂进 `set_channel`；**现在缺什么** = `app`/`Shell` 侧对 `p3().set_channel(..)` 的调用（`Shell::set_channel` 只驱动页眉胶囊，**不转发到页**） | **PM 裁定项（本层不擅自改 6 页版式）**：候选 = ① 由各页 §8.2 状态点承担（需先给 P2/P4/P5/P6 补"帧冻结"驱动源 —— 它们**不消费** `freshness`，属**页面层**改动）；② 在整屏层按区块画 `冻结` 角标（需知道"每区块"的矩形，本层拿不到，且会与页面版式耦合）。本单元**只登记、不改六页**（任务书明令）。P3 接线缺口同归 **B3-2b-2** **✅ PM 已裁定（2026-09-16）**：见 UI 文档附录 **A.8**（SH16/SH19 的实施单独立项 **B3-2c**） **✅ 已实施（2026-09-16，B3-2c）**：**帧驱动**的区块已在卡头打「`冻结` / `数据过期`」角标 —— **P4**（联锁总态卡 / 触发源卡）与 **P6**（装置信息卡 / 运行信息卡），判据**单一**（`pages::frame_mark` = `PageInput` 的 `channel` / `freshness` / 是否有帧，与 P1 同源），构造期建好、运行期**只切可见性 + 换文案**（`render` 是 1 Hz 热路径）；数值**照旧沿用冻结帧**（EDGE-03 的"保留"要求）。**P2 / P5 不套用**（控制通道驱动，页面数值不随读通道冻结）、**P3 的通道条**由 B3-2b-2 承担 ⇒ 三页**有意不加**冻帧角标。对象预算 730 → **742**（实测，见 `ui/tests.rs`）。 |
//! | SH18 | **弹层遮蔽**：EDGE-20 要求"控制通道可达 ⇒ P2/P4 写操作仍可用"，实现上 `ConfirmDialog` 挂 `lv_layer_top()`、**压在整屏遮罩之上**（模块头裁定 ②）⇒ 写操作走到"确认"阶段时，**屏上没有任何"数值已冻结"提示**（遮罩被弹层盖住）—— 而**这恰是操作者据冻结数值做决定的那一刻**：最需要"这是冻结帧"警示的时机，反而完全无提示 | 层级**有意**如此（`lv_layer_top()` 高于活动屏 ⇒ 弹层永远在整屏层之上，这正是 EDGE-20 写路径可用的**前提**）。问题**不在层级**，而在"提示**只**在整屏层表达" | **PM 裁定项（本单元不实施：跨页改动 + 需 PM 裁定）**。**最小改法建议**：让 P2 / P4 在 `channel == Down` 时对**受影响数值区**复用既有 §8.2 状态点 / `数据过期` 口径（`state::live_dot_for`）—— 即"该页本来就有的构件换一个输入源"，而**不是**把"每区块角标"塞进整屏层 **✅ PM 已裁定（2026-09-16）**：见 UI 文档附录 **A.8**（SH16/SH19 的实施单独立项 **B3-2c**） |
//! | SH19 | **EDGE-20 的"打标"要求未落实**：原文要求"实时数值沿用冻结帧**并打标**"。本层实现的是**整屏遮罩** —— 它是**全局**表达（"整屏都不可信"），**替代不了** §8.2 的**逐字段**可信度标记：屏上哪个数值刚收到、哪个是三分钟前的，整屏遮罩**区分不了** | §8.3 的"并打标"与 §8.2 的"逐字段状态点"是**同一套**语义 —— 可信度是**逐字段**属性，遮罩是**全局**属性，两者不同轴 | 同 **SH18**（**PM 裁定项**）：逐字段标记的驱动源在各页（`freshness`）；最小改法 = P2–P6 复用 `state::live_dot_for`，而**不是**在整屏层造角标。本单元**只登记** **✅ PM 已裁定（2026-09-16）**：见 UI 文档附录 **A.8**（SH16/SH19 的实施单独立项 **B3-2c**） **✅ 已实施（2026-09-16，B3-2c）**：**区块级**可信度标记落在各**帧驱动页的卡头**（见 SH16；不是逐字段状态点，也不是整屏层造角标）。⚠️ **两个属性仍不同轴、并存**：整屏遮罩 = **全局**表达（"整屏都不可信"），区块角标 = **局部**表达（"这几张卡的数值是冻结帧"）—— 本次**未合并**二者，判据各自独立（遮罩看 `Shell::set_channel`，角标看各页的 `PageInput`）。 |
//! | SH20 | **app 层 Toast（`App::toast`）被确认弹层遮挡 ⇒ 用户看不到**（B3-2c 整改 **重要 3** 的**代码内登记**；此前**全仓没有任何一条登记**，只存在于交付报告的正文里 —— 评审在 `src/**` 与 UI 文档里 grep 后确认） | **几何实测（1024×768）**：app Toast 矩形 = `Dimens::TOAST_X/Y/W/H` = **(272,544)–(752,604)**；`ConfirmDialog` 先建**全屏**遮罩 **(0,0)–(1024,768)**、再建**居中**面板（宽 `Dimens::DIALOG_W` = **720** ⇒ x ∈ **[152,872]**；高 = `max(内容高, DIALOG_MIN_H = 420)` **垂直居中** ⇒ **至少** y ∈ **[174,594]**）。两者**都挂 `lv_layer_top()`**，而弹层**建得晚** ⇒ 同层内绘在 app Toast **之上**。面板与 Toast 的 x 区间完全**包含**，y 区间 **594 > 544** ⇒ Toast 的 60 px 里**≥50 px 被面板压住**，余下 ≤10 px 落在**压暗**的遮罩里 ⇒ **基本不可见**。⚠️ 注意区分：**页面** Toast 不受本条影响 —— 它在弹层**之后**创建 ⇒ 同层里绘在弹层**之上**（该性质已由 `p4_interlock.rs::show_result` 登记） | **可达路径**（如实列举）：① `absorb_console` 的 `route` `Err` 分支（回执形态 / 解码不符）**只**弹 app 层兜底 Toast，而它**不关弹层**（关弹层的是页面 `show_result`）⇒ 若此刻 P2 / P4 有弹层（P4 的失败回执**按 IL11 就不关弹层**），那条「操作失败」被埋；② 更宽：**读**端点失败时无合成回执、由 app 层兜底 Toast 承担，而读轮询 1 Hz、弹层可同时开着 ⇒ **弹层打开期间任何 app 层 Toast 都不可见**。**后果**：§2.6「降级可见」在这条路径上**名义成立、实际看不见**。**最小改法选项（供 PM 裁定，本单元只登记不改）**：① 弹层打开期间把 app Toast **挪出面板矩形**（面板高随内容变、又居中 ⇒ 固定 y 躲不开，需按「弹层是否打开」动态定位 —— 该信号**已有现成的每拍输入**：`App::tick` 喂给 [`Shell::set_modal_open`] 的 `p2.dialog_open() \|\| p4.dialog_open()`）；② 弹层打开期间**改走页面 Toast 通道**（页面 Toast 建得晚 ⇒ 绘在弹层之上，见上格）；③ 让 `ConfirmDialog` 的面板给 Toast 让位（**改版式**，需 PM 裁定 + 跨页改动）。**登记位置说明**：本表是本模块「偏差 / 缺口」的**单一登记处**，故记在这里（而非 `app.rs`）—— 判据（弹层矩形 vs Toast 矩形）跨 `components.rs` 与本层[`Shell::set_modal_open`] | **✅ PM 已裁定（2026-09-16）**：见 UI 文档附录 **A.8**（不挪位置/不改版式；写路径失败反馈统一走页面通道；读端点残余**接受**） |
...
//! | SH12 | 【**✅ 已实现（2026-09-16，B4b）**】§4.3「超时回归…不改变页面滚动位置以外的状态（**P1 始终从顶部开始**）」**已落地**：`Core::tick` 的**强制切页分支**在 `select(Main)` 之后调 `Core::reset_primary_scroll()` ⇒ P1 页根（= 纵向滚动容器）`scroll_to_y(0)` | **为什么落在 `tick` 的强制切页分支、而不是 `Core::select`**：§4.3 该行的主语是「**超时回归本身**」（表格里「主动返回」是**另一行**，只写「切 P1；按下即反馈」，未规定滚动位置）⇒ 放进 `select` 会把**主动返回 / 点页签**也清掉滚动位置，属**扩大**规范。**为什么用「滚回」而不是「重建 P1」**：B4a 已补 [`crate::lvgl::obj::Obj::scroll_to_y`]（`lvgl/mod.rs` 的 **G3**）⇒ 一个调用即可、不丢 P1 的注入态；重建会走 `pages/**` 且要 B3 重灌全部状态，收益不成比例。**B4b 整改「建议 3.2」收窄口径**：「回归」隐含「**从别页回来**」⇒ 超时那一刻**已经在 P1** 时**整支跳过**（守卫 `c.current.get() != NavPage::Main.index()`）—— 否则用户**停在 P1 阅读**时会被强制回顶、丢掉阅读位置，而这已超出「超时**回归**」的字面（无页可回归）。⚠️ `*last` **无论是否切页都钉在当下**（守卫只包住「切页 + 回顶」，不包 `*last = Some(now)`）：否则"已在 P1 ⇒ 跳过"会让 `last` 永远停在过去 ⇒ 用户此后**手动**切到 P4 的下一拍就被立刻弹回 P1 | **✅ 已实现（B4b 整改）**。回归锁：`ui/tests.rs::shell_chain` ④‴ 段两条 —— ① 停在 P1（先 `scroll_to_y(80)` 并断言 `scroll_y() > 0`，**否则「不回顶」会退化成恒真**）⇒ 超时**不回顶**；② 切到 P4 后推进到 0 s ⇒ 断言 `current() == Main` 且 `scroll_y() == 0`。**探针实测**：把守卫改成恒真 ⇒ 第 ① 条红（`left: 0, right: 80`）；注释掉 `reset_primary_scroll()` ⇒ 第 ② 条红（`left: 80, right: 0`）。**残余（仍为真）**：「用户**手动**上滚」这一**输入侧**信号在 `ui/**` 仍无人消费（与「程序化回顶」是两回事）—— 现 `EventCode::SCROLL` 已镜像（B4b，供 P5 用），如需「用户手动滚动」另接即可 |
...
struct Core {
    ...
    /// 是否有确认弹层打开（**注入**；值由接线层每拍从**页面**的生产可见查询口取或喂入
    /// —— `P2ConfigPage::dialog_open() || P4InterlockPage::dialog_open()`，B3-2c 已闭合 **SH2**）。
...
impl Shell {
    ...
    fn wire(&self) {
        ...
        //    ⚠️ **它不覆盖页内控件**（LVGL 默认不上冒、且 `lv_indev_search_obj` 取最深可点对象）
        //    —— 那**不是**缺口了（**SH5 已由 B4b 收口**）：真实触摸的"按在哪里都算一次按压"
        //    由**输入泵的快照**承担（`app::apply_touch_snapshot`：喂 `TouchSnapshot{pressed:true}`
        //    就调一次 [`Shell::note_activity`]，**与 LVGL 的命中结果无关**）。
        //    本挂钩保留的理由：**合成 / 直接投递**（`Obj::send_event`）不经 `indev`，
        //    离屏用例与"非控件区域"的语义仍要靠它。
    ...
    ///
    /// **B3-2c 起页侧拿得到**：P2 / P4 各有生产可见的 [`p2_config::P2ConfigPage::dialog_open`] /
    /// [`p4_interlock::P4InterlockPage::dialog_open`]，接线层每拍喂两者**取或**
    /// （原先"由 `UiState.confirm.is_some()` 注入"的做法**已被取代** —— `ControlState::confirm`
    /// 零生产调用者，见偏差 **SH2** 与 `state.rs` 的 **S-4**）。`false` 且 P2 不脏时恢复计时
    /// （从**满时长**重新起算）。
    pub fn set_modal_open(&self, open: bool) {
        self.core.modal_open.set(open);
    }

    /// 手工记一次"用户活动"。**生产调用点 = `app::apply_touch_snapshot`**（输入泵的 evdev 快照
    /// 侧，见 `app.rs`）—— 这是 UI §4.3「**任何**触摸事件重置计时」的完整语义所依赖的那一路。
    ///
    /// # 为什么是"快照驱动"而不是"在 LVGL 的输入设备上挂钩子"
    ///
    /// 早先的实现给 `Indev` 挂设备级 `PRESSED` 回调（`lv_indev_add_event_cb`）。那条路**在
    /// `LV_STATE_DISABLED` 的命中对象上不成立**：`lv_indev.c:1339` 的
    /// `is_enabled = !lv_obj_has_state(indev_obj_act, LV_STATE_DISABLED)` 把
    /// `send_event(LV_EVENT_PRESSED, …)`（`:1342`）整个包住 —— 而 `lv_obj_hit_test`
    /// **不排除** `DISABLED`，`lv_indev_search_obj` 照样返回那个禁用对象 ⇒ **回调根本不触发**
    /// （实测：120×120 且置 `DISABLED` 的子对象，按其中点，回调计数 `2 → 2`）。生产命中面
    /// 真实存在（P2 无改动时置灰的「保存」、步进器越界禁用、P4 联锁）。
    /// evdev 快照是**真实输入源**，与 LVGL 的命中测试**无关** ⇒ 天然覆盖禁用态控件 /
    /// `lv_layer_top()` 上的弹层 / 空白处。
    ///
    /// **幂等**：只置一个 `Cell` —— 同一按压跨多拍重复调用不会累积任何副作用
    /// （输入泵按拍喂快照，按住不放会多次经过这里）。
    ///
    ...
    pub fn tick(&self, now: Instant, clock_text: &str) {
        ...
        if !paused && should_return_home(secs) {
            ...
            //
            // ⚠️ `*last` **无论是否真的切页都要钉在当下**：否则"已在 P1 ⇒ 跳过本分支"会让
            // `last` 永远停在过去 ⇒ 用户此后**手动**切到 P4 的下一拍就被立刻弹回 P1。
            *last = Some(now);
            // §4.3「超时回归本身 | …不改变页面滚动位置以外的状态（**P1 始终从顶部开始**）」
            // （偏差 **SH12**）—— 两处收窄，均见该行：
            //   ① 回顶**只在这一条路径**上做（不放进 `Core::select`）：本行的主语就是"超时
            //      **回归**"，而 §4.3 的"主动返回"行（返回键 / 页签）只写"切 P1；按下即反馈"，
            //      未规定滚动位置 ⇒ 放进 `select` 会把主动切页的滚动位置一并清掉，属**扩大**规范；
            //   ② **已经在 P1 时整支跳过**（B4b 整改「建议 3.2」）："回归"隐含"**从别页
            //      回来**"—— 用户正停在 P1 阅读时被强制回顶，丢掉的是阅读位置，而这已超出
            //      「超时**回归**」的字面（无页可回归）。
            if c.current.get() != NavPage::Main.index() {
                drop(last); // 切页会读页句柄 / 写 LVGL，先放开借用（`select` 不碰 `last_activity`）。
                c.select(NavPage::Main, true);
                c.reset_primary_scroll();
            }
        }
...
impl Core {
    ...
    /// 把 P1 的滚动容器滚回**顶部**（UI §4.3「超时回归…P1 始终从顶部开始」；偏差 **SH12**）。
    ///
    /// # 为什么是"滚回"而不是"重建 P1"
    ///
    /// B4a 已给薄层补上 [`crate::lvgl::obj::Obj::scroll_to_y`]（`lvgl/mod.rs` 的 **G3**）
    /// ⇒ 一个调用即可，且**不丢** P1 的注入态（告警列表 / 遥测值 / 回调槽）。等效替代
    /// "销毁并重建 P1 页根"要重灌全部状态、且会走 `pages/**`，代价不成比例（SH12 原文已记）。
    ///
    /// # 边界（如实）
    ///
    /// - 调用点**只有** [`Core::tick`] 的强制切页分支（超时回归）⇒ 主动返回 / 点页签**不**回顶
    ///   （§4.3 的"主动返回"行未规定滚动位置，见该调用点的说明）；
    /// - 该方法只碰 **P1 页根**这一个对象；`scroll_to_y` 无动画（`LV_ANIM_OFF`）⇒ "切页即回顶"，
    ///   不会出现"切过去之后才慢慢滚上去"（且动画期间 `SCROLLED` 状态会点亮滚动条）。
    fn reset_primary_scroll(&self) {
        self.p1.obj().scroll_to_y(0);
    }
