...
///
/// # ⚠️ 已登记的硬门禁：**真字库下带符号 P 值装不下本槽宽**（P1 / P4 真机验收前必须处置）
///
/// **现象**：真字库在位时，三相卡的 P 数值凡为**负**（充电方向，负号恒 `U+2212`）即被
/// `LongMode::DOTS` 截断 —— `−12.3` 按 DOTS 分支逐字节推算被截成 **`−1...`**
/// （同路径的正值对照已**实测**：`123.4` → `12...`，见下文那条真字库下的红灯）。
/// **影响面**：**P1 页「三相有功」卡的 A/B/C 三相 P 值**（本文件 `render_phases`，
/// 本块之后的 `:785-795`）—— 运维终端上这是**数据被截断展示**。
///
/// **机理**：数值 label 的显式宽度就是本常量 = **2 em**（64 px 档 ⇒ **128 px**），而带符号的
/// 1 位小数数值需 **4 个数字宽 + 1 个点宽 ≈ 2.50 em**（`U+2212` 的 `adv_w` 在**每一档都恒等于
/// 一个数字**，见 `fonts/lv_font_metrics.txt`）⇒ **任何负数都越槽**，与数值大小无关。
///
/// **实测数字**（真源 `fonts/lv_font_metrics.txt`，单位 **1/16 px**：`−` 与数字 = **568**、
/// `.` = **285**）：
/// - **本槽实际档位 = 64 px**（`theme.rs` 的 `TextSlot::PhasePower => FontSize::S64`）：
///   `−12.3` = 4×568 + 285 = **2557/16 = 159.8 px** > 槽宽 **128 px**（**越槽 31.8 px**）。
///   对照：正值 `12.5` = 124.3 px（**恰好放得下**，余量 3.7 px）、`123.4` = 159.8 px（越槽）。
/// - 该比值**逐档恒定**（≈ 2.50 em vs 2 em，10 档**全部**越槽）；按 28 px 档折算即
///   **70.1 px > 56 px**（`12-本地显示终端-测试报告.md` §4.4 举的就是这一档的例）。
///
/// **触发前提（关键）**：`noto-font`（**非默认 feature** —— `local-display/Cargo.toml`
/// 的 `default = []`）+ 先跑 `fonts/gen_fonts.sh` 生成 `fonts/lv_font_noto_sc_*.c`。
/// **为何默认构建看不出来**：默认构建下 `Font::of(..)` 恒 `None` ⇒ 走 `Font::fallback()`
/// （LVGL 内置 `lv_font_montserrat_14`），`123.4` 在这 2 em 槽里**放得下** ⇒ 压根不触发 DOTS
/// 截断 ⇒ **本地全绿是"字库缺席"撑起来的伪绿**。本项在**默认门禁里不可见**，只能由真机 /
/// `--features noto-font` 暴露。
///
/// **同源的第二条事实：`LONG_DOTS` 会就地改写 label 的文本缓冲 ⇒「`DOTS` 只影响绘制」为假。**
/// LVGL v9.5.0 的 `lv_label_set_dots()`（`vendor/lvgl/src/widgets/label/lv_label.c:1368`，
/// 由 `lv_label_refr_text` 的 DOTS 分支 `:1314-1348` 调起）**就地覆写 label 自己的文本缓冲区**
/// （`text[dot_begin + i] = '.'`，随后尾部置 `\0`；被覆盖的字符先存进 `dot[]`，由
/// `lv_label_revert_dots()`（`:1357`）在**下一次** `lv_label_refr_text`（`:1111`）/
/// `lv_label_set_text_vfmt`（`:154`）/ `set_text_internal`（`:988`）时才还原）
/// ⇒ **`lv_label_get_text()` 读回的就是截断串**。
/// 故 `ui/tests.rs::pages_chain` 里那条「`` `DOTS` 只影响**绘制**，文本属性仍完整（截断不是丢数据）``」
/// 的断言（**工作树 `:3692`**／HEAD `:3268`）**在真字库下必红**。
/// **本轮实测（2026-09-18，`cargo test -p local-display --features noto-font -j 2`）**：
/// `368 passed; 1 failed`，**唯一红点就是本条** —— `ui/tests.rs:3692` 报
/// `left: Some("12...")` / `right: Some("123.4")`（与"默认构建 369 全绿"对照，
/// 差值就是这一条）。⚠️ 红灯**停在该断言**（同一用例里后面那条负值断言 `:3698` 未被执行），
/// 故 `−1...` 是按 DOTS 分支逐字节推算的结果，不是在这次运行里直接观测到的。
/// ⇒ 同一批处置时**必须一并裁定**"截断后读回文本"的语义（接受失真 / 改判据 / 另存原文）。
///
/// **本轮口径（PM 裁定）**：**只登记，不动版式常量** —— 加宽槽位 / 缩小字号 / 换字形由 UI 设计裁定；
/// 上面那条断言红**不是**本页的 bug，而是"默认构建伪绿"被真字库戳破。
/// ⚠️ **硬门禁**：本项未处置前，**P1 页**（三相卡带符号数值）与 **P4** 的**真机验收不得通过**。
