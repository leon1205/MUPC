...
//!
//! **【B4a 收窄；T21c-2-r1 再订正】** 剩余缺口**只有"视口级浮动件"**（滚动**事件**与滚动位置
//! 读 / 写**均已具备**，见下）。
//!
//! 核实结论（逐条读 `src/lvgl/event.rs` / `lvgl-sys/allowlist.txt`）：
//! - **❌ 订正（T21c-2-r1，2026-09-25）**：原文写「**[仍为真]** 薄层 `EventCode` **未镜像**
//!   `LV_EVENT_SCROLL`」—— **不实**：该事件码**自 B4b（提交 `297b51b`，2026-09-16）起即已
//!   镜像**（`lvgl/event.rs:91` + `lvgl/mod.rs` 的 B4b 条目），P5 的 **AU6** 早已消费并附探针
//!   实测（`p5_audit.rs:40`）。⇒ "用户**手动**上滚"所需的**滚动事件**本层**拿得到**；本行原文
//!   的判断**已过期**（同源陈旧串 = `pages/mod.rs` 的 R2，本批一并订正）。
//! - **【B4a 订正】** 原文写「`allowlist.txt` 里**没有任何**滚动位置读 / 写 API（只有
//!   `lv_obj_set/get_scroll_dir` 与 `lv_obj_set/get_scrollbar_mode`）⇒ **读不到**当前滚动位置、
//!   也**无法**程序化回顶」—— **读写这两个半句都已过期**：薄层现已补
//!   [`crate::lvgl::obj::Obj::scroll_to_y`] / [`crate::lvgl::obj::Obj::scroll_y`]，
//!   `allowlist.txt` 已放行 `lv_obj_scroll_to_y` / `lv_obj_get_scroll_y`（`lvgl/mod.rs` 的 **G3**）。
//!
//! ⇒ 「手动上滚 ⇒ 浮现按钮」在本层**做不到的不是"感知上滚"，而是"视口级浮动件"**：
//! 页根即滚动容器 ⇒ 任何**页内**浮动件都会随内容滚走（或压住正文）。本页只做出**可实现的部分**，
//! 并**不假装**做了感知：
//! - **恒显**「回到最新」按钮（92×92，UI §6.3）。落点 = **列表区之下的专属带**
//!   （`y = 列表区底 + 缝`，右对齐；见 [`BACK_BAND_H`] / **LG11**）—— **PM 已裁定
//!   （2026-09-15）**维持该形态（理由已由"缺事件"改为"缺视口级浮动件"）；
//! - 点击 ⇒ 触发 [`P3LogsPage::set_on_back_to_latest`] 的意图并把
//!   内部 `auto_follow` 标志复位为 `true`（[`P3LogsPage::set_auto_follow`] 是 B3 的注入入口）；
//! - **具名能力缺口（给后续单元）**：① ~~在 `src/lvgl/event.rs` 镜像 `LV_EVENT_SCROLL`~~
//!   **【B4b 已补齐】**（`EventCode::SCROLL`）；~~② 补 `lv_obj_get_scroll_y` /
//!   `lv_obj_scroll_to_y`~~ **【B4a 已补齐】**（`Obj::scroll_y` / `scroll_to_y`，见 `lvgl/mod.rs`
//!   的 **G3**）⇒ **程序化回顶已可做**；③ 补"**视口级浮动**"能力（或由外壳把按钮放在页根之外）
//!   ——本页的按钮是**页内子对象**（页根即滚动容器 ⇒ 按钮随内容滚动，用户滚到旧日志时它
//!   **不在视口内**，这才是"浮现"做不到的**唯一**原因）。
//!
...
//! | LG11 | 「回到最新」按钮：**§6.3 写"右下浮现"，实现为"右下恒显 + 页内专属带"**（**B2c-2 整改 ②**：原实现为"**右上**恒显 + 叠在首 2 行上、**实遮正文**"） | **薄层能力缺口 = "视口级浮动件"**（❌ **订正，T21c-2-r1**：本行原写"无 `LV_EVENT_SCROLL` ⇒ 感知不到手动上滚"—— **不实**，该事件码自 **B4b（`297b51b`）起即已镜像**，`Obj::scroll_y` / `scroll_to_y` 自 B4a 起已具备；同源陈旧串 = `pages/mod.rs` 的 R2）。**真正缺的是**：页根即滚动容器 ⇒ **任何页内浮动件都会随内容滚走 / 压住内容**，"浮现"因此仍不可做。⇒ 取**可判定的安全形态**：按钮独占列表区之下的**一条带**（`y = 列表区底 + GAP_MIN`，右对齐）—— **任何行都与按钮矩形不相交**（几何实测见 `ui/tests.rs` 的 `pages_chain`），代价是页更长、且滚到旧日志时按钮不在视口内 | 需薄层补 ①~~`LV_EVENT_SCROLL`~~（**B4b 已补**）~~②`lv_obj_get_scroll_y` / `lv_obj_scroll_to_y`~~（**B4a 已补**，见 `lvgl/mod.rs` 的 **G3**）③"视口级浮动"能力；届时改回"右下浮现" **✅ PM 已裁定（2026-09-15）**：维持「**恒显 + 页内专属带**」；UI §6.3 的「右下浮现」已改为「右下**恒显**（落在列表区下方专属带内、不遮正文）」，见 UI 附录 **A.7**。③ 归「薄层收口批」 |
...
struct Core {
    ...
    /// 自动跟随标志（**B3 的注入入口**）。
    ///
    /// **T21c-2-r1 订正**：原文写"本标志不驱动视觉，因为 `EventCode::SCROLL` 未镜像 ⇒ 拿不到
    /// 手动上滚事件"——**不实**（该事件码自 **B4b `297b51b`** 起即已镜像；滚动位置读 / 写自
    /// B4a 起已具备，见 `lvgl/mod.rs` 的 **G3**）。**真正的原因**：本标志的消费者（"手动上滚 ⇒
    /// 停止自动跟随 + 浮现按钮"）需要一个**视口级浮动件**，而页根即滚动容器 ⇒ 页内浮动件会随
    /// 内容滚走；且"手动滚动 vs 程序化滚动"本层亦无区分口 ⇒ 见 **R2**。
...
impl P3LogsPage {
    ...
    ///
    /// ⚠️ **R2（B4a 订正；T21c-2-r1 再订正）**：原文写「薄层读不到滚动位置（无
    /// `LV_EVENT_SCROLL`、无 `lv_obj_get_scroll_y`）」—— **两个半句都已过期**：
    /// `lv_obj_get_scroll_y` / `lv_obj_scroll_to_y` 自 **B4a** 起已封装（`lvgl/mod.rs` 的 **G3**），
    /// `EventCode::SCROLL` 自 **B4b（`297b51b`，2026-09-16）** 起已镜像（`lvgl/event.rs:91`）。
    /// ⚠️ **B4a 那一轮写的「仍为真的是前半句」也不成立**（它把 R2 的陈旧串当了依据）。
    /// **当前真实原因**：该标志的消费者需要一个**视口级浮动件**（页根即滚动容器 ⇒ 页内浮动件
    /// 随内容滚走），且本层无「滚动来源（用户 / 程序化）」判据 ⇒ 该标志当前**不驱动任何视觉
    /// 差异**，本页**不假装**已经实现"手动上滚 ⇒ 停止自动滚动"。
