...
//!
//! **【B4a 收窄】** 剩余缺口**只有"感知手动上滚"**（缺 `EventCode::SCROLL` 镜像）。
//! 「程序化读 / 写滚动位置」自 B4a 起**已具备**（`Obj::scroll_to_y` / `scroll_y`，见下）。
//!
//! 核实结论（逐条读 `src/lvgl/event.rs` / `lvgl-sys/allowlist.txt`）：
//! - **【仍为真】** 薄层 `EventCode` **未镜像** `LV_EVENT_SCROLL`（镜像的是 `PRESSED` /
//!   `RELEASED` / `PRESS_LOST` / `CLICKED` / `LONG_PRESSED` / `LONG_PRESSED_REPEAT` /
//!   `VALUE_CHANGED` / `READY` / `CANCEL` / `DELETE`）⇒ "用户**手动**上滚"这一**输入侧**
//!   信号本层拿不到；
...
//!   ∴ 现状 = "**程序化**读 / 写滚动位置**已可做**；**手动上滚**的**事件**仍拿不到"。
//!
//! ⇒ 「手动上滚 ⇒ 停止自动滚动 + 浮现按钮」在本层**结构性不可实现**（缺的是**输入侧事件**，
//! 不是读写能力）。本页只做出**可实现的部分**，并**不假装**做了感知：
...
//! - **具名能力缺口（给后续单元；B4a 已划掉 ②）**：① 在 `src/lvgl/event.rs` 镜像
//!   `LV_EVENT_SCROLL`（**仍缺**）；~~② 在 `lvgl-sys/allowlist.txt` + `src/lvgl/widgets.rs`
//!   补 `lv_obj_get_scroll_y` / `lv_obj_scroll_to_y`~~ **【B4a 已补齐】**（`Obj::scroll_y` /
//!   `Obj::scroll_to_y`，见 `lvgl/mod.rs` 的 **G3**）⇒ **程序化回顶已可做**，
//!   本页"点击只报意图"不再受此限；③ 补"视口级浮动"
...
//! | LG11 | 「回到最新」按钮：**§6.3 写"右下浮现"，实现为"右下恒显 + 页内专属带"**（**B2c-2 整改 ②**：原实现为"**右上**恒显 + 叠在首 2 行上、**实遮正文**"） | **薄层能力缺口**（见 **R2**，**B4a 已收窄**）：无 `LV_EVENT_SCROLL`（**滚动位置读 / 写 API 自 B4a 起已具备**，`Obj::scroll_y` / `scroll_to_y`）⇒ "浮现"（需感知手动上滚）结构性不可实现；"浮动件"也不可实现（页根即滚动容器 ⇒ 任何页内浮动件都会随内容滚走 / 压住内容）。⇒ 取**可判定的安全形态**：按钮独占列表区之下的**一条带**（`y = 列表区底 + GAP_MIN`，右对齐）—— **任何行都与按钮矩形不相交**（几何实测见 `ui/tests.rs` 的 `pages_chain`），代价是页更长、且滚到旧日志时按钮不在视口内 | 需薄层补 ①`LV_EVENT_SCROLL`（**仍缺**）~~②`lv_obj_get_scroll_y` / `lv_obj_scroll_to_y`~~（**B4a 已补**，见 `lvgl/mod.rs` 的 **G3**）③"视口级浮动"能力；届时改回"右下浮现" **✅ PM 已裁定（2026-09-15）**：维持「**恒显 + 页内专属带**」；UI §6.3 的「右下浮现」已改为「右下**恒显**（落在列表区下方专属带内、不遮正文）」，见 UI 附录 **A.7**。补 `LV_EVENT_SCROLL` 归「薄层收口批」（滚动位置 API 那一半已完成） |
...
struct Core {
    ...
    /// 「回到最新」按钮（**恒显**；见 **R2**）。
    back: TextButton,
    /// 自动跟随标志（**B3 的注入入口**；**B4a 订正**：滚动位置读 / 写能力**已具备**
    /// —— `Obj::scroll_to_y` / `scroll_y`，见 `lvgl/mod.rs` 的 **G3** —— 本标志之所以
    /// **仍不驱动视觉**，是因为"**手动上滚**"这一**输入侧事件**仍拿不到
    /// （`EventCode::SCROLL` 未镜像）⇒ 缺口已收窄，见 **R2**）。
...
impl P3LogsPage {
    ...
    ///
    /// ⚠️ **R2（B4a 订正）**：原文写「薄层读不到滚动位置（无 `LV_EVENT_SCROLL`、
    /// 无 `lv_obj_get_scroll_y`）」—— 后半句**已过期**：`lv_obj_get_scroll_y` / `lv_obj_scroll_to_y`
    /// 均已封装（`lvgl/mod.rs` 的 **G3**）。**仍为真**的是前半句：`EventCode::SCROLL`
    /// **未镜像** ⇒ "用户**手动**上滚"这一**输入侧事件**拿不到，故该标志当前**不驱动任何
    /// 视觉差异**；它是"未来接上滚动**事件**后"的落点，本页**不假装**已经实现
    /// "手动上滚 ⇒ 停止自动滚动"。
