...
//!    154 px）**未能验证**：离屏断言在**默认门禁**下抓不到"可视截断"。
//!
//!    ⚠️ **订正（L 收尾，2026-09-18）**：本行原写的理由是「`LongMode::DOTS` 只影响**绘制**，
//!    [`Ipv4Stepper::text`] 仍返回完整串」—— **该前提为假**（LVGL v9.5.0 的
//!    `lv_label_set_dots()` **就地覆写** label 的文本缓冲区，`lv_label_get_text()` 读回的
//!    就是**截断串**；逐字节依据见 `docs/technical-debt.md` 的 **U-42 ②**，同源登记在
//!    `src/ui/pages/p1_status.rs` 的 `PHASE_P_SLOT_W` 文档注释）。
//!    **真实原因**与之无关：默认构建不带字库 ⇒ `Font::of(..)` 恒 `None`、走 LVGL 内置
//!    西文字体 ⇒ 该串**放得下、压根不触发 DOTS**；真字库在位时截断**是可读回的**
//!    （同源红灯见 `ui/tests.rs::pages_chain` 的 `:3692` 断言，`--features noto-font` 下必红）。
//!    本层仍**无文本度量 API**，故"184 px 够不够"这一项**确实**未能验证；
