...
/// 不透明度（UI §7.3 模态遮罩 62 % / §8.3 整屏降级压暗 20 %）。
pub struct Opacity;
...
impl Opacity {
    ...
    /// **整屏降级**遮罩不透明度（UI §8.3 EDGE-03「整屏遮罩压暗 **20 %**」，开发单元 B3-2b-1）。
    ///
    /// 与 [`Opacity::MASK_PERCENT`]（模态遮罩 62 %）**不是同一件事**：模态遮罩要**压住**底层
    /// 让弹层成为唯一焦点；EDGE-03 的压暗只是"提示数据已冻结"，**底层必须仍然可读**
    /// （§8.3 EDGE-20 要求实时数值沿用冻结帧继续展示）⇒ 取下限 20 %。
    pub const DEGRADE_PERCENT: u8 = 20;
}
