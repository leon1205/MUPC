...
impl ConfirmDialog {
    ...
    /// 遮罩对象（全屏、**可点**、**点击不关闭**）。
    ///
    /// ⚠️ **`CLICKABLE` 是本遮罩的语义核心，不是可有可无的旗标**：模态弹层要求"点遮罩区域
    /// **不**落到页面上"（`new` 里的注释原文「全屏、可点以拦穿透」）。**通道断（EDGE-03）的
    /// 整屏遮罩语义恰好相反** —— 它必须**可穿透**（`Shell::overlay_obj()` **摘掉** `CLICKABLE`），
    /// 否则 §8.3 EDGE-20 的「P2/P4 写操作仍可用」不成立。
    /// 两处各有一条旗标断言（本类的在 `ui/tests.rs::ui_chain`，另一条在 `shell_chain`）——
    /// 没有本读回口，"两者语义相反"在离屏**无从断言**。
