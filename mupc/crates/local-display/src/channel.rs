...
// ═══════════════════════════════════════════════════════════════════════════
// 2′. 版本不匹配的**归一**（U-73 / §15.11 #10；设计 §15.6.2 情形 ③）
// ═══════════════════════════════════════════════════════════════════════════

/// 把 [`Error::ProtoVersion`] 归一为「版本不匹配」的 `(got, expected)`。
///
/// **唯一**能返回 `Some` 的分支就是 `ProtoVersion`（产生点 = 本文件的 [`DisplayChannelClient`]
/// 版本校验）—— 超时 / 连接失败 / HTTP 状态 / JSON 解码一律 `None`：它们**不是**版本问题，
/// 不得借此点亮「屏与主进程版本不匹配」（T-16 要求这两条降级的文案**互不相等**，若归因混淆，
/// 现场就分不清"刷固件"还是"查进程"，设计 §15.6.2 ③）。
///
/// **消费点**：`app.rs::absorb` 的 `Err` 分支 —— 照旧 [`crate::state::DisplayState::record_fail`]
/// 记一次失败，**再**调 `record_incompatible`（后者**粘性**，须一次成功帧才清除）。
/// 归一逻辑放在本层而**不在** `app` 里写 `match e { … }`：错误分类的真源在本文件
/// （`Error` 的产生点与文档都在此），接线层不该复制一份分支表。
pub fn version_mismatch(e: &Error) -> Option<(u8, u8)> {
    match e {
        Error::ProtoVersion(_, got, expected) => Some((*got, *expected)),
        _ => None,
    }
}
