...
//!    **现状（B3-2c 同步，2026-09-16）**：该条已**半落地** —— ① **文案已接**：接线层不再
//!    一律弹通用「操作失败」，改为经 [`crate::app::transport_failure_text`] 分派 ⇒
//!    `RetryWindowExpired` 用 `ui/**` 的**专属串**（`TEXT_RETRY_EXPIRED`，含出路指引），
//!    其余错误才回落 `TRANSPORT_FAIL_TEXT`；② **入口仍缺**："作为新操作重试 + 重新确认"
//!    的那条**交互入口**没有（屏上只给指引，用户得自己重新发起）—— 待单独立项（属
//!    `ui/**` 改动）。
//!    **本错是"双料不可达"**（如实登记，勿高估这条上屏路径的覆盖）：该错误本身由
//!    [`ConsoleClient::retry`] 在重放窗口外**硬拒绝**时产生，而 `retry` 的**生产调用点**在
//!    `app.rs` 的 `begin_write` `Err` 分支（`RetryWindowExpired` 是其中一种 `Err`），该分支
//!    近乎**结构不可达** —— 单次超时 5 s ≪ 30 s 窗口，正常失败路径走不到窗口外。
//!    故①的上屏文案虽有**纯逻辑**回归（`app::tests::failure_paths_reach_the_app_toast` ②），
//!    生产上仍可能永不触发。本单元不修：`console.rs` 无 UI 出口，改 `state.rs` 的通用失败
//!    文案会波及全部失败分支。
//!
...
mod tests {
    ...
    ///
    /// ⚠️ **仍可能假红（B3-2b-1 规格符合性评审 建议 5；如实登记，本单元不改判据）**：本机在
    /// **极端内存压力**（并行编译 / 换页）下可发生**数十秒 ~ 数百秒级**的停顿 —— 历史上曾
    /// 观测到单次 ~745 s；评审在 CPU 饱和时**复现过 2 次**。[`drive`] / [`drive_until`] 的
    /// `budget` 是**墙钟**判据 ⇒ 那种停顿下 30 s 护栏**会假红**（**非代码缺陷**）。
    /// **不要**因此把它改成"拍数判据"（挂死时拍数永不到阈值 ⇒ 真的挂起就**永远不报**）。
