...
impl MetricsStore {
    ...
    pub async fn init(&self) -> Result<(), MonitorError> {
        ...
        fs::create_dir_all(&dir).map_err(|e| {
            MonitorError::StorageError(format!("创建指标存储目录失败: {}", e))
        })?;
        Ok(())
    }
    ...
    pub async fn store(&self, snapshot: &SystemSnapshot) -> Result<(), MonitorError> {
        ...
        let json = serde_json::to_string(snapshot).map_err(|e| {
            MonitorError::StorageError(format!("序列化快照失败: {}", e))
        })?;
    ...
    pub async fn query_range(
        ...
        for entry in fs::read_dir(&dir).map_err(|e| {
            MonitorError::StorageError(format!("读取目录失败: {}", e))
        })? {
            let entry = entry.map_err(|e| {
                MonitorError::StorageError(format!("读取目录条目失败: {}", e))
            })?;
            let path = entry.path();
            ...
            let file = File::open(&path).map_err(|e| {
                MonitorError::StorageError(format!("打开文件失败: {}", e))
            })?;
            let reader = BufReader::new(file);

            for line in reader.lines() {
                let line = line.map_err(|e| {
                    MonitorError::StorageError(format!("读取行失败: {}", e))
                })?;
        ...
        for entry in fs::read_dir(&dir).map_err(|e| {
            ...
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                if name.starts_with("metrics_") && name.ends_with(".jsonl") {
                    ...
                    {
                        let file_dt: DateTime<Utc> = file_date
                            .and_hms_opt(0, 0, 0)
                            .unwrap()
                            .and_utc();
                        if file_dt < cutoff {
...
mod tests {
    ...
    fn make_snapshot() -> SystemSnapshot {
        SystemSnapshot {
            timestamp: Utc::now(),
            cpu: CpuMetrics {
                usage_percent: 50.0, per_core: vec![],
                load_avg_1m: 1.0, load_avg_5m: 1.0, load_avg_15m: 1.0,
                temperature_c: None,
            },
            memory: MemoryMetrics {
                total_mb: 8192, used_mb: 4096, available_mb: 4096,
                swap_total_mb: 0, swap_used_mb: 0, usage_percent: 50.0,
            },
            disk: DiskMetrics {
                total_mb: 65536, used_mb: 32768, available_mb: 32768,
                usage_percent: 50.0, read_iops: 0, write_iops: 0,
            },
            temperature: TemperatureMetrics {
                cpu_temp_c: 45.0, npu_temp_c: None,
                board_temp_c: 40.0, ambient_temp_c: None,
            },
            processes: vec![],
        }
    }
