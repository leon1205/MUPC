...
impl ModelRegistry {
    ...
    /// v3.1: 热切换当前活跃场景的 RL 模型权重（OTA 全量替换路径）
    ///
    /// 因 RKNN 私有格式不支持原地权重更新，采用"下载新 .rknn → 销毁旧 session →
    /// 加载新 session → 原子交换 active"的路径。
    ///
    /// 前置条件：
    /// - 新 .rknn 文件已通过 OTA 下载到 model_dir（文件名通过 manifest 解析）
    /// - SHA256 校验值已更新到 manifest.json
    ///
    /// # Safety
    ///
    /// 调用者（SafeOnlineUpdater）需保证新权重已通过安全校验和性能验证。
    pub async fn hot_swap_current(
        &self,
        new_file_name: &str,
        expected_sha256: &str,
    ) -> Result<(), AiEngineError> {
        let model_path = self.model_dir.join(new_file_name);

        if !model_path.exists() {
            return Err(AiEngineError::ModelLoadFailed(format!(
                "热切换模型文件不存在: {}",
                model_path.display()
            )));
        }
        ...
        // SHA256 校验
        Self::verify_sha256(&model_path, expected_sha256).await?;
        ...
        // 2. 销毁旧 standby（持锁期内仅 take，spawn 在锁释放后）
        let old_standby = {
            let mut standby = self.standby.write().await;
            standby.take()
        };
        if let Some((_old_mode, old_runtime)) = old_standby {
            tokio::spawn(async move {
                if let Err(e) = old_runtime.destroy().await {
                    tracing::error!("销毁旧 standby 模型失败: {}", e);
                }
            });
        }

        // 3. 原子交换：当前 active → standby，新 runtime → active
        //    严格遵守锁顺序：先 standby 后 active
        {
            let mut standby = self.standby.write().await;
            let mut active = self.active.write().await;
