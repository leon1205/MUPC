...
/// 动作输出结构体（v2.0 双参数模式）
///
/// ## 符号约定（v2.15 统一声明，与实时控制模块和训练管线一致）
///
/// **p_ref > 0 = 放电（向电网注入功率）、p_ref < 0 = 充电（从电网吸收功率）。**
/// k_droop >= 0，电压升高时增加放电功率，电压降低时减少放电功率（增加充电）。
///
...
/// v2.15: 动作空间从 5 维精简为 2 维，load_shedding/pv_limit 下沉至策略引擎本地兜底
...
impl RLModel {
    ...
    pub async fn decide_fused(
        ...
        debug_assert_eq!(input.len(), 78, "输入维度必须为 78");
...
mod tests {
    ...
    fn test_parse_action_output_insufficient_dims() {
        let cfg = default_action_space_config();
        assert!(parse_action_output(&[1.0], &cfg).is_none());
    }

    #[test]
    fn test_action_output_no_compens_factor() {
        // 验证 ActionOutput 不包含 compens_factor 字段（v2.15 精简为 2 维）
        let action = ActionOutput {
            p_ref: 0.0,
            k_droop: 0.0,
        };
        assert_eq!(action.k_droop, 0.0);
    }

    #[test]
    fn test_parse_action_output_uses_action_space_config() {
        // 自定义配置：充电上限 30kW，放电上限 40kW
