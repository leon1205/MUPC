...
/// 解析 RL 模型原始输出为 ActionOutput（v3.0: 2 维 tanh + 反归一化）
///
/// ONNX 输出: [p_ref(tanh), k_droop(tanh)] ∈ [-1, 1] × [-1, 1]
/// 反归一化到物理值后 clamp 到 ActionSpaceConfig 边界。
///
/// v2.15: load_shedding/pv_limit 下沉至策略引擎，confidence 移至元数据。
pub fn parse_action_output(raw: &[f32], config: &ActionSpaceConfig) -> Option<ActionOutput> {
    if raw.len() < 2 {
        return None;
    }

    let p_norm = raw[0] as f64; // tanh ∈ [-1, 1]
    let k_norm = raw[1] as f64; // tanh ∈ [-1, 1]

    // 反归一化 p_ref: tanh ∈ [-1,1] → 物理 kW
    // 正值=放电, 负值=充电; 取 max_batt_discharge_power/charge_power 作为幅值
    let p_ref = if p_norm >= 0.0 {
        p_norm * config.max_batt_discharge_power
    } else {
        p_norm * config.max_batt_charge_power
    };

    // 反归一化 k_droop: tanh ∈ [-1,1] → [k_min, k_max]
    let k_min = config.k_droop_min.unwrap_or(-100.0);
    let k_max = config.k_droop_max.unwrap_or(100.0);
    let k_droop = k_norm * (k_max - k_min) / 2.0 + (k_max + k_min) / 2.0;

    // v2.15: load_shedding/pv_limit 下沉至策略引擎本地兜底
    Some(ActionOutput {
        p_ref,
        k_droop,
        load_shedding: 0.0,
...
mod tests {
    ...
    #[test]
    fn test_parse_action_output_charge_direction() {
        let cfg = default_action_space_config();
    ...
    #[test]
    fn test_parse_action_output_k_droop_range() {
        let mut cfg = ActionSpaceConfig::default_config();
        cfg.k_droop_min = Some(-50.0);
        cfg.k_droop_max = Some(50.0);
        // k_norm=1.0 → k_droop=1.0*(50-(-50))/2 + (50+(-50))/2 = 50
        let raw = vec![0.0_f32, 1.0];
        let action = parse_action_output(&raw, &cfg).unwrap();
        assert!((action.k_droop - 50.0).abs() < 0.01);
        // k_norm=-1.0 → k_droop=-50
        let raw = vec![0.0_f32, -1.0];
        let action = parse_action_output(&raw, &cfg).unwrap();
        assert!((action.k_droop + 50.0).abs() < 0.01);
    }
    ...
    #[test]
    fn test_parse_action_output_k_droop_full_sweep() {
        // v3.0: tanh ∈ [-1,1] 反归一化后自然在 [k_min, k_max] 范围内
        let mut cfg = default_action_space_config();
        cfg.k_droop_min = Some(-50.0);
        cfg.k_droop_max = Some(50.0);

        // k_norm=0.5 → k_droop=0.5*50+0=25
        let raw = vec![0.0_f32, 0.5];
        let action = parse_action_output(&raw, &cfg).unwrap();
        assert!((action.k_droop - 25.0).abs() < 0.01);

        // k_norm=-0.8 → k_droop=-0.8*50+0=-40
        let raw = vec![0.0_f32, -0.8];
        let action = parse_action_output(&raw, &cfg).unwrap();
        assert!((action.k_droop + 40.0).abs() < 0.01);
    }
