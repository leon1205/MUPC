...
impl EcdhKeyPair {
    /// 生成新的 P-256 ECDH 密钥对
    ///
    /// 使用操作系统随机数源生成安全的私钥，并通过 p256 crate 计算公钥。
    ///
    /// # 返回
    /// - `Ok(EcdhKeyPair)` 新生成的密钥对
    /// - `Err(WirelessError)` 生成失败
    pub fn generate() -> Result<Self, WirelessError> {
        ...
        // p256 公钥：未压缩格式 04 || x(32B) || y(32B) = 65 字节
        let public_key = {
    ...
    /// 从对方公钥派生共享密钥（p256 ECDH）
    ///
    /// 使用 NIST P-256 椭圆曲线 Diffie-Hellman 密钥交换。
    ///
    /// # 参数
    /// - `peer_public`: 对方公钥（65 字节未压缩格式）
    ///
    /// # 返回
    /// - `Ok(Vec<u8>)` 共享密钥（32 字节）
    pub fn derive_shared_secret(&self, peer_public: &[u8]) -> Result<Vec<u8>, WirelessError> {
        ...
        let secret = SecretKey::from_bytes(
            p256::elliptic_curve::generic_array::GenericArray::from_slice(&self.private_key),
        )
        .map_err(|e| WirelessError::EncryptionError(format!("私钥解析失败: {}", e)))?;

        // 解析对方公钥
        let peer_public_key = PublicKey::from_sec1_bytes(peer_public).map_err(|e| {
            WirelessError::EncryptionError(format!("对方公钥解析失败: {}", e))
        })?;

        // ECDH 共享密钥
        let shared = diffie_hellman(secret.to_nonzero_scalar(), peer_public_key.as_affine());
...
pub fn derive_aes_key(shared_secret: &[u8]) -> Result<Vec<u8>, WirelessError> {
    ...
    // HKDF-SHA256: 从 ECDH 共享密钥派生 AES-256 会话密钥
    let hkdf = Hkdf::<Sha256>::new(None, shared_secret);
    let mut aes_key = vec![0u8; AES256_KEY_LEN];
    hkdf.expand(b"mupc-wireless-aes-key", &mut aes_key)
