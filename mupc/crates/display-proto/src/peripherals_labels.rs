...
///
/// # ⚠️ T21c-1 补的三条（**契约层缺口登记**）
///
/// §15.4 的 A4 呈现规格与 UI §6.4.1 的 P4 版面**用到了三条 §15.7.3「UI 固定文案」表里
/// **没有**的字面量**，而 HMI 侧**不得自造中文串**（本模块的纪律）⇒ 按 §15.7.3 的收口
/// 口径**补入本模块**（不改任何结构、不改既有条目）：
///
/// | 常量 | 字面量 | 出处（**不是**新造词） |
/// |------|--------|------------------------|
/// | [`ui_text::COL_SEQ`] | `序号` | UI §6.4.1「下钻视图通用规格」表**表头**行原文（P4 八列之第 1 列） |
/// | [`ui_text::COUNT_UNIT`] | `只` | §15.4「探测器」行汇总行原文「登记 **N** 只 / 可读 **M** 只」的量词 |
/// | [`ui_text::REGISTERED_READABLE_MISMATCH`] | `登记数与可读数不一致` | UI §6.4.1「A4 探测器（汇总）」行原文（`N != M` 的 `WarnBanner` 提示；§15.4 的「显式提示不一致」只有语义、无字面量） |
///
/// 三条的字符**逐字已在**生成字体 cmap 内（实测：`只` 在 `PERIPH_LABELS` 的
/// `fire_sys_7` 单位里已在用；`序号` / `登记数与可读数不一致` 的全部字符亦在
/// `fonts/font_subset_charset.txt` / `lv_font_cmap.txt` 内）⇒ **无需重跑 `gen_fonts.sh`**、
/// `font_subset_charset.txt` 与 `lv_font_cmap.txt` **零字节变化**（同 UI §3.6 补注 4 的既有口径）。
/// 判据由 H-2 / T-23 的覆盖率用例（`local-display/src/ui/tests.rs`）承担。
pub mod ui_text {
    ...
    /// 只（探测器汇总行的量词；§15.4「登记 **N** 只 / 可读 **M** 只」）。
    pub const COUNT_UNIT: &str = "只";
    /// 登记数与可读数不一致（A4 汇总行的 `N != M` 提示；UI §6.4.1 原文。
    /// **不得静默裁剪**，F21.4 / EX-12）。
    pub const REGISTERED_READABLE_MISMATCH: &str = "登记数与可读数不一致";
    /// 序号（下钻明细表的第 1 列列名；UI §6.4.1「表头」行原文）。
    pub const COL_SEQ: &str = "序号";
