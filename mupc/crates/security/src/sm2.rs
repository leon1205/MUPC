...
pub fn sm2_verify(data: &[u8], signature: &[u8], public_key_pem: &str) -> Result<bool> {
    ...
    let public_key =
        UnparsedPublicKey::new(&ECDSA_P256_SHA256_FIXED, public_key_bytes);
    public_key
...
/// 派生共享密钥（ECDH 风格）
#[cfg(feature = "real_gmsm")]
pub fn sm2_derive_shared_key(
    _key_pair: &Sm2KeyPair,
    _peer_public_key: &[u8],
) -> Result<Vec<u8>> {
